...
#ifdef DX8
static void _BuildWaterVertexDecl(int type, DWORD* dwVSDecl)
{
	int	n = 0;
	...
	else if (type == 1)
	{
		dwVSDecl[n++] = D3DVSD_STREAM(0);					// begin tokens.
		dwVSDecl[n++] = D3DVSD_REG(0, D3DVSDT_FLOAT3);		// x, y, z in world
		dwVSDecl[n++] = D3DVSD_REG(1, D3DVSDT_D3DCOLOR);	// diffuse
		dwVSDecl[n++] = D3DVSD_REG(2, D3DVSDT_FLOAT2);		// u, v coords.
		dwVSDecl[n++] = D3DVSD_END();						// end tokens.
	}
	else
...
bool A3DTerrainWaterRender::LoadWaterResources()
{
 ...
 //	devFmt.nWidth		= pViewParam->Width + 100;
 //	devFmt.nHeight		= pViewParam->Height + 100;
	devFmt.nWidth		= pViewParam->Width ;
 	devFmt.nHeight		= pViewParam->Height;

	devFmt.fmtTarget	= A3DFMT_A8R8G8B8; // we need an alpha channel on refract target to hold water height and terrain height difference
	devFmt.fmtDepth		= A3DFMT_D24X8;

	m_pReflectTarget = new A3DRenderTarget();
	if( !m_pReflectTarget->Init(m_pA3DDevice, devFmt, true, true) )
		return false;

	if( !m_bSimpleWater && m_bExpensiveWater )
	{
		m_pRefractTarget = new A3DRenderTarget();
		if( !m_pRefractTarget->Init(m_pA3DDevice, devFmt, true, true) )
			return false;	
	}

	float fov = m_pHostCamera->GetFOV() + DEG2RAD(5.0f);
	//float fov = m_pHostCamera->GetFOV() ;
	float ratio = devFmt.nWidth * 1.0f / devFmt.nHeight;
	...
	//	Create viewports
	if( !m_pA3DDevice->CreateViewport(&m_pReflectViewport, 0, 0, devFmt.nWidth, devFmt.nHeight,
	...
	if( !m_pA3DDevice->CreateViewport(&m_pRefractViewport, 0, 0, devFmt.nWidth, devFmt.nHeight,
	...
	m_pEdgePixelShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader(SHADER_DIR"ps\\water_edge_ps.txt", false);
...
#ifdef DX8
	DWORD dwVSDecl[20];
	_BuildWaterVertexDecl(0, dwVSDecl);
#else
...
#endif
	m_pEdgeVertexShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadVertexShader(SHADER_DIR"vs\\water_edge_vs.txt", false, dwVSDecl);
	...
	m_pWaterCheapShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader(SHADER_DIR"ps\\water_cheap.txt", false);
	...
	m_pWaterCheapScrollShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader(SHADER_DIR"ps\\water_cheap_scroll.txt", false);
	...
	m_pWaterExpensiveShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader(SHADER_DIR"ps\\water_expensive.txt", false);
	...
	m_pWaterExpScrollShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader(SHADER_DIR"ps\\water_expensive_scroll.txt", false);
	...
	m_pWaterRefractShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader(SHADER_DIR"ps\\waterrefract.txt", false);
	...
	m_pWaterRefractScrollShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader(SHADER_DIR"ps\\waterrefract_scroll.txt", false);
	if( NULL == m_pWaterRefractShader )
	{
		g_A3DErrLog.Log("A3DTerrainWater::Init(), failed to load waterrefract.txt");
		return false;
	...
	m_pWaterAlphaSimple = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader(SHADER_DIR"ps\\water_alpha_simple.txt", false);
	...
	m_pWaterVertexShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadVertexShader(SHADER_DIR"vs\\water_vs.txt", false, dwVSDecl);
	...
	m_pWaveShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader(SHADER_DIR"ps\\waveblend.txt", false);
...
#ifdef DX8
	DWORD vertDeclWaves[20];
	_BuildWaterVertexDecl(1, vertDeclWaves);
#else
...
#endif
	m_pWaveVertexShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadVertexShader(SHADER_DIR"vs\\wavevs.txt", false, vertDeclWaves);
...
#endif
		m_pWaterStaticVertexShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadVertexShader(SHADER_DIR"vs\\water_static_mesh_vs.txt", false, dwVSDecl);
...
bool A3DTerrainWaterRender::RenderReflect(int nAreaArrayIdx, const A3DVECTOR3& vecSunDir, A3DViewport * pCurrentViewport, A3DTerrain2 * pTerrain, A3DSky * pSky, A3DTerrainOutline * pTerrainOutline, 
...
// 	}
	...
	if( !m_bSimpleWater && m_bCanReflect && iNumNormalArea > 0)
	{
		// first render terrain and sky into reflect target
 		m_pA3DDevice->SetRenderTarget(m_pReflectTarget);

#ifdef DX8
		IDirect3DSurface8 * pCurRestoreToTarget = m_pA3DDevice->GetRestoreToRenderTarget();
		IDirect3DSurface8 * pCurRestoreToDepth = m_pA3DDevice->GetRestoreToDepthBuffer();
#else
		IDirect3DSurface9* pCurRestoreToTarget = m_pA3DDevice->GetRestoreToRenderTarget();
		IDirect3DSurface9* pCurRestoreToDepth = m_pA3DDevice->GetRestoreToDepthBuffer();
#endif
		...
		if (m_type != WATER_TYPE_ALPHA_SIMPLE)
		{
			...
			m_pA3DDevice->GetD3DDevice()->SetClipPlane(0, p);
			...
			m_pA3DDevice->SetRestoreToRenderTarget(m_pReflectTarget);
			...
			m_pA3DDevice->SetRestoreToRenderTarget(pCurRestoreToTarget, pCurRestoreToDepth);
		...
		if (m_type != WATER_TYPE_GROUND)
		{
			if( m_bExpensiveWater)
			{
				m_pA3DDevice->SetRenderTarget(m_pRefractTarget);
				m_pA3DDevice->SetRestoreToRenderTarget(m_pRefractTarget);
...
*/
	...
	m_pA3DDevice->GetD3DDevice()->SetRenderState(D3DRS_COLORWRITEENABLE, 0x0000000F);
...
void A3DTerrainWaterRender::SetWaterType(WATER_TYPE type)
{
	...
	else if (type == WATER_TYPE_GROUND)
	{
		if (!m_pGndReflectShader)
		{
			m_pGndReflectShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader(SHADER_DIR"ps\\ground_reflect.txt", false);
			if (!m_pGndReflectShader)
			{
