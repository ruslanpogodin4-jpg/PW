													 ...
													 A3DTerrain2 * pRealTerrain)
{
	...
	for( int idx = 0; idx < m_pWater->GetWaterAreasArrayNum(); idx++)
	{
		...
		for(i=0; i<pAreaArry->GetSize(); i++)
		{
			...
			{
				if (dz1 < 0.0f)
					fDistToCam = a_FastSqrt(dx1 * dx1 + dz1 * dz1);
				else if (dz2 < 0.0f)
					fDistToCam = a_FastSqrt(dx1 * dx1 + dz2 * dz2);
				else
					fDistToCam = -dx1;
			}
			else if (dx2 < 0.0f)
			{
				if (dz1 < 0.0f)
					fDistToCam = a_FastSqrt(dx2 * dx2 + dz1 * dz1);
				else if (dz2 < 0.0f)
					fDistToCam = a_FastSqrt(dx2 * dx2 + dz2 * dz2);
				else
					fDistToCam = -dx2;
			}
			else
...
bool A3DTerrainWaterSimpleAlphaRender::InitWaterAreasHeightRT()
{
	...
	A3DDEVFMT devFmt;
	devFmt.nHeight = m_nWaterAreasRTHeight;
	devFmt.nWidth = m_nWaterAreasRTWidth;
	devFmt.fmtTarget = A3DFMT_A8R8G8B8;
	
	m_pWaterAreasHeightRT = new A3DRenderTarget;
	if( !m_pWaterAreasHeightRT->Init(m_pA3DDevice, devFmt, true, false))
		return false;
	...
	dwVSDecl[n ++] = D3DVSD_REG(1, D3DVSDT_FLOAT3);		// normal
	...
	m_pWaterAreasRTVertexShader = pShaderMan->LoadVertexShader(SHADER_DIR"vs\\water_areas_rt_vs.txt", false, dwVSDecl);
	if( m_pWaterAreasRTVertexShader == NULL)
		return false;
	
	m_pWaterAreasRTPixelShader = pShaderMan->LoadPixelShader(SHADER_DIR"ps\\water_areas_rt_ps.txt", false);
	if( m_pWaterAreasRTPixelShader == NULL)
		return false;
	
	m_pSimpleAlphaEdgeVertexShader = pShaderMan->LoadVertexShader(SHADER_DIR"vs\\simple_alpha_edge_vs.txt", false, dwVSDecl);
	if( m_pSimpleAlphaEdgeVertexShader == NULL)
		return false;
	m_pSimpleAlphaEdgePixelShader = pShaderMan->LoadPixelShader(SHADER_DIR"ps\\simple_alpha_edge_ps.txt", false);
...
bool A3DTerrainWaterSimpleAlphaRender::RenderWaterAreasHeightRT()
{
	...
	//Set render target
	m_pA3DDevice->SetRenderTarget(m_pWaterAreasHeightRT);
