...
void MatrixToQuad(const A3DMATRIX4& mat, A3DQUATERNION& q)
{
	...
	{			 
		...
		{
			s = (float)a_FastSqrt(1.0f + mat._11 - mat._22 - mat._33); // s is 2x
			q.x = s *0.5f;
			s = 0.5f / s;
		...
		{
			s = (float)a_FastSqrt(1.0f + mat._22 - mat._11 - mat._33); // s is 2y
			q.y = s *0.5f;
			s = 0.5f / s;
		...
		{
			s = (float)a_FastSqrt(1.0f + mat._33 - mat._11 - mat._22); // s is 2z
			q.z = s *0.5f;
			s = 0.5f / s;
...
void QuadToAxisAngle(const A3DQUATERNION& q, A3DVECTOR3& vecAxis, FLOAT& vAngle)
{
	...
	sine = (float)a_FastSqrt(1.0f - cosine * cosine);
