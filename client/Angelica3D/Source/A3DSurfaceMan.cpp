...
bool A3DSurfaceMan::LoadCursorSurfaceFromFile(const char* szFileName, A3DCOLOR colorKey, A3DSurface** ppSurface)
{
	...
	else
	{
		...
		D3DFORMAT format = D3DFMT_A8R8G8B8;
#ifdef DX8
		hval = m_pA3DDevice->GetD3DDevice()->CreateImageSurface(image_info.Width, image_info.Height, format, &pDXSurface);
#else
		hval = m_pA3DDevice->GetD3DDevice()->CreateOffscreenPlainSurface(image_info.Width, image_info.Height, format, D3DPOOL_SCRATCH/*D3DPOOL_SYSTEMMEM*/, &pDXSurface, NULL);
#endif
		if (FAILED(hval))
		{
			fileImage.Close();
