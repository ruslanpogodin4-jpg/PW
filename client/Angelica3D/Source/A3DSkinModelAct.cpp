...
//	Action data in file. when SKMDFILE_VERSION >= 6 && < 7
					...
					A3DVECTOR3& vPos, A3DQUATERNION& quRot)
{
	...
	vPos = pBoneTrack->pAbsPosTrack->GetKeyValue(iAbsTime);
	quRot = pBoneTrack->pAbsRotTrack->GetKeyValue(iAbsTime);
	quRot.Normalize();
...
void A3DSkinModelAction::UpdateJointActions(DWORD dwAbsTime)
{
	...
	for (i = 0; i < iNumBone; i++)
	{
		...
		if (m_bAbsTrack && aAbsTracks.Find(iBone) >= 0)
		{
			...
			if (m_pCore->GetBoneStateOnAbsTrack(iJoint, (int)dwAbsTime, vPos, quRot))
				bUseAbsTrack = true;
		}
		...
		else
		{
			...
			m_pCore->GetBoneStateOnTrack(iJoint, (int)dwAbsTime, vPos, quRot);
...
int A3DSMActionChannel::AddActionToRankList(A3DSkinModelAction* pAction, bool bRestart)
{
	...
	while (pos)
	{
		...
		{
			//	Check name
			if (!bRestart && !stricmp(pNode->pAction->GetCoreData()->GetName(), pAction->GetCoreData()->GetName()))
