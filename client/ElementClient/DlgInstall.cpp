...
void CDlgInstall::OnCommandStart(const char * szCommand)
{
	...
	if( 0 == stricmp(GetName(), "Win_Enchase") )
	{
		...
		int nStoneMask = ((CECIvtrStone*)pIvtrB)->GetDBEssence()->combined_switch;
		...
		case CECIvtrItem::ICID_WEAPON:
			...
			bFit = ((nStoneMask & SCS_WEAPON_CANNOT_USE) == 0);
			break;
		case CECIvtrItem::ICID_ARMOR:
			nEquipLevel = ((CECIvtrArmor*)pEquipA)->GetDBEssence()->level;
			bFit = ((nStoneMask & SCS_ARMOR_CANNOT_USE) == 0);
			break;
		case CECIvtrItem::ICID_DECORATION:
			nEquipLevel = ((CECIvtrDecoration*)pEquipA)->GetDBEssence()->level;
			bFit = ((nStoneMask & decoration_mask) != 0);
			break;
		}
		if (!bFit)
		{
			GetGameUIMan()->MessageBox("", GetGameUIMan()->GetStringFromTable(11500), MB_OK,
				A3DCOLORRGBA(255, 255, 255, 160), &pMsgBox);
			pMsgBox->SetLife(3);
			return;
		}
