...
class FShopSearcher
{
	...
	bool IsMatch(const GSHOP_ITEM& item)
	{
		...
		const void* p = _pDataMan->get_data_ptr(item.id, ID_SPACE_ESSENCE, dataType);
		const FASHION_ESSENCE* pEssence = static_cast<const FASHION_ESSENCE*>(p);

		bMatched = bMatched && dataType == DT_FASHION_ESSENCE && _pDlgShop->GetFashionShop()->Gender() == pEssence->gender;

		return bMatched;
	}

public:
	FShopSearcher(CDlgFashionShop* pDlgShop, const ACString& searchPattern)
		: _pDlgShop(pDlgShop)
...
//	将商品 status 分类排序输出
class FShopCollector
{
	CDlgFashionShop::GShopItemsPtr _pItems;	//	商品列表

	typedef CDlgFashionShop::FShopItemIndexArray QShopItemIndexArray;
	...
	int GetCategory(int itemIndex)const
	{
		...
		if (itemIndex >= 0 && itemIndex < _pItems->size())
		{
			...
			if (!CDlgFashionShop::GetFirstBuyStatus(item, &status)) {
				ASSERT(false);
			}
			else {
				switch (status)
...
public:
	...
	FShopCollector(CDlgFashionShop::GShopItemsPtr pItems)
		: _pItems(pItems)
		, _count(0)
	{}
...
bool CDlgFashionShop::ShouldIgnore(const GSHOP_ITEM& item)const {
	return false;
}

CECShopBase& CDlgFashionShop::GetShopData() {
	return const_cast<CECShopBase&>(GetShopDataConst());
}

const CECShopBase& CDlgFashionShop::GetShopDataConst()const {
	bool bIsQShop = (m_pFashionShop == CECFashionShopManager::Instance().QShopFashionShop());
	if (bIsQShop) {
...
CDlgFashionShop::GShopItemsPtr CDlgFashionShop::GetShopItems()
{
	bool bIsQShop = (m_pFashionShop == CECFashionShopManager::Instance().QShopFashionShop());

	return bIsQShop ? globaldata_getgshopitems() : globaldata_getgshopitems2();
}

void CDlgFashionShop::UpdateSearchList()
{
...
bool CDlgFashionShop::SearchByPattern(const ACString& strPattern, FShopItemIndexArray& result, int maxSearchCount)
{
	GShopItemsPtr pItems = GetShopItems();
	FShopSearcher searcher(this, strPattern);
	FShopCollector collector(pItems);
...
bool CDlgFashionShop::GetFirstBuyStatus(const GSHOP_ITEM& item, unsigned int* status/* =NULL */)
{
...
ACString CDlgFashionShop::GetSearchPattern()
{
	// 获取当前搜索框的有效搜索模式
	ACString strPattern;
	if (m_pTxt_Search)
...
void CDlgFashionShop::OnEventDBClk_Lst_Search(WPARAM wParam, LPARAM lParam, AUIObject* pObj)
{
...
void CDlgFashionShop::OnEventKeyDown_Txt_Search(WPARAM wParam, LPARAM lParam, AUIObject* pObj)
{
	if (wParam == VK_ESCAPE)
...
void CDlgFashionShop::OnCommand_Search(const char* szCommand)
{
	if (!m_pLst_Search || m_pLst_Search->GetCount() <= 0) return;
	int nCurSel = m_pLst_Search->GetCurSel();
	if (nCurSel < 0 || nCurSel >= m_pLst_Search->GetCount())
...
void CDlgFashionShop::ShowSearchItem(int itemIndex)
{
	...
	elementdataman* _pDataMan = g_pGame->GetElementDataMan();	//	elements.data
	const GSHOP_ITEM& item = (*items)[itemIndex];

	CECFashionShop::enumFashionType		m_fashionType = CECFashionShop::FT_SUITE;
	...
	p = _pDataMan->get_data_ptr(pEssence->id_sub_type, ID_SPACE_ESSENCE, dataType);
	...
	switch (pSubTypeEssence->equip_fashion_mask) {
		case EQUIP_MASK64_FASHION_HEAD:
			m_fashionType = CECFashionShop::FT_HEAD;
			break;
		case EQUIP_MASK64_FASHION_BODY:
			m_fashionType = CECFashionShop::FT_UPPER_BODY;
			break;
		case EQUIP_MASK64_FASHION_LEG:
			m_fashionType = CECFashionShop::FT_LOWER_BODY;
			break;
		case EQUIP_MASK64_FASHION_FOOT:
		case EQUIP_MASK64_FASHION_WRIST:
			m_fashionType = CECFashionShop::FT_WAIST_N_SHOES;
			break;
		case EQUIP_MASK64_FASHION_WEAPON:
			m_fashionType = CECFashionShop::FT_WEAPON;
			break;
	}

	UpdateFashionType();
	if (m_pFashionShop) {
		m_pFashionShop->ChangeFashionType(m_fashionType);
	}

	m_pFashionModel->Fit(item.id);

	int fashionidx(-1);
	for (int i = 0; i < m_pFashionShop->Count(); i++) {

		const CECFashionShop::FashionSelection* pSelectedFashion = m_pFashionShop->ItemAt(i);

		int idx = pSelectedFashion->itemIndex;
		if (idx == itemIndex) {
			fashionidx = i;
			break;
		}
	...
	if (fashionidx >= 0) {
		int barlevel = fashionidx / FASHION_SHOP_ITEM_WIDTH;
		m_pScl_Item->SetBarLevel(barlevel);
	}

	SelectFashionShopItem(fashionidx);
