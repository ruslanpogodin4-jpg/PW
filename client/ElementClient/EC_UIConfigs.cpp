...
CECVipConfig& CECVipConfig::Instance()
{
	static CECVipConfig s_dummy;
	return s_dummy;
}

CECVipConfig::CECVipConfig()
{
}

bool CECVipConfig::Load(const char* filePath)
{
	m_VipScoreNeeded.clear();
	m_VipScoreReducedPerDay.clear();
	...
	if (ItemFile.Open(filePath))
	{
		//	Vip Score Needed
		{
			const ACHAR* szSec = _AL("VipScoreNeed");
			int i(1);
			ACString key;
			do {
				key.Format(_AL("Lv%d"), i);
				int value = ItemFile.GetValueAsInt(szSec, key, 0);
				if (!value) {
					break;
				}
				if (m_VipScoreNeeded.find(value) == m_VipScoreNeeded.end()) {
					m_VipScoreNeeded[i] = value;
				}
				++i;
			} while (true);
		}

		//	Vip Score Reduced Per Day
		{
			const ACHAR* szSec = _AL("VipScoreReducePerDay");
			int i(1);
			ACString key;
			do {
				key.Format(_AL("Lv%d"), i);
				...
				if (m_VipScoreReducedPerDay.find(value) == m_VipScoreReducedPerDay.end()) {
					m_VipScoreReducedPerDay[i] = value;
				}
				++i;
			} while (true);
		}
	...
	{
		a_LogOutput(1, "CECVipConfig::Init, Failed to Open %s", filePath);
	}

	return true;
}

int CECVipConfig::GetVipScoreNeededByLevel(int level)const
{
	int scoreNeeded = -1;
	// If level is higher than highest level, return highest level score
	if (CECVipConfig::ReachedVipMaxLevel(level)) level = m_VipScoreNeeded.size(); // Size could be different than max level, but shouldnt.
	
	VipScoreNeededMap::const_iterator cit = m_VipScoreNeeded.find(level);
	if (cit != m_VipScoreNeeded.end()) scoreNeeded = cit->second;

	return scoreNeeded;
}

int CECVipConfig::GetVipScoreReducePerDayByLevel(int level)const
{
	int scoreToReduce = -1;
	VipScoreReduceMap::const_iterator cit = m_VipScoreReducedPerDay.find(level);
	if (cit != m_VipScoreReducedPerDay.end()) scoreToReduce = cit->second;
	return scoreToReduce;
}

bool CECVipConfig::ReachedVipMaxLevel(int level)const
{
	return (level > 0 && (size_t)level >= m_VipScoreNeeded.size());
}
