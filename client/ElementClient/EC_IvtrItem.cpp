...
CECIvtrItem* CECIvtrItem::CreateItem(int tid, int expire_date, int iCount, int idSpace/* 0 */)
{
	...
	case DT_FIREWORKS2_ESSENCE:		pItem = new CECIvtrFirework2(tid, expire_date);		break;
	...
	case DT_ITEM_PACKAGE_BY_PROFESSION_ESSENCE:pItem = new CECIvtrOccupPackage(tid, expire_date); break;
	case DT_ASTROLABE_INC_EXP_ESSENCE: pItem = new CECIvtrAstrolabeExpPill(tid, expire_date); break;
	case DT_ASTROLABE_INC_INNER_POINT_VALUE_ESSENCE: pItem = new CECIvtrAstrolabePointIncPill(tid, expire_date); break;
	case DT_ASTROLABE_RANDOM_ADDON_ESSENCE: pItem = new CECIvtrAstrolabeRandomAddonPill(tid, expire_date); break;
	case DT_FIX_POSITION_TRANSMIT_ESSENCE:pItem = new CECIvtrFixPositionTransmit(tid, expire_date); break;
	default:
	{
	//	ASSERT(0);
