...
public:		//	Types
	...
	struct BATTLEINFO
	{
		...
		bool IsArenaBattle() const { return nType == BT_ARENA; }
	...
	struct FIX_POSITION_INFO
	{
		int world_tag;
		A3DVECTOR3 pos;
		ACString positionName;
		FIX_POSITION_INFO() : world_tag(-1) {}
	};
	struct SOLO_CHALLENGE_AWARD_INFO
	{
		int layer;
		int time;
		int totalDrawItemTimes;
		int drawnItemTimes;
		typedef S2C::cmd_solo_challenge_award_info_notify::_layer_info::_award_info SOLOTOWERCHALLENGE_AWARD_INFO;
		std::vector<SOLOTOWERCHALLENGE_AWARD_INFO> awards;
		void ResetAwardInfo() {
			layer = 0; time = 0; totalDrawItemTimes = 0; drawnItemTimes = 0; awards.clear();
		}
	};
	struct SOLO_CHALLENGE_INFO
	{
		int maxLayerClimbed;
		int totalClimbingTime;
		int totalScoreEarned;
		int currentScore;
		int skillsStackNum[16];
		SOLO_CHALLENGE_AWARD_INFO awardInfo;
	};
...
public:		//	Operations
	...
	bool GetCameraOnFocus() { return m_bCameraOnFocus; }
	void SetCameraOnFocus(bool value) { m_bCameraOnFocus = value; }
	...
	bool CmdRemoteRepair();
	bool CmdPickupAll();
	bool CmdQuickTeleport();
	...
	A3DVECTOR3 GetVelocity() { return m_vVelocity; }
	...
	//	set host player talking with NPC
	void SetTalkingWithNPC(bool TalkingWithNPC) {  m_bTalkWithNPC = TalkingWithNPC; }
     //	Set host using vip services ?
	void SetVipScreenActive(bool vipscreen) {  m_bVipScreenActive = vipscreen; }
	//	Is host Using vip services ?
	bool IsVipScreenActive() { return m_bVipScreenActive; }
	//	Is spelling magic
	...
	//	Set current service NPC
	void SetCurServiceNPC(int curnpc) { m_idSevNPC = curnpc; }
	...
	//  Get Equipment by id & addoncode
	CECIvtrEquip* GetEquipByID(int tid, int addoncode, int& index, bool secring = false);
	CECIvtrEquip* GetEquipByID(int id, int& index, bool secring = false);	// id: high byte is tid, low byte is addoncode
	...
	//  Get Equipment shortcut sets by index
	int	GetEquipSCSets(int index) { ASSERT(index >= 0 && index < SIZE_FASHIONSCSET); return m_aEquipmentSCSets[index]; }
	void SetEquipSCSets(int index, int value) { ASSERT(index >= 0 && index < SIZE_FASHIONSCSET); m_aEquipmentSCSets[index] = value; }
	void SetEquipSCSets(int index, CECIvtrEquip* pEquip);
	...
	//  Get current AutoEquip suit id
	int GetCurEquipSuitID() { return m_iCurEquipmentSuitID; }
	void SetCurEquipSuitID(int id) { m_iCurEquipmentSuitID = id; }
	...
	//  Equip Equipments by suit id
	bool EquipEquipmentsBySuitID(int id);
	...
	int GetAstralLevel() { return m_iAstralLevel; }
	int GetAstralEnergy() { return m_iAstralExp; }
	...
	int GetVipScore() const { return m_nVipScore; }
	void SetVipInfo(int level, int score) { m_nVipLevel = level; m_nVipScore = score; m_bVipInitialized = true; /*a_LogOutput(1, "SetVipInfo: Level: %d :: Score: %d", level, score);*/	}
	bool IsVipInitialized() { return m_bVipInitialized; }
	void SetCashResurrectInfo(int cash_need, int cash_left) { m_bCanResurrectWithCash = true; m_nResurrectCashNeed = cash_need; m_nResurrectCashLeft = cash_left; a_LogOutput(1, "Cash Resurrect info: m_nResurrectCashNeed: %d :: m_nResurrectCashLeft: %d", m_nResurrectCashNeed, m_nResurrectCashLeft); }
	bool CanResurrectWithCash() { return m_bCanResurrectWithCash;  }
	int GetCashResurrectCashNeeded() { return m_nResurrectCashNeed;  }
	int GetVipLevelForVipService(VipServices vipService)const;
	int GetFixPositionEnergy()const;
	int GetCountFixPositionList()const;
	const FIX_POSITION_INFO * GetFixPositionInfoByIndex(int index);
	int CECHostPlayer::GetCashVipFixPositionNum();

	// Solo Tower
	const SOLO_CHALLENGE_INFO& GetSoloChallengeInfo() { return m_SoloTowerInfo; }
	void AddSoloChallengeAward(int itemId, int itemCount, int pos) { m_SoloTowerInfo.awardInfo.drawnItemTimes++; m_SoloTowerInfo.awardInfo.awards.push_back({ itemId, itemCount });	}
	void SetSoloChallengeCurrentPoints(int points) { m_SoloTowerInfo.currentScore = points; }
	void SetSoloChallengeMaxPoints(int points) { m_SoloTowerInfo.totalScoreEarned = points; }
	void SoloChallengeActivateSkill(int skillIndex, int stack) { m_SoloTowerInfo.skillsStackNum[skillIndex] = stack;  }
...
protected:	//	Attributes
	...
	bool			m_bVipScreenActive;		//	true, is using vip services
	...
	int					m_aEquipmentSCSets[SIZE_FASHIONSCSET];	// Equipment shortcut sets
	int					m_iCurEquipmentSuitID;	//  Current Equipment suit id
	...
	A3DGFXEx*		m_pStarChartGlow[ASTROLABE_ADDON_MAX];
	A3DGFXEx*		m_pMoveTargetGFX;
	A3DGFXEx*		m_pSelectedGFX;
	A3DGFXEx*		m_pHoverGFX;
	A3DGFXEx*		m_pTargetDirGFX;
	...
	bool			m_bCameraOnFocus;		//	Camera Locked At NPC
	...
	int				m_nVipLevel;
	int				m_nVipScore;
	bool			m_bVipInitialized;
	// Quick Teleport info
	FIX_POSITION_INFO m_FixPositionList[FIX_POSITION_TRANSMIT_MAX_POSITION_COUNT];
	int				m_iFixPositionEnergy;
	// Cash resurrect
	bool			m_bCanResurrectWithCash;
	int			m_nResurrectCashNeed;
	int			m_nResurrectCashLeft;
	// SoloTowerChallenge info
	SOLO_CHALLENGE_INFO m_SoloTowerInfo;
...
protected:	//	Operations
	...
	void OnMsgAstrolabeNotify(const ECMSG& Msg);
	void OnMsgAstrolabeOperateResult(const ECMSG& Msg);
	void OnMsgFixPositionTransmitAddPosition(const ECMSG& Msg);
	void OnMsgFixPositionTransmitDeletePosition(const ECMSG& Msg);
	void OnMsgFixPositionTransmitRenamePosition(const ECMSG& Msg);
	void OnMsgFixPositionEnergyInfo(const ECMSG& Msg);
	void OnMsgFixPositionAllInfo(const ECMSG& Msg);
	void OnMsgSoloChallangeAwardInfoNotify(const ECMSG& Msg);
	void OnMsgSoloChallangeOperateResult(const ECMSG& Msg);
	void OnMsgSoloChallangeStateNotify(const ECMSG& Msg);
	void OnMsgSoloChallangeBuffInfoNotify(const ECMSG& Msg);
	void OnMsgArena(const ECMSG& Msg);
