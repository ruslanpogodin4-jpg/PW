...
AUI_ON_COMMAND("showdrop",		OnCommand_showdrop)
...
void CDlgTarget::OnCommand_showdrop(const char *szCommand)
{
	int idSelObj = GetHostPlayer()->GetSelectedTarget();
	if (!ISNPCID(idSelObj)) return;

	CECNPC *pNPC = GetWorld()->GetNPCMan()->GetNPC(idSelObj);
	if (!pNPC) return;

	const CECMonster *pMonster = dynamic_cast<const CECMonster *>(pNPC);
	if (!pMonster) return;

	const MONSTER_ESSENCE *pEssence = pMonster->GetMonsterEssence();
	if (!pEssence) return;

	CDlgWikiMonsterDrop *pDlg = dynamic_cast<CDlgWikiMonsterDrop *>(
		GetGameUIMan()->GetDialog("Win_WikiMonsterDrop"));
	if (!pDlg) return;

	pDlg->SetSearchDataPtr((void *)pEssence);
