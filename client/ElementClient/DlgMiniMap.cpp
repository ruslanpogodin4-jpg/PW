...
AUI_ON_COMMAND("instrestart", OnCommand_instancerestart)
AUI_ON_COMMAND("asm_nodelay", OnCommand_skillsender)
AUI_ON_COMMAND("asm_sschallenge", OnCommand_autocharge)
AUI_ON_COMMAND("Chk_AA", OnCommand_autoattack)
AUI_ON_COMMAND("color_name", OnCommand_color_name)
...
CDlgMiniMap::CDlgMiniMap()
{
	...
	m_pResetInstanceTime = NULL;
	m_pChk_AutoAttack = NULL;
	m_pChk_SkillSender = NULL;
	m_pChk_AutoCharge = NULL;
...
void CDlgMiniMap::OnTick()
{
	if( stricmp(GetName(), "Win_Map") == 0 )
	{
		...
		PAUILABEL pLabel;
		if( binfo.idBattle && !binfo.IsArenaBattle() )
		{
		...
		m_pChk_SkillSender->Check(gs.bSkillSender);
		m_pChk_AutoCharge->Check(gs.bAutoCharge);
		m_pChk_AutoAttack->Check(gs.bAutoAttack);
...
bool CDlgMiniMap::OnInitDialog()
{
	...
	if( stricmp(GetName(), "Win_Map") == 0 )
	{
		...
		m_pChk_SkillSender = (PAUICHECKBOX)GetDlgItem("asm_nodelay");
		m_pChk_AutoCharge = (PAUICHECKBOX)GetDlgItem("asm_sschallenge");
		m_pChk_AutoAttack = (PAUICHECKBOX)GetDlgItem("Btn_AutoAttack");
	...
	PAUIOBJECT pColorBtn = GetDlgItem("color_name");
	//if (pColorBtn)pColorBtn->Show(false);
...
void CDlgMiniMap::OnCommand_instancerestart(const char* szCommand)
{
	GetGameSession()->c2s_CmdReloadInstance();
}

void CDlgMiniMap::OnCommand_skillsender(const char* szCommand)
{
	EC_GAME_SETTING gs = g_pGame->GetConfigs()->GetGameSettings();
	gs.bSkillSender = m_pChk_SkillSender->IsChecked();
	GetGame()->GetConfigs()->SetGameSettings(gs);
}

void CDlgMiniMap::OnCommand_autoattack(const char* szCommand)
{
	EC_GAME_SETTING gs = g_pGame->GetConfigs()->GetGameSettings();
	gs.bAutoAttack = m_pChk_AutoAttack->IsChecked();
	GetGame()->GetConfigs()->SetGameSettings(gs);
}

void CDlgMiniMap::OnCommand_autocharge(const char* szCommand)
{
	EC_GAME_SETTING gs = g_pGame->GetConfigs()->GetGameSettings();
	gs.bAutoCharge = m_pChk_AutoCharge->IsChecked();
	GetGame()->GetConfigs()->SetGameSettings(gs);
}
...
void CDlgMiniMap::OnCommand_color_name(const char* szCommand)
{
	PAUIDIALOG pDlg = GetGameUIMan()->GetDialog("Win_ArenaQueueChoose");
	if (pDlg) pDlg->Show(!pDlg->IsShow());
	//PAUIDIALOG pShow = GetGameUIMan()->m_pDlgColorName;
	//if (pShow)
	//{
	//	pShow->Show(!pShow->IsShow());
	//}
