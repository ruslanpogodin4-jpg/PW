	...
	while (pCandidate)
	{
		...
		if (command(pCandidate))
		{
			...
			if (nAdded < GetPageSize())
			{
				...
				// Always count the slot regardless of OnAddSearch result.
				// If OnAddSearch returns false (ERRORITEM etc.), the row stays
				// hidden but the slot is consumed so the page boundary stays
				// correct and the next page won't repeat the last item.
				nAdded++;
			}
			else
			{
