...
//  Find some matters mine which is near enough to host player
bool CECMatterMan::FindMattersMineInRegion(abase::vector<CECMatter*>& matters)
{
	CECHostPlayer* pHost = m_pGameRun->GetHostPlayer();

	//	Look all matters and find only mines
	MatterTable::iterator it = m_MatterTab.begin();
	for (; it != m_MatterTab.end(); ++it)
	{
		CECMatter* pMatter = *it.value();
...
bool CECMatterMan::Tick(DWORD dwDeltaTime)
{
	...
	CECParallelTickOnlyAnimationManager<CECMatter*> elems;
	//	Tick all matters
	MatterTable::iterator it = m_MatterTab.begin();
	for (; it != m_MatterTab.end(); ++it)
	{
		CECMatter* pMatter = *it.value();
		elems.AddElem(pMatter, dwDeltaTime);
		pMatter->TickBeforeAnimate(dwDeltaTime);
	}
	...
	{
		CECMatter* pMatter = *it.value();
		pMatter->TickAfterAnimate(dwDeltaTime);
	}
...
static const char* szGFXFile = "npc\\宝箱_任务物品.gfx";
//	Render routine
bool CECMatterMan::Render(CECViewport* pViewport)
{
	...
	for (; it != m_MatterTab.end(); ++it)
	{
		...
		if (pMatter->GetModel() != NULL) {
			if (pMatter->IsRequiredForTask()) {
				if (!pMatter->IsRenderingTaskGFX()) {
					pMatter->GetModel()->PlayGfx(szGFXFile, NULL);
					pMatter->SetRenderingTaskGFX(true);
				}
			}
			else {
				if (pMatter->IsRenderingTaskGFX()) {
					pMatter->GetModel()->RemoveGfx(szGFXFile, NULL);
					pMatter->SetRenderingTaskGFX(false);
				}
			}
		}
	}
