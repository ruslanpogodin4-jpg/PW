...
void CDlgDragDrop::OnGeneralScene(PAUIDIALOG pDlgSrc, PAUIOBJECT pObjSrc, PAUIDIALOG pDlgOver, PAUIOBJECT pObjOver, CECIvtrItem* pIvtrSrc)
{
	...
	else HANDLE_DLG("Win_ChangeEquipment", OnChangeEquipDragDrop)
										...
										PAUIDIALOG pDlgOver, PAUIOBJECT pObjOver, CECIvtrItem* pIvtrSrc)
{
	...
	if( strstr(pObjSrc->GetName(), "Item_") )
	{
		...
		else if (strstr(pDlgOver->GetName(), "Win_ChangeEquipment")
			&& pObjOver && strstr(pObjOver->GetName(), "Item_"))
		{
			CDlgChangeEqp* pChangeEquipment = (CDlgChangeEqp*)pDlgOver;
			CECIvtrEquip* pSrcEquip = (CECIvtrEquip*)pIvtrSrc;
			pChangeEquipment->Store(pObjOver, pSrcEquip);
		}
		else if( pObjOver )
		{
...
void CDlgDragDrop::OnChangeEquipDragDrop(PAUIDIALOG pDlgSrc, PAUIOBJECT pObjSrc, PAUIDIALOG pDlgOver, PAUIOBJECT pObjOver, CECIvtrItem* pIvtrSrc)
{
	if (strstr(pObjSrc->GetName(), "Item_"))
	{
		// Drag a fashion short cut
		//
		CDlgChangeEqp* pChangeEqp = (CDlgChangeEqp*)GetGameUIMan()->GetDialog("Win_ChangeEquipment");

		if (pDlgOver == pChangeEqp
			&& pObjOver
			&& strstr(pObjOver->GetName(), "Item_"))
		{
			// Drag within Win_ChangeSkirt
			pChangeEqp->Replace(pObjSrc, pObjOver);
		}
		else if (!pDlgOver)
		{
			// Drop a fashion short cut
			pChangeEqp->Discard(pObjSrc);
		}
	}
	else if (strstr(pObjSrc->GetName(), "Img_No"))
	{
		...
		if (pDlgOver && pObjOver)
		{
				...
				strstr(pObjOver->GetName(), "Item_"))
			{
				// Create quick bar item
				int iSlot = atoi(pObjOver->GetName() + strlen("Item_"));
				int iEquip = (int)pObjSrc->GetDataPtr();
				CECShortcutSet* pSCS = CECGameUIMan::GetSCSByDlg(pDlgOver->GetName());

				if (!pSCS->GetShortcut(iSlot - 1) || !g_pGame->GetConfigs()->GetGameSettings().bLockQuickBar)
					pSCS->CreateAutoEquipShortcut(iSlot - 1, iEquip - 1);
			}
		}
	}
}
