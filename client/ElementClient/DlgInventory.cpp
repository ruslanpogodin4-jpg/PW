...
AUI_ON_COMMAND("Btn_Repair", OnCommand_RemoteRepair)
AUI_ON_COMMAND("Btn_ChangeEqp", OnCommand_ChangeEqp)
AUI_ON_COMMAND("btn_add_socket", OnCommand_RemoteAddSocket)
AUI_ON_COMMAND("btn_new_socket", OnCommand_RemoteNewSocket)
AUI_ON_COMMAND("btn_refine", OnCommand_RemoteRefine)
AUI_ON_COMMAND("btn_clear_socket", OnCommand_RemoteClearSocket)
AUI_ON_COMMAND("btn_patch_socket", OnCommand_RemotePatchSocket)
AUI_ON_COMMAND("btn_change_socket", OnCommand_RemoteChangeSocket)
AUI_ON_COMMAND("btn_bank", OnCommand_RemoteBank)
AUI_ON_COMMAND("btn_mail", OnCommand_RemoteMail)
...
bool CDlgInventory::OnInitDialog()
{
	...
	m_pBtn_ChangeEqp = (PAUISTILLIMAGEBUTTON)GetDlgItem("Btn_ChangeEqp");
...
void CDlgInventory::OnEventLButtonDown(WPARAM wParam, LPARAM lParam, AUIObject *pObj)
{
	...
	if(strstr(pObj->GetName(), "Item_"))
	{
		...
		if (pAstrolabe &&
			pAstrolabe->IsSwallowing() &&
			pAstrolabe->SwallowItem(iDst, pStored))
			return;
	}
...
void CDlgInventory::OnCommand_Astrolabe(const char* szCommand) {
	int iItem = GetHostPlayer()->GetEquippedItem(EQUIPIVTR_ASTROLABE);
	if (iItem)
	{
		CECGameUIMan* pUIMan = GetGameUIMan();
		pUIMan->m_pDlgAstrolabe->Show(true);
	}
}

void CDlgInventory::OnCommand_ChangeEqp(const char* szCommand) {
	PAUIDIALOG pDlg = GetGameUIMan()->GetDialog("Win_ChangeEquipment");
	if (pDlg) pDlg->Show(!pDlg->IsShow());
}


void CDlgInventory::OnCommand_RemoteRepair(const char* szCommand) {
	CECHostPlayer* pHost = GetHostPlayer();
	// Check if user has enough VIP level to use this feature
	int requiredVipLevel = pHost->GetVipLevelForVipService(CECHostPlayer::VipServices::CVS_REPAIR);
	if (pHost->GetVipLevel() < requiredVipLevel) {
		ACString msgStr;
		msgStr.Format(GetStringFromTable(11730), requiredVipLevel, GetStringFromTable(11720));
		GetGameUIMan()->AddChatMessage(msgStr, GP_CHAT_MISC);
		return;
	}
	CECInventory* pPackEquip = pHost->GetPack(IVTRTYPE_EQUIPPACK);

	// Check if user has equipment to repair
	if (pPackEquip->HasDisrepairItem())
	{
		// Calculate repair cost based on VIP level and equipement

		int regularRepairCost = pPackEquip->GetRepairAllItemCost();

		// Cost ratio by VIP level
		static float costRatio[MAX_VIP_LEVEL + 1] = { -1.f, 3.f, 2.8f, 2.6f, 2.4f, 2.2f, 2.0f };

		int tmpPlayerVipLevel = pHost->GetVipLevel();
		if (tmpPlayerVipLevel > 0) {
			if (tmpPlayerVipLevel > MAX_VIP_LEVEL) tmpPlayerVipLevel = MAX_VIP_LEVEL;
		}
		int remoteRepairCost = regularRepairCost * costRatio[tmpPlayerVipLevel];

		// Ask player if he is willing to repair
		ACString strText;
		strText.Format(m_pAUIManager->GetStringFromTable(242), max(remoteRepairCost, 1));
		m_pAUIManager->MessageBox("Game_RemoteRepair", strText, MB_OKCANCEL, A3DCOLORRGBA(255, 255, 255, 160));
	}
	else
	{
		m_pAUIManager->MessageBox("", m_pAUIManager->GetStringFromTable(252), MB_OK, A3DCOLORRGBA(255, 255, 255, 160));
	}
}

void CDlgInventory::OnCommand_moneytonote(const char* szCommand) {
	GetGameSession()->c2s_CmdExchangeYinpiao(false, 1);
}
void CDlgInventory::OnCommand_notetomoney(const char* szCommand) {
	GetGameSession()->c2s_CmdExchangeYinpiao(true, 1);
}

bool CDlgInventory::AcquireRemoteNPC(int npcid)
{
	CECHostPlayer* pHost = GetHostPlayer();
	/*int requiredVipLevel = pHost->GetVipLevelForVipService(CECHostPlayer::VipServices::CVS_REPAIR);
	...
	if (pHost->GetVipLevel() < requiredVipLevel) {
		ACString msgStr;
		msgStr.Format(GetStringFromTable(11730), requiredVipLevel,L"Remote Services");
		GetGameUIMan()->AddChatMessage(msgStr, GP_CHAT_MISC);
		return false;
	}*/

	g_pGame->GetGameSession()->c2s_CmdNPCSevHello(npcid);

	if (GetHostPlayer()->IsAutoMoving() || GetHostPlayer()->IsPlayerMoving()) {
		GetGameUIMan()->MessageBox("", GetStringFromTable(20010), MB_OK, A3DCOLORRGBA(255, 255, 255, 160));
		return false;
	}
...
void CDlgInventory::OnCommand_RemoteAddSocket(const char* szCommand)
{
	if (AcquireRemoteNPC(REMOTE_NPCSERVICE)) {
		GetGameUIMan()->m_pDlgEquipSlot->Show(true);
	}
}

void CDlgInventory::OnCommand_RemoteNewSocket(const char* szCommand)
{
	if (AcquireRemoteNPC(REMOTE_NPCSERVICE)) {
...
void CDlgInventory::OnCommand_RemoteRefine(const char* szCommand)
{
	if (AcquireRemoteNPC(REMOTE_NPCSERVICE)) {
...
void CDlgInventory::OnCommand_RemoteClearSocket(const char* szCommand)
{
	if (AcquireRemoteNPC(REMOTE_NPCSERVICE)) {
		GetGameUIMan()->m_pDlgUninstall->Show(true);
	}
}

void CDlgInventory::OnCommand_RemotePatchSocket(const char* szCommand)
{
	if (AcquireRemoteNPC(REMOTE_NPCSERVICE)) {
...
void CDlgInventory::OnCommand_RemoteChangeSocket(const char* szCommand)
{
	if (AcquireRemoteNPC(REMOTE_NPCSERVICE)) {
...
void CDlgInventory::OnCommand_RemoteBank(const char* szCommand)
{
	
	if (AcquireRemoteNPC(REMOTE_NPCSERVICE)) {
		PAUIDIALOG pShow1 = NULL;
...
void CDlgInventory::OnCommand_RemoteMail(const char* szCommand)
{
	if (AcquireRemoteNPC(REMOTE_NPCSERVICE)) {
