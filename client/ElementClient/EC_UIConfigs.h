...
//	configs/vipconfig.ini
class CECVipConfig
{
private:
	CECVipConfig();
	CECVipConfig(const CECVipConfig&);
	CECVipConfig& operator == (const CECVipConfig&);

	//	ScoreNeeded
	typedef abase::hash_map<int, int> VipScoreNeededMap;
	VipScoreNeededMap	m_VipScoreNeeded;

	//	Score Reduced per day
	typedef abase::hash_map<int, int> VipScoreReduceMap;
	VipScoreReduceMap	m_VipScoreReducedPerDay;

public:

	bool	Load(const char* filePath);

	static CECVipConfig& Instance();
