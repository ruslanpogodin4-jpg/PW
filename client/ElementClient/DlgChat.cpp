...
void CDlgChat::RebuildChatContents(PAUITEXTAREA pText, int nChannelSet)
{
	...
	for( i = 0; i < (int)m_vecChatMsg.size(); i++ )
	{
		...
		int nIconIndex = 0;
		GetGameUIMan()->TransformNameColor(msg.pItem, strName, clrName, &pIconSprite, &nIconIndex);
		pText->AppendText(msg.strMsg, msg.nMsgIndex, strName, clrName,
			NULL, 0, UM_USE_AUIOBJECT_OPTION, 0xff000000,
			msg.pItem ? &(AUI_UNMARSH_IVTRITEM_ICON_INFO{pIconSprite, nIconIndex}) : NULL);
				...
				if( pPack )
				{
					if (msg.nSlot >= 0)
					{
						...
						if( pItem )
						{
							ACString strName;
							A3DCOLOR clrName;
							GetGameUIMan()->TransformNameColor(pItem, strName, clrName, NULL, NULL);
...
void CDlgChat::UpdateSuperFarCryMsg(bool bForceUpdateContent)
{
	...
	if (m_curSuperFarCryMsgValid && m_curSuperFarCryShowMode == true)
	{
		...
		if (bForceUpdateContent || m_curSuperFarCryMsg.strMsg != strTextOld)
		{
			...
			A2DSprite *pIconSprite = NULL;
			int nIconIndex = 0;
			GetGameUIMan()->TransformNameColor(m_curSuperFarCryMsg.pItem, strName, clrName, &pIconSprite, &nIconIndex);
			pText->SetText(_AL(""));
			pText->AppendText(m_curSuperFarCryMsg.strMsg, m_curSuperFarCryMsg.nMsgIndex, strName, clrName,
				NULL, 0, UM_USE_AUIOBJECT_OPTION, 0xff000000,
				m_curSuperFarCryMsg.pItem ? &(AUI_UNMARSH_IVTRITEM_ICON_INFO{pIconSprite, nIconIndex}) : NULL);
		}
