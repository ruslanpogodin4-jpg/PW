...
void CDlgQShopBuy::SetBuyingItem(int itemindex, int buyindex, CECShopBase *pShop)
{
	...
	while(true) {
		...
		CDlgQShop* pDlgShop = GetGameUIMan()->m_pDlgQShop;
		if (pDlgShop->IsCashVipShopItem(m_pShop->GetItem(itemindex))) { // Items sold for Vip Points 
			strTemp.Format(L"%d %s", nPrice, GetStringFromTable(11741));
			...
			// Just give the regular price label
...
int CDlgQShopBuy::GetMaximumNumCanBuy()
{
	...
	int player_cash = 0;
	CDlgQShop* pDlgShop = GetGameUIMan()->m_pDlgQShop;
	if (pDlgShop->IsCashVipShopItem(m_pShop->GetItem(m_ItemIndex))) { // Items sold for Vip Points 
		player_cash = pDlgShop->GetVipScore();
	}
	else {
		player_cash = m_pShop->GetCash();
	}
...
#ifdef VIP
	// Check for Period limit, and correct MaxItemCanBuy based on that too
	const GSHOP_ITEM& gshop_item = *(m_pShop->GetItem(m_ItemIndex));
	if (gshop_item.buy_times_limit_mode > CECShopBase::CASH_VIP_SHOPPING_LIMIT_NONE) {
		// Get amount of purchases so far
		int amountOfPurchases = m_pShop->GetItemPurchasesAmount(gshop_item.buy_times_limit_mode, gshop_item.id);
		int remainingPurchases = gshop_item.buy_times_limit - amountOfPurchases;
		if (m_MaxItemCanBuy > remainingPurchases) m_MaxItemCanBuy = remainingPurchases;
	}
#endif
	return m_MaxItemCanBuy;
}
...
void CDlgQShopBuy::SetTotalMoneyNeededText()
{
	...
	if (pObj) {
		...
		if (pDlgShop->IsCashVipShopItem(m_pShop->GetItem(m_ItemIndex))) { // Items sold for Vip Points 
			ACString strTemp;
			strTemp.Format(L"%d %s", CalculateMoneyNeeded(), GetStringFromTable(11741));
			pObj->SetText(strTemp);
		}
		else {
