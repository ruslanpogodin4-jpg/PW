...
void CECElsePlayer::SetPlayerInfo(const S2C::info_player_1& Info)
{
	...
	if (Info.state2 & GP_STATE2_BODY_SIZE)
	{
		ScaleBody(*(float*)pData);
		pData += sizeof(float);
	}
...
//	Tick Before routine
bool CECElsePlayer::TickBeforeAnimate(DWORD dwDeltaTime)
{
	PROFILE_FUNCTION();
	CECPlayer::Tick(dwDeltaTime);

	if (m_pSprite )
		m_pSprite->TickBeforeAnimate(dwDeltaTime);
	if (m_pGoblin )
		m_pGoblin->TickBeforeAnimate(dwDeltaTime);
	...
	if (m_iBoothState != 2)
	{
		...
		if (m_AttachMode != enumAttachNone && m_bHangerOn)
		{
			if ((!pBuddy || !pBuddy->GetPlayerModel()) && m_pPlayerModel)
			{
				...
				m_pPlayerModel->TickBeforeAnimate(dwDeltaTime);				
				m_pTickedModel = m_pPlayerModel;
			}
		}
		else if (m_pPetModel)
		{
			m_pPetModel->SetSlowestUpdateFlag(!m_bVisible || m_bUseHintModel);
			m_pPetModel->TickBeforeAnimate(dwDeltaTime);
			m_pTickedModel = m_pPetModel;
		}
		...
		// Moved this tick to tick phase, not in render!
		...
		if (pSimpleModel) {
			...
			pSimpleModel->TickBeforeAnimate(g_pGame->GetTickTime());
			m_pTickedModel = pSimpleModel;
		}
	}	
	return true;
}
bool CECElsePlayer::TickOnlyAnimation(DWORD dwDeltaTime)
{
	if (m_pTickedModel)
	{
		m_pTickedModel->TickAnimation(dwDeltaTime);
	}
	if (m_pSprite)
	{
		m_pSprite->TickAnimation(dwDeltaTime);
	}
	if (m_pGoblin)
	{
		m_pGoblin->TickAnimation(dwDeltaTime);
	}
	return true;
}
bool CECElsePlayer::TickAfterAnimate(DWORD dwDeltaTime)
{
	PROFILE_FUNCTION();
	if (m_pSprite)
		m_pSprite->TickAfterAnimate(dwDeltaTime);
	if (m_pGoblin)
		m_pGoblin->TickAfterAnimate(dwDeltaTime);
	if (m_pTickedModel)
	{
		m_pTickedModel->TickAfterAnimate(dwDeltaTime, false, true, false);
		m_pTickedModel->UpdateModelAABB();
	}
	CECHostPlayer* pHost = m_pPlayerMan->GetHostPlayer();
	bool bSelected = pHost->GetSelectedTarget() == GetPlayerInfo().cid;
									...
									int iRenderFlag, bool bHighLight)
{
	...
	if (pSimpleModel)
	{
		...
		// Moved to Tick phase, should not be in render
		/*
		if (pSimpleModel->GetComActCount()>0)
		{
			const char *szAct = pSimpleModel->GetComActByIndex(0)->GetName();
...
void CECElsePlayer::Killed(int idKiller)
{
	...
	CECHostPlayer* pHost = m_pPlayerMan->GetHostPlayer();
		if(pHost->GetPlayerInfo().cid == idKiller) {
				CECGameRun* pGameRun = g_pGame->GetGameRun();
				pGameRun->AddFixedMessage(FIXMSG_KILLEDPLAYER, GetName());
		}
...
int CECElsePlayer::GetEquip64(int n)
{
	return m_aNewEquips[EQUIPIVTR_ASTROLABE];
}