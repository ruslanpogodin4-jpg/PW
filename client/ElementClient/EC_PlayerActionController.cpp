...
public:
	...
	virtual bool PlaySkillCastActionWithName(int idSkill, const char *szActName, bool bNoFX, bool* pActFlag/* =NULL */, DWORD dwFlagMode/* =COMACT_FLAG_MODE_NONE */){
		if (IsMoving()){
			return false;
		}
...
public:
	...
	virtual bool PlaySkillCastActionWithName(int idSkill, const char *szActName, bool bNoFX, bool* pActFlag, DWORD dwFlagMode){
		if (IsMoving()){
...
#endif
		return GetModel()->PlayActionByName(CECPlayerActionController::ACT_CHANNEL_UPPERBODY, szActName, 1.0f, true, 200, true, MakeCastSkillActionUserData(idSkill), bNoFX, NULL, dwFlagMode)
			&& GetModel()->PlayActionByName(CECPlayerActionController::ACT_CHANNEL_LOWERBODY, szActName, 1.0f, true, 200, true, MakeCastSkillActionUserData(idSkill), true, pActFlag, dwFlagMode);
	}
	virtual bool PlaySkillAttackActionWithName(int idSkill, const char *szActName, bool bNoFX, bool *pActFlag, DWORD dwFlagMode){
		if (IsMoving()){
			if (!CanCombineWithMoveForSkill(idSkill)){	//	动作不支持，则直接忽略技能动作播放
...
bool CECPlayerActionController::PlaySkillCastActionWithName(int idSkill, const char *szActName, bool bNoFX/* =false */, bool* pActFlag/* =NULL */, DWORD dwFlagMode/* =COMACT_FLAG_MODE_NONE */){
	if (m_actionPlayPolicy &&
		m_actionPlayPolicy->PlaySkillCastActionWithName(idSkill, szActName, bNoFX, pActFlag, dwFlagMode)){
		m_bSkillAttackActionPlayed = false;
		return true;
	}
	return false;
