...
void CDlgInfo::OnEventLButtonUp_List(WPARAM wParam, LPARAM lParam, AUIObject *pObj)
{
	...
	else if (0 == stricmp(InfoThis.strType, "Game_SoloTowerChallengeReward"))
	{
...
void CDlgInfo::RemoveInfo(int nType, INFORMATION Info)
{
	...
	if (INFO_QUESTIONTASK == nType || INFO_QUESTION == nType || INFO_SOLOCHALLENGEREWARD == nType)
	{
		m_vecInfo[nType].clear();
...
void CDlgInfo::OnTick()
{
	...
	for( i = 0; i < INFO_NUM; i++ )
	{
		...
		{
			if (i == INFO_SOLOCHALLENGEREWARD)
			{
				if (GetGameUIMan()->m_pDlgSoloTowerChallengeAward->IsShow() || !GetGameUIMan()->m_pDlgSoloTowerChallengeState->IsShow()) {
					m_vecInfo[i][j].dwLife = 1; // Force to dissapear.
				}
			}
