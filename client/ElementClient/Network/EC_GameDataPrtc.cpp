...
{\
	...
	case ASTROLABE_INFO_NOTIFY: dwSize = sizeof(cmd_astrolabe_info_notify); break;
	case ASTROLABE_OPERATE_RESULT: dwSize = sizeof(cmd_astrolabe_operate_result); break;
	case SOLO_CHALLENGE_AWARD_INFO_NOTIFY:
		CHECK_VALID(cmd_solo_challenge_award_info_notify);
		break;
	case SOLO_CHALLENGE_OPERATE_RESULT: dwSize = sizeof(cmd_solo_challenge_operate_result); break;
	case SOLO_CHALLENGE_CHALLENGING_STATE_NOTIFY: dwSize = sizeof(cmd_solo_challenge_challenging_state_notify); break;
	case SOLO_CHALLENGE_BUFF_INFO_NOTIFY:
		CHECK_VALID(cmd_solo_challenge_buff_info_notify);
		break;
	case FIX_POSITION_TRANSMIT_ADD_POSITION: dwSize = sizeof(cmd_fix_position_transmit_add_position); break;
	case FIX_POSITION_TRANSMIT_DELETE_POSITION: dwSize = sizeof(cmd_fix_position_transmit_delete_position); break;
	case FIX_POSITION_TRANSMIT_RENAME: dwSize = sizeof(cmd_fix_position_transmit_rename); break;
	case FIX_POSITION_ENERGY_INFO: dwSize = sizeof(cmd_fix_position_energy_info); break;
	case FIX_POSITION_ALL_INFO:
		CHECK_VALID(cmd_fix_position_all_info);
		break;
	case CASH_VIP_MALL_ITEM_PRICE: 
		CHECK_VALID(cmd_cash_vip_mall_item_price);
		break;
	case CASH_VIP_MALL_ITEM_BUY_RESULT: dwSize = sizeof(cmd_cash_vip_mall_item_buy_result); break;
	case CASH_VIP_INFO_NOTIFY: dwSize = sizeof(cmd_cash_vip_info_notify); break;
	case PURCHASE_LIMIT_INFO_NOTIFY: 
		CHECK_VALID(cmd_purchase_limit_info_notify)
		break;
	case LOOKUP_ENEMY_RESULT: break; 
	case CASH_RESURRECT_INFO: dwSize = sizeof(cmd_cash_resurrect_info); break;
	case BODY_SCALE_NOTIFY: dwSize = sizeof(cmd_body_scale_notify); break;
	case ARENA_QUEUE_NOTIFICATION: dwSize = sizeof(cmd_arena_queue_notification); break;
	case ARENA_QUEUE_CONFIRM_BATTLE: dwSize = sizeof(cmd_arena_queue_confirm_battle); break;
	case ARENA_QUEUE_CANCEL: dwSize = sizeof(cmd_arena_queue_cancel); break;

	case ENTER_ARENA_BATTLEGROUND: dwSize = sizeof(cmd_enter_arena_battleground); break;
	case ARENA_SCORE_UPDATE: dwSize = dwDataSize; break;
	case ARENA_END_RESULT: dwSize = dwDataSize; break;
...
/*
	...
	*/
	...
	case SCENE_SERVICE_NPC_LIST:
		{
			...
			pGameRun->PostMessage(MSG_SCENE_SERVICE_NPC_LIST, -1, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
			break;
		}
	...
	case ASTROLABE_INFO_NOTIFY:
		pGameRun->PostMessage(MSG_HST_ASTROLABE_INFO_NOTIFY, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
	case ASTROLABE_OPERATE_RESULT:
		pGameRun->PostMessage(MSG_HST_ASTROLABE_OPERATE_RESULT, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
	case SOLO_CHALLENGE_AWARD_INFO_NOTIFY:
		pGameRun->PostMessage(MSG_HST_SOLOCHALLENGE_AWARD_INFO, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
	case SOLO_CHALLENGE_OPERATE_RESULT:
		pGameRun->PostMessage(MSG_HST_SOLOCHALLENGE_OPERATE_RESULT, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
	case SOLO_CHALLENGE_CHALLENGING_STATE_NOTIFY:
		pGameRun->PostMessage(MSG_HST_SOLOCHALLENGE_STATE_INFO, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
	case SOLO_CHALLENGE_BUFF_INFO_NOTIFY:
		pGameRun->PostMessage(MSG_HST_SOLOCHALLENGE_BUFF_INFO, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
	case FIX_POSITION_TRANSMIT_ADD_POSITION:
		pGameRun->PostMessage(MSG_HST_FIX_POSITION_TRANSMIT_ADD_POSITION, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
	case FIX_POSITION_TRANSMIT_DELETE_POSITION:
		pGameRun->PostMessage(MSG_HST_FIX_POSITION_TRANSMIT_DELETE_POSITION, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
	case FIX_POSITION_TRANSMIT_RENAME:
		pGameRun->PostMessage(MSG_HST_FIX_POSITION_TRANSMIT_RENAME, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
	case FIX_POSITION_ENERGY_INFO:
		pGameRun->PostMessage(MSG_HST_FIX_POSITION_ENERGY_INFO, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
	case FIX_POSITION_ALL_INFO:
		pGameRun->PostMessage(MSG_HST_FIX_POSITION_ALL_INFO, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
	case CASH_VIP_MALL_ITEM_PRICE:
	case CASH_VIP_MALL_ITEM_BUY_RESULT:
		pGameRun->PostMessage(MSG_CASHVIPMALLITEMINFO, -1, 0, (DWORD)pDataBuf, pCmdHeader->cmd);
		break;
		break;
	case CASH_VIP_INFO_NOTIFY:
		{
			cmd_cash_vip_info_notify* vipInfo = (cmd_cash_vip_info_notify*)pDataBuf;
			CECHostPlayer* pHost = pGameRun->GetHostPlayer();
			if (pHost)
			{
				if (pHost->IsVipInitialized())
				{
					int delta = vipInfo->score - pHost->GetVipScore();
					if (delta < 0) {
						CECUIHelper::QShopBatchBuy(); // Refresh BatchBuy window in case its purchasing multiple items.
					}
				}
				pHost->SetVipInfo(vipInfo->level, vipInfo->score);
			}
		}
		break;
	case PURCHASE_LIMIT_INFO_NOTIFY:
		pGameRun->PostMessage(MSG_PURCHASELIMITINFO, -1, 0, (DWORD)pDataBuf, pCmdHeader->cmd);
		break;
	case LOOKUP_ENEMY_RESULT:
		break;
	case CASH_RESURRECT_INFO:
		{
			cmd_cash_resurrect_info* resurrectInfo = (cmd_cash_resurrect_info*)pDataBuf;
			CECHostPlayer* pHost = pGameRun->GetHostPlayer();

			if (pHost)
			{
				pHost->SetCashResurrectInfo(resurrectInfo->cash_need, resurrectInfo->cash_left);
			}
		}
		break;
	case BODY_SCALE_NOTIFY:
		{
			cmd_body_scale_notify* bodyScaleNotify = (cmd_body_scale_notify*)pDataBuf;
			CECHostPlayer* pHost = pGameRun->GetHostPlayer();
			
			CECWorld* pWorld = g_pGame->GetGameRun()->GetWorld();
			if (pWorld) {
				if (ISPLAYERID(bodyScaleNotify->id)) {
					CECPlayerMan* pPlayerMan = pWorld->GetPlayerMan();
					CECPlayer* pPlayer = pPlayerMan->GetPlayer(bodyScaleNotify->id);
					if ( pPlayer )
						pPlayer->ScaleBody(bodyScaleNotify->scale);
				}
			}
		}

	case ARENA_QUEUE_NOTIFICATION:
		pGameRun->PostMessage(MSG_HST_ARENA_QUEUE_NOTIFICATION, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;

	case ARENA_QUEUE_CONFIRM_BATTLE:
		pGameRun->PostMessage(MSG_HST_ARENA_QUEUE_CONFIRMATION, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;

	case ENTER_ARENA_BATTLEGROUND:
		pGameRun->PostMessage(MSG_HST_ENTER_ARENA_BATTLEGROUND, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;

	case ARENA_SCORE_UPDATE:
		pGameRun->PostMessage(MSG_HST_ARENA_SCORE_UPDATE, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;

	case ARENA_END_RESULT:
		pGameRun->PostMessage(MSG_HST_ARENA_END_RESULT, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
	case ARENA_QUEUE_CANCEL:
		pGameRun->PostMessage(MSG_HST_ARENA_CANCEL, MAN_PLAYER, 0, (DWORD)pDataBuf, pCmdHeader->cmd, dwDataSize);
		break;
