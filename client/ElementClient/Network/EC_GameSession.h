...
public:		//	Operations
	...
	// Keep player online after logout
	void SetLockedLogoutState();
	...
	void c2s_CmdRemoteRepair(){ ::c2s_SendCmdRemoteRepair(); }
	void c2s_CmdPickupAll(int iCount, const void* pMatterList){ ::c2s_SendCmdPickupAll(iCount, pMatterList); }
	void c2s_CmdFixPositionTransmitOperateRequestAdd(A3DVECTOR3 pos, ACString szName) { ::c2s_SendCmdFixPositionTransmitOperateRequestAdd(pos, szName); }
	void c2s_CmdFixPositionTransmitOperateRequestDelete(int index) { ::c2s_SendCmdFixPositionTransmitOperateRequestDelete(index); }
	void c2s_CmdFixPositionTransmitOperateRequestTeleport(int index) { ::c2s_SendCmdFixPositionTransmitOperateRequestTeleport(index); }
	void c2s_CmdFixPositionTransmitOperateRequestRename(int index, ACString szName) { ::c2s_SendCmdFixPositionTransmitOperateRequestRename(index, szName); }
	void c2s_CmdCashResurrect(int param = 0) { m_CmdCache.SendCmdReviveCash(param); }
	void c2s_CmdSoloChallengeOperateRequest(int optType, const void* data) { ::c2s_SendCmdSoloChallengeOperateRequest(optType, data); }
	void c2s_CmdNPCSevSoloChallengeSelectStage(int stage) { ::c2s_SendCmdNPCSevSoloChallengeSelectStage(stage); }
	...
	// Accessory Sockets
	void c2s_SendCmdNPCSevMakeAccessorySlot(int src_index, int src_id, unsigned int material_id, int material_count) { ::c2s_SendCmdNPCSevMakeAccessorySlot(src_index, src_id, material_id, material_count); }
	// 卡牌转生
	...
	//Astrolabe Commands
	void c2s_CmdAstrolabeShuffleAtt(int inv_index, int itemid);
	void c2s_CmdAstrolabeIncPoint(int inv_index, int itemid);
	void c2s_CmdAstrolabeRandomAddon(int count, int limit, int inv_index, int itemid);
	void c2s_CmdAstrolabeSwallow(int type, int inv_index, int itemid);

	// NEW
	void c2s_CmdReloadInstance() { ::c2s_SendCmdReloadInstance(); }

	void c2s_SendCmdArenaEnter(int battle_mode) { ::c2s_SendCmdArenaEnter(battle_mode); };
	void c2s_SendCmdArenaAccept(bool accepted) { ::c2s_SendCmdArenaAccept(accepted); };
	void c2s_SendCmdArenaExit() { ::c2s_SendCmdArenaExit(); };
	...
	bool GetErrorMessage(int iRetCode, AWString& errMsg);
	...
	// Vip system
	void c2s_CmdGetCashVipMallItemPrice(int start_index, int end_index) { ::c2s_SendCmdGetCashVipMallItemPrice(start_index, end_index); }
	void c2s_SendCmdCashVipMallShopping(int iCount, const void* pGoods) { ::c2s_SendCmdCashVipMallShopping(iCount, pGoods); }
	void c2s_CmdNPCSevCashVipMallShopping(int iCount, const void* pGoods) { ::c2s_SendCmdNPCSevCashVipMallShopping(iCount, pGoods); }
