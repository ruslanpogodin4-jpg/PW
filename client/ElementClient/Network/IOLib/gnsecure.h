...
enum { RANDOM = 0, NULLSECURITY = 1, ARCFOURSECURITY = 2, MD5HASH = 3, HMAC_MD5HASH = 4, 
	COMPRESSARCFOURSECURITY = 5, DECOMPRESSARCFOURSECURITY = 6, SHA256HASH = 7, BITSHIFTSECURITY = 8, XORSECURITY = 9
};
...
class BitShiftSecurity : public Security
{
	static int fd;
	Security* Clone() const { return new BitShiftSecurity(*this); }
	static int Init() { srand((unsigned int)time(NULL)); return 0; }
public:
	BitShiftSecurity() { }
	BitShiftSecurity(Type type) : Security(type) { }
	BitShiftSecurity(const BitShiftSecurity& rhs) : Security(rhs) { }
	Octets& Update(Octets& o) {

		unsigned char* bytePtr = (unsigned char*)o.begin();

		for (size_t i = 0; i < o.size(); i++)
		{
			if (bytePtr[i] != 0 && bytePtr[i] != 0x5A) {
				bytePtr[i] = bytePtr[i] ^ 0x5A;
			}
		}
...
class XorSecurity : public Security
{
	Security* Clone() const { return new XorSecurity(*this); }

public:
	XorSecurity() {}
	XorSecurity(Type type) : Security(type) {}
	XorSecurity(const XorSecurity& rhs) : Security(rhs) {}
	Octets& Update(Octets& o)
	{

		unsigned char key[8] = { 'B', 'O', 'R', 'E', 'A', 'L', 'I','S' };
		unsigned char* bytePtr = (unsigned char*)o.begin();

		for (size_t i = 0; i < o.size(); i++)
		{
			unsigned char xortarget = key[i % (sizeof(key) / sizeof(char))];
			if (bytePtr[i] != 0 && bytePtr[i] != xortarget)
				bytePtr[i] = bytePtr[i] ^ xortarget;
		}
