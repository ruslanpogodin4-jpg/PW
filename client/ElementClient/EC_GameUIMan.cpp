	...
	char cChannel, int idPlayer, const ACHAR *pszPlayer, char byFlag, char cEmotion, CECIvtrItem *pItem,const ACHAR *pszMsgOrigion)
{
	...
	ACString strName;
	A3DCOLOR clrName;
	A2DSprite *pIconSprite = NULL;
	int nIconIndex = 0;
	TransformNameColor(pItem, strName, clrName, &pIconSprite, &nIconIndex);	
	...
	msgWithColor += GetChatChannelImage(cChannel);
	{
		...
		GetLocalTime(&st);
		wchar_t szTime[32];
		swprintf_s(szTime, _countof(szTime), L"^C8C8C8[%02d:%02d:%02d] ", st.wHour, st.wMinute, st.wSecond);
		msgWithColor += szTime;
		// Restore channel color after timestamp
		if( ISNPCID(idPlayer) )
			msgWithColor += _AL("^C8FF64");
		else if( cChannel == GP_CHAT_COUNTRY && bIsKing )
			msgWithColor += CDlgChat::m_pszKingColor;
		else
			msgWithColor += CDlgChat::GetChatColor(cChannel, idPlayer);
	}
	...
	{
		// Convert color tags ^rrggbb (lowercase) to ^RRGGBB (uppercase) for the chat parser
		AWString strMsgFixed = msg.strMsg;
		int nLen = strMsgFixed.GetLength();
		WCHAR* pBuf = strMsgFixed.GetBuffer(nLen + 1);
		for (int _i = 0; _i < nLen - 6; _i++)
		{
			if (pBuf[_i] == L'^')
			{
				for (int _j = _i + 1; _j <= _i + 6; _j++)
				{
					if (pBuf[_j] >= L'a' && pBuf[_j] <= L'f')
						pBuf[_j] -= 32;
				}
			}
		}
		strMsgFixed.ReleaseBuffer();
		msgWithColor += strMsgFixed;
	}
		...
		(int)m_pDlgChatWhisper->GetData() == idPlayer )
	{
		...
		pShow1->AppendText(msgWithColor, pItem ? CDlgChat::m_nMsgIndex : 0, strName, clrName,
			NULL, 0, UM_USE_AUIOBJECT_OPTION, 0xff000000, pItem ? &(AUI_UNMARSH_IVTRITEM_ICON_INFO{pIconSprite, nIconIndex}) : NULL);
		...
		if( gs.bChannel[nActiveChannelSet][cChannel] )
		{
			...
			*/
			...
			pShow1->AppendText(msgWithColor, msg.nMsgIndex, strName, clrName,
				NULL, 0, UM_USE_AUIOBJECT_OPTION, 0xff000000, msg.pItem ? &(AUI_UNMARSH_IVTRITEM_ICON_INFO{pIconSprite, nIconIndex}) : NULL);
...
void CECGameUIMan::TransformNameColor(CECIvtrItem *pItem, ACString &strName, A3DCOLOR &clrName,
	A2DSprite **ppIconSprite, int *pIconIndex)
{
	if (pItem)
	{
		...
		if (ppIconSprite && pIconIndex)
		{
			AString strFile;
			af_GetFileTitle(pItem->GetIconFile(), strFile);
			*pIconIndex  = GetIconIndex(ICONS_INVENTORY, strFile);
			*ppIconSprite = m_pA2DSpriteIcons[ICONS_INVENTORY];
		}
	}
	else
	{
		strName.Empty();
		clrName = 0xffffffff;
		if (ppIconSprite) *ppIconSprite = NULL;
		if (pIconIndex)   *pIconIndex   = 0;
	}
}
