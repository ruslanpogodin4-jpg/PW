...
void CDlgInputName::OnCommand_confirm(const char * szCommand)
{
	...
	case INPUTNAME_RENAME_WORLD_MARK:
		...
		break;
	case INPUTNAME_RENAME_MARKIP: // Quick teleport : rename
	case INPUTNAME_ADD_MARKIP: // Quick teleport : add
		// Prevent names with more than 16 characters
		if (strName.GetLength() > 15) {
			GetGameUIMan()->MessageBox("", GetStringFromTable(11719), MB_OK, A3DCOLORRGBA(255, 255, 255, 160)); // Within 16 characters
			return;
		}
		// Return string for processing on Quick Teleport system
		GetGameUIMan()->m_pDlgMarkVip->OnNameInput(GetData(), strName);
		break;
	}
