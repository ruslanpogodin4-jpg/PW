...
//	Tick routinue
bool CECSprite::TickBeforeAnimate(DWORD dwDeltaTime)
{
	...
	m_pModelSprite->TickBeforeAnimate(dwDeltaTime);
	return true;
}
bool CECSprite::TickAnimation(DWORD dwDeltaTime)
{
	if ( m_pModelSprite )
		m_pModelSprite->TickAnimation(dwDeltaTime);
	return true;
}
bool CECSprite::TickAfterAnimate(DWORD dwDeltaTime)
{
	if (!m_pModelSprite)
		return true;
	m_pModelSprite->TickAfterAnimate(dwDeltaTime, false, true);
	m_pModelSprite->UpdateModelAABB();
	return true;
}
//	Tick routinue
bool CECSprite::Tick(DWORD dwDeltaTime, bool bSkipAnimation)
{
	A3DVECTOR3 vecTargetPos;
	if( !m_pPlayer || !m_pPlayer->GetPlayerModel() || !m_pModelSprite )
		return true;
