...
void CDlgSkillSubList::AddSkillSubDlg(int skillID) {
	...
	if ((size_t)m_skillSubCount >= m_skillSubDialogs.size()) {
		...
		// adjust scale
		A3DRECT oldRec = pDlg->GetRect();
		A3DRECT newRec = oldRec;
		newRec.left *= m_pAUIManager->GetWindowScale();
		newRec.top *= m_pAUIManager->GetWindowScale();
		newRec.bottom *= m_pAUIManager->GetWindowScale();
		newRec.right *= m_pAUIManager->GetWindowScale();

		pDlg->Resize(oldRec, newRec);
