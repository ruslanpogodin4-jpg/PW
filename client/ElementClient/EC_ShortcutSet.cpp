...
//	Create a auto equipment shortcut at specified position
bool CECShortcutSet::CreateAutoEquipShortcut(int iSlot, int iEquipIdx)
{
	CECSCAutoEquip* pAutoEquipSC = new CECSCAutoEquip;
	if (!pAutoEquipSC)
		return false;

	if (!pAutoEquipSC->Init(iEquipIdx))
	{
		delete pAutoEquipSC;
		a_LogOutput(1, "CECShortcutSet::CreateAutoEquipShortcut, Failed to initialize auto fashion shortcut");
		return false;
	}

	SetShortcut(iSlot, pAutoEquipSC);
	return true;
}
...
bool CECShortcutSet::SaveConfigData(void* pDataBuf, int* piSize)
{
	...
	for (int i=0; i < m_aShortcuts.GetSize(); i++)
	{
		...
		case CECShortcut::SCT_SYSMODULE:
			...
			break;
		case CECShortcut::SCT_AUTOEQUIP:

			iTotalSize += sizeof(int);
			if (pDataBuf)
			{
				*((int*)pData) = ((CECSCAutoEquip*)pSC)->GetAutoEquipIndex();
				pData += sizeof(int);
			}
...
bool CECShortcutSet::LoadConfigData(const void* pDataBuf, int* piSize, DWORD dwVer)
{
	...
	while (1)
	{
		...
		case CECShortcut::SCT_AUTOEQUIP:
		{
			if (dwVer >= 5)
			{
				int iAutoEquipIndex = *(int*)pData;
				pData += sizeof(int);

				if (iAutoEquipIndex >= 0)
					CreateAutoEquipShortcut(iSlot, iAutoEquipIndex);
			}
			else
			{
				ASSERT(0);
