...
{
	bool v = true;
	Tick(dwDeltaTime, NULL, v);
}
void A3DSkillGfxEvent::Tick(DWORD dwDeltaTime, A3DGFXEx** ppGfx, bool& useDeltaTime)
{
	...
	else if (m_dwCurSpan > m_dwFlyTimeSpan) //飞行超时
	{
		if (!m_bTargetExist)
			...
			HitTarget(GetTargetCenter(), ppGfx, useDeltaTime);
	}
...
{
	bool v = true;
	HitTarget(vTarget, NULL, v);
}
void A3DSkillGfxEvent::HitTarget(const A3DVECTOR3& vTarget, A3DGFXEx** ppGfx, bool& useDeltaTime)
{
	m_enumState = enumHit;
	ReleaseFlyGfx();
