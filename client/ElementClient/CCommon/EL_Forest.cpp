...
bool CELForest::Init(A3DDevice * pA3DDevice, A3DLight * pDirLight, ALog * pLogFile)
{
...
#ifdef TREE_BUMP_ENABLE
	m_pBranchPixelShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader(SHADER_DIR"ps\\treebranch.txt", false);
	...
	m_pBranchVertexShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadVertexShader(SHADER_DIR"vs\\treebranchvs.txt", false, dwVSDecl);
#endif
	return true;
}
