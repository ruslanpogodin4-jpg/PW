...
bool globaldata_load()
{
...
#ifdef VIP
	// now load gshop3 items. (which is actually gshop2.data!)
	...
	for (i = 0; i < size; i++)
	{
		...
		// Important note: We are pushing all gshop2 (CashVip shop) items at the end of gshop.data (qshop)
		// This is done in order to work with a unified shop in the UI (gshop + gshop2).
		// gshop2 category 5 will be displayed as gshop category 5 for the user.
		// We should prevent gshop.data to have any information in category 5 in order to avoid wrong UI behaviours.
		// UI will require this items to be pushed at the end in order to calculate indexes correctly later.
		if (item.main_type == 5) global_gshop_items.push_back(item); // Adding VIP shop items in Cash Shop to handle as it were only one to the UI.
	...
	for (i = 0; i < 8; i++)
	{
		...
		if (i == 5) global_gshop_maintypes.at(i) = main_type;  // Adding VIP shop category and subcategories in Cash shop to proper names displaying
