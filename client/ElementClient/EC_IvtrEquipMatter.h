...
public:		//	Operations
	...
	virtual int GetEquipmentType() const { return EQUIP_ASTROLABE; }

	const IVTR_ESSENCE_ASTROLABE& GetEssence() { return m_Essence; }
	...
	int SumAptit();
	int GetVSlot(int index);
	int GetAptit(int index);
	float GetMaxRatio(bool bBirthStar);
	float GetAddonRatioForAptitude(float aptitude, int level);
	int GetExpForLevel(int level);
	int TestInsertExp(int newExpAmount);
	int SumExp();
...
protected:	//	Attributes
	...
	ASTROLABE_LEVELEXP_CONFIG* m_pDBExperience;
	ASTROLABE_APPEARANCE_CONFIG* m_pDBAppearance;
