...
void c2s_SendCmdReloadInstance()
{
	_SendNakeCommand(C2S::RELOAD_INSTANCE);
}
...
void c2s_SendCmdCashVipMallShopping(int iCount, const void* pGoods)
{
	using namespace C2S;

	int iSizeGoods = iCount * sizeof(cmd_cash_vip_mall_shopping::goods);
	int iSize = sizeof(cmd_header) + sizeof(int) + iSizeGoods;
	BYTE* pBuf = (BYTE*)a_malloctemp(iSize);
	if (!pBuf)
		return;

	((cmd_header*)pBuf)->cmd = C2S::CASH_VIP_MALL_SHOPPING;

	cmd_cash_vip_mall_shopping* pCmd = (cmd_cash_vip_mall_shopping*)(pBuf + sizeof(cmd_header));
	pCmd->count = iCount;
	memcpy(pCmd->list, pGoods, iSizeGoods);
...
void c2s_SendCmdNPCSevCashVipMallShopping(int iCount, const void* pGoods)
{
	using namespace C2S;
	...
	cmd_cash_vip_mall_shopping* pContent = (cmd_cash_vip_mall_shopping*)(pBuf + sizeof(cmd_header) + sizeof(cmd_sevnpc_serve));
	pContent->count = iCount;
	memcpy(pContent->list, pGoods, iSizeGoods);
...
void c2s_SendCmdGetCashVipMallItemPrice(int start_index, int end_index)
{
	using namespace C2S;

	const int iSize = sizeof(cmd_header) + sizeof(cmd_get_cash_vip_mall_item_price);
	BYTE* pBuf = (BYTE*)a_malloctemp(iSize);

	((cmd_header*)pBuf)->cmd = C2S::GET_CASH_VIP_MALL_ITEM_PRICE;

	cmd_get_cash_vip_mall_item_price* pCmd = (cmd_get_cash_vip_mall_item_price*)(pBuf + sizeof(cmd_header));
	pCmd->start_index = start_index;
	pCmd->end_index = end_index;
...
void c2s_SendCmdRemoteRepair()
{
	_SendNakeCommand(C2S::REMOTE_REPAIR);
}

void c2s_SendCmdPickupAll(int iCount, const void* pMatterList)
{
	using namespace C2S;

	int iSizeMatter = iCount * sizeof(cmd_pickup_matter_all::matter);
	int iSize = sizeof(cmd_header) + sizeof(int) + iSizeMatter;
	BYTE* pBuf = (BYTE*)a_malloctemp(iSize);
	if (!pBuf)
		return;

	((cmd_header*)pBuf)->cmd = PICKUP_ALL;

	cmd_pickup_matter_all* pCmd = (cmd_pickup_matter_all*)(pBuf + sizeof(cmd_header));
	pCmd->count = iCount;
	memcpy(pCmd->matter, pMatterList, iSizeMatter);
...
void c2s_SendCmdFixPositionTransmitOperateRequestAdd(A3DVECTOR3 pos, const void* szName)
{
	using namespace C2S;
	...
	cmdopt_fix_position_transmit_add_position data;
	data.pos[0] = pos.x;
	data.pos[1] = pos.y;
	data.pos[2] = pos.z;
	...
	memcpy(data.position_name, szName, iNameSize);


	SendCmdFixPositionTransmitOperateRequest(0, &data);
}

void c2s_SendCmdFixPositionTransmitOperateRequestDelete(int index)
{
	using namespace C2S;

	cmdopt_fix_position_transmit_delete_position data;
	data.index = index;

	SendCmdFixPositionTransmitOperateRequest(1, &data);
}

void c2s_SendCmdFixPositionTransmitOperateRequestTeleport(int index)
{
	using namespace C2S;

	cmdopt_fix_position_transmit_transmit data;
	data.index = index;

	SendCmdFixPositionTransmitOperateRequest(2, &data);
}

void c2s_SendCmdFixPositionTransmitOperateRequestRename(int index, const void* szName)
{
	using namespace C2S;
	...
	cmdopt_fix_position_transmit_rename data;
	data.index = index;
	...
	SendCmdFixPositionTransmitOperateRequest(3, &data);
}

void SendCmdFixPositionTransmitOperateRequest(int optType, const void* pData)
{
	using namespace C2S;

	int iSizeContent = 0;
	switch (optType) {
	case 0: // FIX_POSITION_TRANSMIT_OPT_ADD_POSITION
		iSizeContent = sizeof(cmdopt_fix_position_transmit_add_position);
		break;
	case 1: // FIX_POSITION_TRANSMIT_OPT_DELETE_POSITION
		iSizeContent = sizeof(cmdopt_fix_position_transmit_delete_position);
		break;
	case 2: // FIX_POSITION_TRANSMIT_OPT_TRANSMIT
		iSizeContent = sizeof(cmdopt_fix_position_transmit_transmit);
		break;
	case 3: // FIX_POSITION_TRANSMIT_OPT_RENAME
		iSizeContent = sizeof(cmdopt_fix_position_transmit_rename);
		break;
	}

	int iSize = sizeof(cmd_header) + sizeof(cmd_fix_position_transmit_operate_request) + iSizeContent;
	BYTE* pBuf = (BYTE*)a_malloctemp(iSize);
	if (!pBuf)
		return;
	((cmd_header*)pBuf)->cmd = C2S::FIX_POSITION_TRANSMIT_OPERATE_REQUEST;

	cmd_fix_position_transmit_operate_request* pCmd = (cmd_fix_position_transmit_operate_request*)(pBuf + sizeof(cmd_header));
	pCmd->opttype = optType;
	memcpy(pCmd->data, pData, iSizeContent);
...
void c2s_SendCmdCashResurrect(int param) {
	using namespace C2S;
	...
	((cmd_header*)pBuf)->cmd = RESURRECT_BY_CASH;
...
void c2s_SendCmdSoloChallengeOperateRequest(int optType, const void* pData) {
	...
	switch (optType) {
		case 1: // SOLO_CHALLENGE_OPT_SELECT_AWARD
			iSizeContent = sizeof(cmdopt_solo_challenge_opt_select_award);
			break;
		case 2: // SOLO_CHALLENGE_OPT_SCORE_COST
			iSizeContent = sizeof(cmdopt_solo_challenge_opt_score_cost);
			break;
	}

	int iSize = sizeof(cmd_header) + sizeof(cmd_solo_challenge_operate_request) + iSizeContent;

	BYTE* pBuf = (BYTE*)a_malloctemp(iSize);
	if (!pBuf)
		return;
	((cmd_header*)pBuf)->cmd = C2S::SOLO_CHALLENGE_OPERATE_REQUEST;

	cmd_solo_challenge_operate_request* pCmd = (cmd_solo_challenge_operate_request*)(pBuf + sizeof(cmd_header));
	pCmd->opttype = optType; // SOLO_CHALLENGE_OPT_LEAVE_THE_ROOM

	switch (optType) {
	case 1: // SOLO_CHALLENGE_OPT_SELECT_AWARD
		memcpy(pCmd->data, pData, sizeof(cmdopt_solo_challenge_opt_select_award));
		break;
	case 2: // SOLO_CHALLENGE_OPT_SCORE_COST
		memcpy(pCmd->data, pData, sizeof(cmdopt_solo_challenge_opt_score_cost));
		break;
	}
...
void c2s_SendCmdNPCSevSoloChallengeSelectStage(int stage)
{
	_SendNPCSev(GP_NPCSEV_SOLO_CHALLENGE_STAGE, static_cast<void*>(const_cast<int*>(&stage)), sizeof(int));
}

void c2s_SendCmdNPCSevMakeAccessorySlot(int src_index, int src_id, unsigned int material_id, int material_count)
{
	using namespace C2S;

#pragma pack(1)

	struct make_accessory_slot_request
	{
...
#pragma pack()
	...
	int iSize = sizeof(cmd_header) + sizeof(cmd_sevnpc_serve) + sizeof(make_accessory_slot_request);
	BYTE* pBuf = (BYTE*)a_malloctemp(iSize);
	if (!pBuf)
		return;
	...
	cmd_sevnpc_serve* pCmd = (cmd_sevnpc_serve*)(pBuf + sizeof(cmd_header));
	pCmd->service_type = GP_NPCSEV_MAKE_SLOT_FOR_DECORATION;
	pCmd->len = sizeof(make_accessory_slot_request);

	BYTE* pData = pBuf + sizeof(cmd_header) + sizeof(cmd_sevnpc_serve);
	make_accessory_slot_request* pContent = (make_accessory_slot_request*)pData;
	pContent->src_index = src_index;
	pContent->src_id = src_id;
	pContent->material_id = material_id;
	pContent->material_count = material_count;
...
void c2s_SendCmdArenaEnter(int battle_mode)
{
	using namespace C2S;

	int iSize = sizeof(cmd_header) + sizeof(cmd_arena_request_battle_queue);
	BYTE* pBuf = (BYTE*)a_malloctemp(iSize);
	if (!pBuf)
		return;

	((cmd_header*)pBuf)->cmd = ARENA_REQUEST_BATTLE_QUEUE;

	cmd_arena_request_battle_queue* pCmd = (cmd_arena_request_battle_queue*)(pBuf + sizeof(cmd_header));
	pCmd->battle_mode = battle_mode;
...
void c2s_SendCmdArenaAccept(bool accepted)
{
	using namespace C2S;

	int iSize = sizeof(cmd_header) + sizeof(cmd_arena_accept_battle);
	BYTE* pBuf = (BYTE*)a_malloctemp(iSize);
	if (!pBuf)
		return;

	((cmd_header*)pBuf)->cmd = ARENA_ACCEPT_BATTLE;

	cmd_arena_accept_battle* pCmd = (cmd_arena_accept_battle*)(pBuf + sizeof(cmd_header));
	pCmd->accepted = accepted;
...
void c2s_SendCmdArenaExit()
{
	_SendNakeCommand(C2S::ARENA_EXIT_BATTLE);
