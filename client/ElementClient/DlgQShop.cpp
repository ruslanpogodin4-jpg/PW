...
void CDlgQShop::ShowMainType(int nCommandMainType) {
	...
	bool bShowPreviewShop(false);
	if (IsPreviewShopCategory(nMainType)){
		bShowPreviewShop = true;
	} else if (IsFashionShopCategory(nMainType)){
	...
	} else if (bShowPreviewShop) {
		if (!IsPreviewShopCategory(m_nMainType)){
			nMainType = m_nMainType;
		}else{
			for (int i(0); i < (int)maintype->size(); ++ i){
				if (!IsPreviewShopCategory(i)){
					nMainType = i;
				}
			}
		}
	...
	if (bShowPreviewShop){
		ShowPreviewShop(GetShopDataConst().GetOwnerNPCID());
	}
}
...
bool CDlgQShop::ShowCategory(int mainType, int subType, unsigned int ownerNPCID)
{
	...
	while (true)
	{
		...
		if (IsPreviewShopCategory(mainType)){
			bShown = ShowPreviewShop(ownerNPCID);
			break;
		}
...
bool CDlgQShop::IsPreviewShopCategory(int mainType){
	return GetShopData().GetPreviewShopCategory().MatchMainType(mainType);
}

bool CDlgQShop::ShowPreviewShop(unsigned int ownerNPCID)
{
	return CECUIHelper::ShowPreviewShop(IsQShop(), this, ownerNPCID);
}
