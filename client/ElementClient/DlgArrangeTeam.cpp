...
void CDlgArrangeTeam::DoDataExchange(bool bSave)
{
	...
	// BOLA TO ADD
	DDX_Control("Lbl_NumPeopleAround", m_pLbl_NumPeopleAround);
}
...
bool CDlgArrangeTeam::UpdateTeam(bool bUpdateNear)
{
	...
	if( bUpdateNear )
	{
		...
		// BOLA TO ADD
		ACString txtnumppl; 
		txtnumppl.Format(L"(%d)", PlayerNear.GetSize());
		m_pLbl_NumPeopleAround->SetText(txtnumppl);
		m_pLbl_NumPeopleAround->SetColor(A3DCOLORRGB(255,0,0));
	}
