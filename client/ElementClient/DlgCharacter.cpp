...
bool CDlgCharacter::OnInitDialog()
{
	...
	m_pTxt_AntiDefenseDegree = (PAUILABEL)GetDlgItem("Txt_AntiDefenseDegree");
	m_pTxt_AntiResistanceDegree = (PAUILABEL)GetDlgItem("Txt_AntiResistanceDegree");
...
void CDlgCharacter::RefreshHostDetails()
{
	...
	strText.Format(GetStringFromTable(11146), (vigour_realm > 1500 ? 1500 : vigour_realm));
...
*/
	...
	a_sprintf(szText, _AL("%d"), rbp.iPraySpeed);
	GetDlgItem("Txt_CastTime")->SetText(szText);

	a_sprintf(szText, _AL("%d"), rbp.iTaskCount);
	GetDlgItem("Txt_TaskCompleted")->SetText(szText);

	a_sprintf(szText, _AL("%d/%d"), GetHostPlayer()->GetTitles().size(), titlesnum);
	GetDlgItem("Txt_TitlesAcquired")->SetText(szText);

	a_sprintf(szText, _AL("%d"), rbp.iKilledMonsters);
	GetDlgItem("Txt_MonstersKilled")->SetText(szText);

	a_sprintf(szText, _AL("%d"), rbp.iKilledPlayers);
	GetDlgItem("Txt_PlayersKilled")->SetText(szText);

	a_sprintf(szText, _AL("%d"), rbp.iDeathByPlayers);
	GetDlgItem("Txt_PlayerDeaths")->SetText(szText);
