...
bool CECHostPlayer::CmdPickupAll()
{
	...
	int requiredVipLevel = GetVipLevelForVipService(VipServices::CVS_PICKALL);
	if (GetVipLevel() < requiredVipLevel) {
		CECGameUIMan* pGameUI = g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan();
		ACString msgStr;
		msgStr.Format(pGameUI->GetStringFromTable(11730), requiredVipLevel, pGameUI->GetStringFromTable(11722));
		pGameUI->AddChatMessage(msgStr, GP_CHAT_MISC);
		return false;
	}

	// See if we need to cancel sitdown work.
	if (m_pWorkMan->IsSitting())
	{
		g_pGame->GetGameSession()->c2s_CmdStandUp();
		return false;
	...
	CECMatterMan* pMatterMan = g_pGame->GetGameRun()->GetWorld()->GetMatterMan();
	int countMatter;
	if (pMatterMan->FindMattersInRange(10.0f, true, matters)) {

		int* matterList = new int[matters.size() * 2];
		int i = 0;
		for (matIt = matters.begin(); matIt != matters.end(); ++matIt) {
			matterList[i++] = (*matIt)->GetMatterID();
			matterList[i++] = (*matIt)->GetTemplateID();
		}

		g_pGame->GetGameSession()->c2s_CmdPickupAll(matters.size(), matterList);
		delete matterList;
	}
...
bool CECHostPlayer::CmdRemoteRepair()
{	
	g_pGame->GetGameSession()->c2s_CmdRemoteRepair();
	return true;
}

bool CECHostPlayer::CmdQuickTeleport()
{
	// Check if user has enough VIP level to use this feature
	int requiredVipLevel = GetVipLevelForVipService(VipServices::CVS_FIX_POSITION);
	if (GetVipLevel() < requiredVipLevel) {
		CECGameUIMan* pGameUI = g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan();
		ACString msgStr;
		msgStr.Format(pGameUI->GetStringFromTable(11730), requiredVipLevel, pGameUI->GetStringFromTable(11723));
		pGameUI->AddChatMessage(msgStr, GP_CHAT_MISC);
		return false;
	}

	CECGameUIMan* pGameUI = g_pGame->GetGameRun()->GetUIManager()->GetInGameUIMan();
	pGameUI->GetDialog("Win_MarkVIP")->Show(true);
	return true;
}