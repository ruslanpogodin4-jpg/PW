...
// 注意，用于printf的格式化字符串与用于sscanf的格式化字符串不同
// 用于sscanf的格式为: %[*] [width] [{h | l | ll | I64 | L}]type
// 用于printf的格式为: %[flags] [width] [.precision] [{h | l | ll | I | I32 | I64}]type
// 读取的时候没有.precision，而写的时候有
static const char* _format_double_w	= "Double: %.14lg";		// 这里用.14lg使得小数点后14位的值可以被存储或者读取，默认%lf,%lg都只有小数点后6位
