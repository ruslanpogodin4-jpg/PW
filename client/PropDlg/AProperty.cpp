...
{
	assert(pBase);
	return pBase->GetPropVal(m_nIndex);
}

void AVariantProperty::SetValue(APropertyObject * pBase, AVariant value)
{
	assert(pBase);
	pBase->SetPropVal(m_nIndex, value);
}

void AVariantProperty::SetStringValue(APropertyObject *pBase, const char * str)
{
	AString tmpStr(str);
	pBase->SetPropVal(m_nIndex, AVariant(tmpStr));
}

void AVariantProperty::GetStringValue(APropertyObject *pBase, AString &str)
{
	str = (AString)pBase->GetPropVal(m_nIndex);
}