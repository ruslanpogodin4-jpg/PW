...
Edge CGiftWrap::SearchFirstEdge(const A3DVECTOR3& vRefNormal)
{
	...
	for(i=1;i<m_nVNum;i++)
	{
		if(yMin-m_pVertexes[i].y>m_fIdenticalPosErr)	// 说明比yMin更小
		{
	...
	else
	{
		...
		{
			if(ZMin-m_pVertexes[LowestVertexes[i]].z>m_fIdenticalPosErr)	// 说明比yMin更小
			{
		...
		else
		{
			...
			{
				if(XMin-m_pVertexes[ZMinVertexes[i]].x>m_fIdenticalPosErr)	// 说明比xMin更小
				{
					XMin=m_pVertexes[ZMinVertexes[i]].x;
					xminID=ZMinVertexes[i];
				}
				if(m_pVertexes[ZMinVertexes[i]].x-XMax>m_fIdenticalPosErr)	// 说明比xMin更小
				{
