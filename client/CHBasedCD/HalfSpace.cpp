...
*/
	...
	const float* points = (const float*)Verts;

	BEST_FIT::computeBestFitPlane(VNum, points, sizeof(A3DVECTOR3), 0, 0, p);
	A3DVECTOR3 vN(p[0], p[1], p[2]);
	hsPlane.SetNormal(vN);
	hsPlane.SetD(-p[3]);

	// now check the distance error...
	for(int i=0; i<VNum; i++)
	{
		if(hsPlane.DistV2Plane(Verts[i]) > MaxDistError)
			return false;
	}
