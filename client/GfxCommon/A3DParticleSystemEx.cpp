...
bool A3DParticleSystemEx::Init(A3DDevice* pDevice)
{
	...
	::InitializeCriticalSection(&m_csLstFree);
...
void A3DParticleSystemEx::SetPoolSize(int nSize)
{
	...
	pMan->LockParticlePoolList();
	ParticlePoolList& pool = pMan->GetParticlePool();
	bool bLimit = pMan->GetParticlePoolLimit();
...
void A3DParticleSystemEx::TriggerAffectors(float fTimeSpan, DWORD dwTick)
{
	...
	//long long start = std::chrono::time_point_cast<std::chrono::milliseconds>(begin).time_since_epoch().count();
	//long long endV = std::chrono::time_point_cast<std::chrono::milliseconds>(end).time_since_epoch().count();

	//long long t = endV - start;

	//g_particleTime += (__int64)t;
}
...
void A3DParticleSystemEx::Release()
{
	...
	::DeleteCriticalSection(&m_csLstFree);
