...
{
public:
	A3DGFXStreamMan() : m_nRef(0), m_pBuf(NULL), m_DxBuf(NULL), m_DxSizeLock(0) { ::InitializeCriticalSection(&m_csStream); }
	virtual ~A3DGFXStreamMan() { ::DeleteCriticalSection(&m_csStream); delete m_pBuf; }
...
public:
	...
	{
		memcpy(m_DxBuf, m_pBuf, m_DxSizeLock);
		m_SharedStream.UnlockVertexBuffer();
		Unlock();
	}
