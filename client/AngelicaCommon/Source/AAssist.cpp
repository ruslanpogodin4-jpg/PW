...
inline float a_FastInvSqrt(float x) {
	float xhalf = 0.5f * x;
	int i = *(int*)&x;
	i = 0x5f3759df - (i >> 1);
	x = *(float*)&i;
...
//float a_FastInvSqrt(float x) { return 1.0 / sqrt(x); }

float a_FastSqrt(float x) { return 1.0 / a_FastInvSqrt(x); }
//float a_FastSqrt(float x) { return sqrt(x); }
