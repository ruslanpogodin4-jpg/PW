...
namespace pc {
	...
	void myLZARI::Error(char *message)
	{
	...
	void myLZARI::PutBit(int bit)  /* Output one bit (bit = 0,1) */
	{
	...
	void myLZARI::FlushBitBuffer(void)  /* Send remaining bits */
	{
		int  i;
	...
	int myLZARI::GetBit(void)  /* Get one bit (0 or 1) */
	{	
		if ((mask_getbit >>= 1) == 0) 
		{
			if (!m_bMem)
	...
	void myLZARI::InitTree(void)  /* Initialize trees */
	{
		int  i;
	...
	void myLZARI::DeleteNode(int p)  /* Delete node p from tree */
	{
		int  q;
	...
	void myLZARI::StartModel(void)  /* Initialize model */
	{
		int ch, sym, i;
	...
	void myLZARI::UpdateModel(int sym)
	{
		int i, c, ch_i, ch_sym;
	...
	void myLZARI::Output(int bit)  /* Output 1 bit, followed by its complements */
	{
		PutBit(bit);
		for ( ; shifts > 0; shifts--) PutBit(! bit);
	}

	void myLZARI::EncodeChar(int ch)
	{
		int  sym;
	...
	void myLZARI::EncodePosition(int position)
	{
		unsigned long int  range;
	...
	int myLZARI::BinarySearchSym(unsigned int x)
		/* 1      if x >= sym_cum[1],
	...
	int myLZARI::BinarySearchPos(unsigned int x)
	...
	int myLZARI::DecodePosition(void)
	{
		int position;
		unsigned long int  range;
	...
	void myLZARI::Compress(const char *lpszInfile,const char *lpszOutfile)
	{
		m_bMem = FALSE;
	...
	void myLZARI::UnCompress(const char *lpszInfile,const char *lpszOutfile)
	{
		m_bMem = FALSE;
	...
	void myLZARI::UnCompressW(const wchar_t *lpszInfile, const wchar_t *lpszOutfile)
	{
		m_bMem = FALSE;

		 _wfopen_s(&infile, lpszInfile, L"rb");
		 _wfopen_s(&outfile, lpszOutfile, L"wb");
		if(infile && outfile)
		{
			Decode();
			fclose(infile);
	...
	void myLZARI::Compress(const BYTE *pInBuffer,int nInLength,const BYTE *&pOutBuffer ,int &nOutLength)
	{
	...
	void myLZARI::UnCompress(const BYTE *pInBuffer,int nInLength,const BYTE *&pOutBuffer ,int &nOutLength)
	{
		m_pInBuffer = pInBuffer;
		m_nInLength = nInLength;
		m_nInCur = 0;
