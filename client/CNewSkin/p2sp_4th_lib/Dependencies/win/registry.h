...
public:
    CRegistryKey();
    CRegistryKey(HKEY rootkey, const wchar_t* subkey, REGSAM access);
    ~CRegistryKey();
    ...
    HANDLE WatchEvent() const { return watch_event_; }
    HKEY Handle() const { return key_; }

private:
    CRegistryKey(const CRegistryKey&);
    void operator=(const CRegistryKey&);
...
// adequate.
class CRegistryValueIterator
{
public:
    CRegistryValueIterator(HKEY root_key, const wchar_t* folder_key);

    ~CRegistryValueIterator();
...
private:
    CRegistryValueIterator(const CRegistryValueIterator&);
    void operator=(const CRegistryValueIterator&);
...
class CRegistryKeyIterator
{
public:
    CRegistryKeyIterator(HKEY root_key, const wchar_t* folder_key);

    ~CRegistryKeyIterator();
...
private:
    CRegistryKeyIterator(const CRegistryKeyIterator&);
    void operator=(const CRegistryKeyIterator&);
