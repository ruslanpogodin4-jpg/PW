...
static void StringAppendVT(StringType* dst, const typename StringType::value_type* format,
                           va_list ap)
{
    // First try with a small fixed size buffer.
    // This buffer size should be kept in sync with StringUtilTest.GrowBoundary
    // and StringUtilTest.StringPrintfBounds.
    typename StringType::value_type stack_buf[1024] = { 0 };
    int mem_length = sizeof(stack_buf) / sizeof(stack_buf[0]);

    va_list ap_copy = ap;

    int result = vsnprintfT(stack_buf, mem_length, format, ap_copy);
    va_end(ap_copy);

    if(result>=0 && result<static_cast<int>(mem_length))
    {
        // It fit.
    ...
    while(true)
    {
        ...
        if(mem_length > 32*1024*1024)
        {
            ...
            ATLASSERT(!"Unable to printf the requested string due to size.");
            return;
        }
