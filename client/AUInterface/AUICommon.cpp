...
ACString UnmarshalEditBoxText(const ACHAR* szText, EditBoxItemsSet& ItemsSet, int nMsgIndex, const ACHAR* szIvtrItem,
	...
	AUI_UNMARSH_IVTRITEM_ICON_INFO* pIconInfo,
	abase::vector<A2DSprite*>* pvecImageList)
{
	if( !szText )
		return _AL("");
	...
	return UnmarshalEditBoxTextEx(szText, ItemsSet, nMsgIndex, szIvtrItem, clIvtrItem, nItemMask, &scriptInfo, &underlineInfo, pIconInfo, pvecImageList);
}
			...
			if ((pItem->GetType() >= enumEICustom && (nItemMask & (1 << enumEICustom))) || (nItemMask & (1 << pItem->GetType())))
			{
				...
				else
				{
					...
					case enumEIIvtrlItem:
						...
						// Insert icon before item name (pItem already added as ch, insert icon before ch in str)
						if (pIconInfo && pvecImageList && pIconInfo->pIconSprite && pIconInfo->nIconIndex >= 0 && ch != 0)
						{
							int nImgIdx = -1;
							for (int k = 0; k < (int)pvecImageList->size(); k++)
							{
								if ((*pvecImageList)[k] == pIconInfo->pIconSprite)
								{ nImgIdx = k; break; }
							}
							if (nImgIdx == -1)
							{
								nImgIdx = (int)pvecImageList->size();
								pvecImageList->push_back(pIconInfo->pIconSprite);
							}

							EditBoxItemBase* pIconItem = new EditBoxItemBase(enumEIImage);
							pIconItem->SetImageIndex(nImgIdx);
							pIconItem->SetImageFrame(pIconInfo->nIconIndex);
							pIconItem->SetImageScale(0.5f);

							wchar_t chIcon = ItemsSet.AppendItem(pIconItem);
							if (chIcon == 0)
								delete pIconItem;
							else
							{
								// Remove last char (ch = item), insert icon, re-append ch
								str.CutRight(1);
								str += chIcon;
								str += ch;
							}
