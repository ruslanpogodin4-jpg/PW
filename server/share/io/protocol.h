...
namespace GNET
{
	...
	private:
		struct Represent
		{
			...
			static Rep &GetRep(const Protocol *protocol)
			{
				...
				if (it == map->end())
					it = map->insert(std::make_pair(protocol, Rep())).first;
				return (*it).second;
			}
	...
	public:
		...
		public:
			...
			protected:
				...
				ID nextsid()
				{
					Thread::Mutex::Scoped l(session_id_mutex);
					return ++session_id;
				}
			...
			private:
				...
				{
					Octets &input = Input();
					const unsigned char *p = (const unsigned char *)input.begin();
					printf("======== ON RECV : ");
					...
					try
					{
						...
						{
							count++;
							printf("======== DECODED PROTOCOL TYPE :  %d \n", p->type);
						}
		...
		public:
			...
			virtual void CheckForOOGAttack(Session::ID sid, size_t type, size_t sizebuf) {}
		};
	...
	private:
		...
		public:
			...
			Task(Manager *m, Manager::Session::ID id, Protocol *p) : manager(m), sid(id), protocol(p) {}
			void Run()
			{
				try
				{
	...
	private:
		static void CheckPolicy(Type type, unsigned int size, const Manager::Session::Stream &is)
		{
			if (Protocol::GetStub(type))
			{
				...
				if (!is.session->StatePolicy(type))
				{
					...
					{
						Log::log(is.session->GetVerbose(), "Protocol state error. sid=%d,type=%d,size=%d,acceptsize=%d",
		...
		static Protocol *Decode(const Manager::Session::Stream &is)
		{
			...
			unsigned int size = 0;
			const unsigned char *p = (const unsigned char *)is.begin();
			printf("======== DECODE : ");
			...
			try
			{
				...
				while (loop)
				{
					...
					if (is.check_policy)
					{
						...
						is.session->manager->CheckForOOGAttack(is.session->GetSid(), type, size);
