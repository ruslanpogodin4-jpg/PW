...
#include "arenabattlequeueapply.hpp"
#include "arenabattleserverregister.hpp"
#include "arenabattleend.hpp"
#include "arenabattlestart_re.hpp"
#include <pthread.h>
#include <conf.h>
#include <queue>
#include <glog.h>
...
#define MAX_USER_CMD_QUEUE_NUM 2
	...
	bool SystemChatMsg(const void *msg, size_t size, char channel, const void *data, size_t dsize)
	{
		...
		if (data)
			packet.data.insert(packet.data.end(), data, dsize);
		packet.roleid = 0;
	...
	void ReportCheater2Gacd(int roleid, int cheattype, const void *buf, size_t size)
	{
		...
		if (buf)
			acrc.cheatinfo.replace(buf, size);
	...
	void GMArenaBattleQueueApply(IntraArenaApplyEntry *list, size_t count, int battle_mode, int teamid)
	{
		GNET::ArenaBattleQueueApply data;
		for (size_t i = 0; i < count; i++)
		{
			data.list.push_back(ArenaApplyEntry(list[i].roleid, list[i].total_victory, list[i].total_defeat, list[i].kill_number, list[i].death_number));
		}
	...
	void SendArenaBattleServerRegister(int type, int world_index, int world_tag, int battle_type)
	{
		GProviderClient::DispatchProtocol(0, ArenaBattleServerRegister(type, battle_type, world_index, world_tag));
	}

	void SendArenaBattleEnd(int battle_id, int result, int winner)
	{
		GProviderClient::DispatchProtocol(0, ArenaBattleEnd(battle_id, result, winner));
	}

	void ResponseArenaBattleStart(int retcode, int battle_id)
	{
		GProviderClient::DispatchProtocol(0, ArenaBattleStart_Re(battle_id, retcode));
	}

	void
	CountryBattleJoin(int roleid, int country_id, int worldtag, int major_strength, int minor_strength, char is_king)
	{
