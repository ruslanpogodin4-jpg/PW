...
struct SessionInfo
{
	...
	SessionInfo() : userid(0), roleid(0), gsid(0), login_time(0), busy(0), local(0), peer(0), checker(NULL), passwd_flag(0), accountinfo_flag(0), _load_state(0)
	{
	}
...
class GLinkServer : public TimerManager, public Timer::Observer
{
	...
	void CheckForOOGAttack(Session::ID sid, size_t type, size_t sizebuf);
	PassiveIO* passiveio;
public:
...
class AntiOOGTimer : public Thread::Runnable
{
	int update_time;
public:
	AntiOOGTimer(int _time,int _proir=1) : Runnable(_proir),update_time(_time) { }
	void Run();
private:
	void HeartbeatOOG();
};
