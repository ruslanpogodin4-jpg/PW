...
void GLinkServer::CheckForOOGAttack(Session::ID sid, size_t type, size_t sizebuf)
{
	...
	{
		SessionInfo &info = it->second;
		info._load_state += oog->Get(type);
		if (oog->Check(info._load_state))
		{
			Log::log(LOG_ERR, "PLAYER DISCONNECTED BY OOG FILTER: sid=%d,userid=%d,roleid=%d,load_state=%d,protocol_opcode=%d,protocol_size=%d", sid, info.userid, info.roleid, info._load_state, type, sizebuf);
			Close(sid, CLOSE_EXCEPTION);
		}
	}
}

void AntiOOGTimer::HeartbeatOOG()
{
	...
	{
		SessionInfo &info = (*it).second;
		oog->HeartBeat(info._load_state);
	}
}

void AntiOOGTimer::Run()
{
	HeartbeatOOG();
