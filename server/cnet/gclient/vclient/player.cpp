...
void gplayer_imp::PlayerEnterWorld()
{
	...
	// Приветственное сообщение при входе игрока на сервер
	{
		char welcome_msg[256];
		int msg_len = snprintf(welcome_msg, sizeof(welcome_msg),
			"Игрок %.*s Добро пожаловать на сервер!",
			(int)_username_len, _username);
		if (msg_len > 0)
			broadcast_chat_msg(_parent->ID.id, welcome_msg, msg_len, GMSV::CHAT_CHANNEL_SYSTEM, 0, 0, 0);
	}
}
