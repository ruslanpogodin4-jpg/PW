...
namespace GDB
{
	namespace
	{
				...
				for(it = extraprop.data.begin(); it != extraprop.data.end(); ++it)
				{
					try
					{
						switch(it->first)
						{
                            ...
                            case GROLE_STATUS_EXTRAPROP_CASH_RESURRECT_INFO:
                                ...
                                break;
							case GROLE_STATUS_EXTRAPROP_HOME_LEVEL_DATA:
								{
									Marshal::OctetsStream(it->second) >> info.home_level;
								}
								break;

							case GROLE_STATUS_EXTRAPROP_HOME_TASK_DATA:
								{
									Octets task_info;
									Marshal::OctetsStream(it->second) >> info.home_task_info.task_trigger_times >> info.home_task_info.task_refresh_times >> task_info;

									if (task_info.size() != sizeof(info.home_task_info.task_id)) throw Marshal::Exception();
                                    memcpy(info.home_task_info.task_id, task_info.begin(), task_info.size());
								}
								break;
								case GROLE_STATUS_EXTRAPROP_NEWSTATS_DATA:
                                {
                                    Marshal::OctetsStream(it->second)
                                        >> info.newstats_extern.killed_monsters
                                        >> info.newstats_extern.killed_players
                                        >> info.newstats_extern.death_by_monsters
                                        >> info.newstats_extern.death_by_players;
                                }
                                break;
								case GROLE_STATUS_EXTRAPROP_ARENABATTLE_DATA:
                                {
                                    Marshal::OctetsStream(it->second)
                                        >> info.arena_extern.total_victory
                                        >> info.arena_extern.total_defeat
                                        >> info.arena_extern.total_kills
                                        >> info.arena_extern.total_deaths
                                        >> info.arena_extern.total_assistances
                                        >> info.arena_extern.arena_rank
                                        >> info.arena_extern.arena_grade_matchmaking;
                                }
                                break;
					...
					catch(Marshal::Exception)
					{
						switch(it->first)
						{
							...
							case GROLE_STATUS_EXTRAPROP_HOME_TASK_DATA:
								{
									info.home_task_info.task_refresh_times =0;
									info.home_task_info.task_trigger_times =0;

									memset(info.home_task_info.task_id, 0, sizeof(info.home_task_info.task_id));
								}
								break;
							case GROLE_STATUS_EXTRAPROP_NEWSTATS_DATA:
								{
									info.newstats_extern.killed_monsters = 0;
									info.newstats_extern.killed_players = 0;
									info.newstats_extern.death_by_monsters = 0;
									info.newstats_extern.death_by_players = 0;
								}
								break;
							case GROLE_STATUS_EXTRAPROP_ARENABATTLE_DATA:
								{
									info.arena_extern.total_victory = 0;
									info.arena_extern.total_defeat = 0;
									info.arena_extern.total_kills = 0;
									info.arena_extern.total_deaths = 0;
									info.arena_extern.total_assistances = 0;
									info.arena_extern.arena_rank = 0;
									info.arena_extern.arena_grade_matchmaking = 0;
								}
								break;
		...
		inline void  set_extraprop(GNET::Octets& odata,const base_info& info, const vecdata & data)
		{
			...
			if(info.home_level)
			{
				extraprop.data[GROLE_STATUS_EXTRAPROP_HOME_LEVEL_DATA]
					= Marshal::OctetsStream() << info.home_level;
			}

			if(info.home_task_info.task_refresh_times >= 0 && info.home_task_info.task_trigger_times >= 0)
			{
				 extraprop.data[GROLE_STATUS_EXTRAPROP_HOME_TASK_DATA]
                    = (Marshal::OctetsStream()
                        << info.home_task_info.task_trigger_times
                        << info.home_task_info.task_refresh_times
                        << Octets(info.home_task_info.task_id, sizeof(info.home_task_info.task_id)));
			}


			extraprop.data[GROLE_STATUS_EXTRAPROP_NEWSTATS_DATA]
               = (Marshal::OctetsStream()
                   << info.newstats_extern.killed_monsters
                   << info.newstats_extern.killed_players
                   << info.newstats_extern.death_by_monsters
			...
			extraprop.data[GROLE_STATUS_EXTRAPROP_ARENABATTLE_DATA]
               = (Marshal::OctetsStream()
                   << info.arena_extern.total_victory
                   << info.arena_extern.total_defeat
                   << info.arena_extern.total_kills
