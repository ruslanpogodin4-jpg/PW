...
std::vector<int> GDeliveryServer::_limit_hwid_instances;

bool GDeliveryServer::IsLimitedInstance(int gsid)
{
	for (unsigned int i = 0; i < _limit_hwid_instances.size(); i++)
	{
		if (_limit_hwid_instances[i] == gsid)
			return true;
	}
	return false;
	// abase::hash_map<int, int>::iterator it = _limit_hwid_instances.find(gsid);
	// return it != _expire_items.end() && (it->second == 0);
}
