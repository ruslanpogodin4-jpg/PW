...
namespace GNET
{
	...
	public:
		...
		public:
			...
			int GetRoleCount()
			{
				if (!IsRoleListInitialed())
					return -1; // rolelist is not initialized
				count = 0;
			...
			int AddRole()
			{
				...
				if (GetRoleCount() == MAX_ROLE_COUNT)
				{
					return -1;
				}
				int id = 0;
			...
			int AddRole(int roleid)
			{
				...
				{
					return roleid;
				} // the role will be overlayed
				if (GetRoleCount() == MAX_ROLE_COUNT)
				{
					return -1;
				}
				if (roleid < 0 || roleid > MAX_ROLE_COUNT - 1)
				{
					return -1;
				}
				rolelist += (1 << roleid);
	...
	public:
		...
		int status;
		int gmstatus;
		Octets hwid; // Hold account hardware info
		int locked_logout;
		int linkid;
		int gameid;
		int ip; //本次账号登陆ip
		...
		UserInfo(int uid, const Octets &uname, unsigned int sid, unsigned int lsid, int st, const Octets &hardware)
			: userid(uid), account(uname), linksid(sid), localsid(lsid), status(st), hwid(hardware)
		{
			for (int i = 0; i < 16; i++)
			{
...
#define MAX_PLAYER_NUM_DEFAULT 5000
	...
	public:
		...
		void EraseName(const Octets &name)
		{
		 ...
		 bool FindHardwareID(const Octets &hwid, int gsid)
		 {
		 	Thread::RWLock::RDScoped l(locker);
		 	for (UserMap::iterator it = usermap.begin(), ite = usermap.end(); it != ite; ++it)
		 	{
		 		if ((*it).second.hwid == hwid)
				  if ((*it).second.role != NULL && (*it).second.role != 0)
		 			if ((*it).second.role->gameid == gsid)
		 				return true;
			}
		 	return false;

		 	// if(itmp->second->hwid == hwid)
		 	//	if(itmp->second->role->gameid == gsid)
		 	//		return true
		 }
		...
		void UserLogin(int userid, const Octets &account, int linksid, int localsid, bool isgm, int type, int data, int ip, const Octets &iseckey, const Octets &oseckey, const Octets &hwid, bool notify_client = true);
