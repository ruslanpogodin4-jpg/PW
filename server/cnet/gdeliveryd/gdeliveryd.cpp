...
int main(int argc, char *argv[])
{
	...
	std::string nw_hwid = conf->find(manager->Identification(), "nw_hwid_protected");
	if (nw_hwid == "true")
		manager->SetNWProtected(true);
	...
	if (!countrybattle)
		DisabledSystem::SetDisabled(SYS_COUNTRYBATTLE);
	...
	if (0 != DELIVERY_TOOL::OnFilterMode(argc, argv))
		exit(-1);
	...
	Log::setprogname("gdeliveryd");
	{
		...
		std::string limit = conf->find(manager->Identification(), "limit_hwid_gsid");

		char *delim = ",";
		char *buffer = new char[limit.length() + 1];

		if (NULL == buffer)
			return 0;

		memcpy(buffer, limit.c_str(), limit.length());
		buffer[limit.length()] = 0;

		char *token = strtok(buffer, delim);
		while (NULL != token)
		{
			manager->_limit_hwid_instances.push_back(atoi(token));
	...
	if (!ArenaBattleMan::GetInstance()->Initialize())
	{
		Log::log(LOG_ERR, "Fatal: Initialize Arena Battle Failed.");
		exit(EXIT_FAILURE);
	}
