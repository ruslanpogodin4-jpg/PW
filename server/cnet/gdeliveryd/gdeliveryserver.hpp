...
class GDeliveryServer : public Protocol::Manager
{
	...
	bool is_nw_hwid_protected;

public:
	static GDeliveryServer *GetInstance() { return &instance; }
	static std::vector<int> _limit_hwid_instances;
	...
	GDeliveryServer() : accumulate_limit(0), challenge_algo(0), token_login_support(false), max_name_len(18)
	{
		...
		link_version = 0;
		is_central_ds = false;
		is_nw_hwid_protected = false;
	}

	bool IsFreeZone() { return aid==freeaid; }

	static bool IsLimitedInstance(int gsid);
	...
	bool IsNWProtected() { return is_nw_hwid_protected; }
	void SetNWProtected(bool b) { is_nw_hwid_protected = b; }
