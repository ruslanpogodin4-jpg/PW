...
public:
	...
	void Merge(filter *f)
	{
		...
		_parent.ModifyTeamVisibleState(HSTATE_INCDEFENCE,_timeout);
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_INCATTACK,_timeout);
		if(_ratio!=nf->GetRatio())
		{
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
				_parent.ModifyTeamVisibleState(HSTATE_INCMAGIC,_timeout);
		if(_ratio!=nf->GetRatio())
		{
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_SPEEDUP,_timeout);
		if(_ratio!=nf->GetRatio())
		{
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_INCHP,_timeout);
		if(_ratio!=nf->GetRatio())
		{
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_INCMP,_timeout);	
		if(_ratio!=nf->GetRatio())
		{
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
					_parent.ModifyTeamVisibleState(HSTATE_FASTMPGEN,_timeout);	
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_FASTHPGEN,_timeout);	
		if(_point!=nf->GetPoint())
		{
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_INCDODGE,_timeout);
		if(_ratio!=nf->GetRatio())
		{
...
public:
	...
	void AdjustDamage(damage_entry & ent, const XID & attacker, const attack_msg & msg, float damage_adjust)
	{
		...
		ret.attack_range = msg.attack_range + 5.0;
		ret.speed = msg.speed;
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_FASTATTACK,_timeout);	
		if(_ratio!=nf->GetRatio())
		{
			_parent.ImpairScaleAttackSpeed(_ratio);
			_ratio = nf->GetRatio();
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_ICEBLADE,_timeout);	
		if(_ratio!=nf->GetRatio())
		{
			_ratio = nf->GetRatio();
		}
...
public:
	...
	void Merge(filter *f)
	{
		...
		if(_damage < 1)
			_damage = 1;
		if(_hstate > 0){
			_parent.ModifyTeamVisibleState(_hstate,_timeout);
		}				  
	}
...
protected:
	...
	{
		filter::Save(ar);
                ar << _trigger_prob << _period << _lasttime;
		return true;
	}
...
public:
	...
	filter_Activatereboundskill(object_interface  object, int trigger_prob, int period)
			:filter(object,FILTER_MASK), _trigger_prob(trigger_prob),_period(period),_lasttime(0)
	{
		_filter_id = FILTER_ACTIVATESKILL;
	}
	...
	void OnRelease()
	{
		if (!_parent.TestCoolDown(_parent.GetSkillWrapper().GetAsid()+COOLINGID_BEGIN)) return;	
		if (_period) {
			int remaining_time = _period + _lasttime - _parent.GetSystime();
			if (remaining_time > 0)
				_parent.SetCoolDown(_parent.GetSkillWrapper().GetAsid()+COOLINGID_BEGIN, remaining_time * 1000);
		}
	}
};
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_TOXICBLADE,_timeout);	
		if(_ratio!=nf->GetRatio())
		{
			_ratio = nf->GetRatio();
		}
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_FIREBLADE,_timeout);	
		if(_ratio!=nf->GetRatio())
		{
			_ratio = nf->GetRatio();
		}
...
public:
	...
	void Merge(filter *f)
	{
		...
		int tmp = _damage * _timeout;
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_EAGLECURSE,_timeout);	
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_ADDATTACKDEGREE,_timeout);	
		if(_add!=nf->GetValue())
		{
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_ADDDEFENCEDEGREE,_timeout);	
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_INCDEFENCE2,_timeout);
		if(_ratio!=nf->GetRatio())
		{
			_parent.ImpairScaleDefense(_ratio);
			_ratio = nf->GetRatio();
...
public:
	...
	void Merge(filter *f)
	{
		...
		int tmp = _damage * _timeout;
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_LEAFDANCE,_timeout);	
		_damage = nf->GetDamage() + tmp/_timeout;
	}
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_INCHP2,_timeout);	
		if(_ratio!=nf->GetRatio())
		{
			_parent.ImpairScaleMaxHP(_ratio);
			_ratio = nf->GetRatio();
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_INCATTACK2,_timeout);
		if(_ratio!=nf->GetRatio())
		{
			_parent.ImpairScaleDamage(_ratio);
			_ratio = nf->GetRatio();
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_INCMAGIC2,_timeout);	
		if(_ratio!=nf->GetRatio())
		{
			_parent.ImpairScaleMagicDamage(_ratio);
			_ratio = nf->GetRatio();
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_SPEEDUP2,_timeout);	
		if(_ratio!=nf->GetRatio())
		{
			_parent.ImpairSpeed(_ratio);
			_ratio = nf->GetRatio();
...
public:
    ...
    void Merge(filter *f)
	{
		...
		{
			_timeout = GetTimeOut(nf);
			_parent.ModifyTeamVisibleState(HSTATE_INCHPSTEAL,_timeout);	
		}
...
public:
	...
	void Merge(filter* f)
	{
		...
		_parent.ModifyTeamVisibleState(HSTATE_SOULRETORT,_timeout);	
	}
...
public:
	...
	void Merge(filter* f)
	{
		...
		_parent.ModifyTeamVisibleState(HSTATE_SOULSEALED,_timeout);
	}
...
public:
	...
	void Merge(filter* f)
	{
		...
		_parent.ModifyTeamVisibleState(HSTATE_SOULBEATBACK,_timeout);
	}
...
public:
	...
	void Merge(filter* f)
	{
		...
		_parent.ModifyTeamVisibleState(HSTATE_SOULSTUN,_timeout);	
	}
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_FASTMPGEN2,_timeout);
		if(_point!=nf->GetPoint())
		{
			_parent.ImpairMPGen(_point);
			_point = nf->GetPoint();
...
public:
	...
	void Merge(filter* f)
	{
		...
		_parent.ModifyTeamVisibleState(HSTATE_SOULRETORT2,_timeout);
	}
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = GetTimeOut(nf);
		_parent.ModifyTeamVisibleState(HSTATE_INCATTACKRANGE,_timeout);	
...
public:
	...
	void Merge(filter *f)
	{
		...
		_parent.ModifyTeamVisibleState(HSTATE_INCHURT3,_timeout);
	}
};
...
public:
    ...
    void Merge(filter *f) // 接受新加入条件 for 仙魔技能
	{
		...
		_parent.ModifyTeamVisibleState(HSTATE_INCSPECSKILLDAMAGE,_timeout);	
		_count = nf->_count;
	}
...
public:
	...
	void Merge(filter *f)
	{
		...
		_timeout = nf->_timeout;
		_parent.ModifyTeamVisibleState(HSTATE_INCFLYSPEED,_timeout);	
	}
};
...
public:
    ...
    void Merge(filter *f) // 接受新加入条件 for 仙魔技能
	{
		...
		MergeCond(nf->_conds);	
		_timeout = nf->_timeout;
		_parent.ModifyTeamVisibleState(HSTATE_APPENDENCHANT,_timeout);
		_count = nf->_count;
	}
	void OnAttach()
	{
...
public:
    ...
    void Merge(filter *f) // 接受新加入条件 for 仙魔技能
	{
		...
		MergeCond(nf->_conds);	
		_timeout = nf->_timeout;
		_parent.ModifyTeamVisibleState(HSTATE_COOLDOWNAWARD,_timeout);
		_count = nf->_count;
	}
	void TranslateSendAttack(const XID & target, attack_msg & msg)
	{
...
public:
	...
	void Merge(filter *f)
	{
		...
		if(nf->_timeout && _stacks < MAX_STACK)
		{
			++_stacks;
			_timeout = GetTimeOut(nf);
...
class filter_solo_Addattackdegree : public timeout_filter
{
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_MERGE | FILTER_MASK_HEARTBEAT | FILTER_MASK_NOSAVE | FILTER_MASK_BUFF,
		MAX_STACK = 20
	};
	...
	filter_solo_Addattackdegree(){}
public:
	DECLARE_SUBSTANCE(filter_solo_Addattackdegree); 
	filter_solo_Addattackdegree(object_interface  object,int r,int period)
				        :timeout_filter(object,period,FILTER_MASK),_add(r),_stacks(1)
	{
	...
	void Merge(filter *f)
	{
		assert(dynamic_cast<filter_solo_Addattackdegree*>(f));
		filter_solo_Addattackdegree* nf = (filter_solo_Addattackdegree*) f;
		...
		if(nf->_timeout && _stacks < MAX_STACK)
		{
			...
			_add += nf->GetValue();
			_parent.EnhanceAttackDegree(_add);
			_parent.ModifyTeamVisibleState(HSTATE_SOLO_ADDATTACKDEGREE, _stacks);
			_parent.SendClientAttackData();
			_parent.SetSoloChallengeFilterData(_filter_id, _stacks);
		}
	}
	...
	void OnAttach()
	{
		_parent.InsertTeamVisibleState(HSTATE_SOLO_ADDATTACKDEGREE, _stacks);
		_parent.EnhanceAttackDegree(_add);
		_parent.SendClientAttackData();
		_parent.SetSoloChallengeFilterData(_filter_id, _stacks);
	}
	...
	void OnRelease()
	{
		_parent.RemoveTeamVisibleState(HSTATE_SOLO_ADDATTACKDEGREE);
		_parent.ImpairAttackDegree(_add);
		_parent.SendClientAttackData();
		_parent.SetSoloChallengeFilterData(_filter_id, 0);
	}
...
class filter_solo_Adddefencedegree : public timeout_filter
{
protected:
	enum 
	{
	...
	filter_solo_Adddefencedegree(){}
public:
	DECLARE_SUBSTANCE(filter_solo_Adddefencedegree); 
	filter_solo_Adddefencedegree(object_interface  object,int r,int period)
				        :timeout_filter(object,period,FILTER_MASK),_value(r),_stacks(1)
	{
		_filter_id = FILTER_SOLO_ADDDEFENSEDEGREE;
	}
	
	void Merge(filter *f)
	{
		assert(dynamic_cast<filter_solo_Adddefencedegree*>(f));
		filter_solo_Adddefencedegree* nf = (filter_solo_Adddefencedegree*) f;
		if(nf->_timeout && _stacks < MAX_STACK)
		{
			_stacks++;
			_timeout = GetTimeOut(nf);
			...
			_value += nf->GetValue();
			_parent.EnhanceDefendDegree(_value);
			_parent.ModifyTeamVisibleState(HSTATE_SOLO_ADDDEFENSEDEGREE, _stacks);
			_parent.SendClientAttackData();
			_parent.SetSoloChallengeFilterData(_filter_id, _stacks);
		}
	}
	...
	void OnAttach()
	{
		_parent.InsertTeamVisibleState(HSTATE_SOLO_ADDDEFENSEDEGREE, _stacks);
		_parent.EnhanceDefendDegree(_value);
		_parent.SendClientAttackData();
		_parent.SetSoloChallengeFilterData(_filter_id, _stacks);
	}
	...
	void OnRelease()
	{
		_parent.RemoveTeamVisibleState(HSTATE_SOLO_ADDDEFENSEDEGREE);
		_parent.ImpairDefendDegree(_value);
		_parent.SendClientAttackData();
		_parent.SetSoloChallengeFilterData(_filter_id, 0);
	}
...
class filter_Inchurt6 : public timeout_filter
{
protected:
	enum 
	{
	...
	filter_Inchurt6(){}
public:
	DECLARE_SUBSTANCE(filter_Inchurt6); 
	filter_Inchurt6(object_interface  object,float ratio, int period)
				        :timeout_filter(object,period,FILTER_MASK)
	{
		_filter_id = FILTER_INCHURT6;
		_ratio = 1.0+ratio;
	}
...
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_REMOVE_ON_DEATH | FILTER_MASK_HEARTBEAT | FILTER_MASK_UNIQUE | FILTER_MASK_BUFF,
	};
...
public:
	...
	filter_Incantidefensedegree(object_interface  object,int r,int period, int mask=0)
				        :timeout_filter(object,period,FILTER_MASK|mask),_value(r)
	{
...
class filter_Incantiresistancedegree : public timeout_filter
{
protected:
	enum 
	{
	...
	filter_Incantiresistancedegree(){}
public:
	DECLARE_SUBSTANCE(filter_Incantiresistancedegree); 
	filter_Incantiresistancedegree(object_interface  object,int r,int period, int mask)
				        :timeout_filter(object,period,FILTER_MASK|mask),_value(r)
	{
		_filter_id = FILTER_INCANTIRESISTANCEDEGREE;
	}

	void OnAttach()
	{
		_parent.InsertTeamVisibleState(HSTATE_INCANTIRESISTANCEDEGREE, _timeout);
		_parent.IncAntiResistanceDegree(_value);
	}

	void OnRelease()
	{
		_parent.RemoveTeamVisibleState(HSTATE_INCANTIRESISTANCEDEGREE);
		_parent.DecAntiResistanceDegree(_value);
	}
...
class filter_MnfactionDecresist : public filter
{
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_ADJUST_DAMAGE | FILTER_MASK_UNIQUE | FILTER_MASK_DEBUFF 
					| FILTER_MASK_NOSAVE
	};

	float _ratio;

	filter_MnfactionDecresist(){}
public:
	DECLARE_SUBSTANCE(filter_MnfactionDecresist); 
	filter_MnfactionDecresist(object_interface  object,float r)
				        :filter(object,FILTER_MASK),_ratio(r)
	{
		_filter_id = FILTER_MNFACTION_DECRESIST;
		_ratio = 1.0 + r;
	}
	...
	void OnAttach()
	{
		_parent.InsertTeamVisibleState(HSTATE_MNFACTION_DECREASIST);
	}

	void OnRelease()
	{
		_parent.RemoveTeamVisibleState(HSTATE_MNFACTION_DECREASIST);
	}
};
...
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_HEARTBEAT | FILTER_MASK_MERGE | FILTER_MASK_REMOVE_ON_DEATH
	};
...
public:
	...
	filter_Incattack4(object_interface  object,int period,int ratio, int pile_limit,int filter_mask = 0)
				        :timeout_filter(object,period,FILTER_MASK | filter_mask),_ratio(ratio),_pile_times(1),_pile_limit(pile_limit)
	{
...
public:
	...
	filter_Incmagic3(object_interface  object, int period, int ratio, int pile_limit, int filter_mask=0)
				        :timeout_filter(object,period,FILTER_MASK | filter_mask),_ratio(ratio),_pile_times(1),_pile_limit(pile_limit)
	{
...
class filter_Enhanceskilldamage4 : public timeout_filter//增加技能对npc的伤害
{
protected:
	enum 
	{
...
public:
	...
	filter_Enhanceskilldamage4(object_interface  object,int period,int ratio, int pile_limit, int filter_mask=0)
				        :timeout_filter(object,period,FILTER_MASK|filter_mask),_ratio(ratio),_pile_times(1),_pile_limit(pile_limit)
	{
	...
	void Merge(filter *f)
	{
		...
		_parent.ModifyTeamVisibleState(HSTATE_ENHANCESKILLDAMAGE4, _pile_times);
	}

	void OnAttach()
	{
		_parent.IncVisibleState(VSTATE_ENHANCESKILLDAMAGE4);
		_parent.InsertTeamVisibleState(HSTATE_ENHANCESKILLDAMAGE4, _pile_times);
		_parent.EnhanceSkillDamage(_ratio);
	}

	void OnRelease()
	{
		_parent.DecVisibleState(VSTATE_ENHANCESKILLDAMAGE4);
		_parent.RemoveTeamVisibleState(HSTATE_ENHANCESKILLDAMAGE4);
		_parent.ImpairSkillDamage(_ratio);
	}

	int OnQuery(int index)
	{
		return index == FILTER_QUERY_DAMAGE ?  0 : _pile_times;
	}
...
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_HEARTBEAT | FILTER_MASK_DO_DAMAGE | FILTER_MASK_DEBUFF | FILTER_MASK_UNIQUE
		            | FILTER_MASK_REMOVE_ON_DEATH | FILTER_MASK_TRANSFERABLE_DEBUFF
	};
...
public:
	...
	filter_Slow2(object_interface  object,int period, int ratio, int slow_time, int filter_mask = 0)
				        :timeout_filter(object,period,FILTER_MASK | filter_mask),_ratio(ratio),_slow_time(slow_time)
	{
	...
	void OnAttach()
	{
		_parent.IncVisibleState(VSTATE_SLOW2);
		_parent.InsertTeamVisibleState(HSTATE_SLOW2, _timeout);
		_parent.IncImmuneMask(IMMUNESLOW);
	}

	void OnRelease()
	{
		_parent.DecVisibleState(VSTATE_SLOW2);
		_parent.RemoveTeamVisibleState(HSTATE_SLOW2);
		_parent.DecImmuneMask(IMMUNESLOW);
	}
...
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_ADJUST_DAMAGE | FILTER_MASK_HEARTBEAT | FILTER_MASK_MERGE | FILTER_MASK_REMOVE_ON_DEATH
		            | FILTER_MASK_TRANSFERABLE_BUFF
	};
...
public:
	...
	filter_Dechurt4(object_interface  object,int period, float ratio, int pile_limit, int filter_mask=0)
				        :timeout_filter(object,period,FILTER_MASK|filter_mask),_ratio(ratio),_pile_times(1),_pile_limit(pile_limit)
	{
	...
	void Merge(filter *f)
	{
		...
		_parent.ModifyTeamVisibleState(HSTATE_DECHURT4, _pile_times);
	}

	void AdjustDamage(damage_entry & dmg)
	{
	...
	void OnAttach()
	{
		_parent.InsertTeamVisibleState(HSTATE_DECHURT4, _pile_times);
	}

	void OnRelease()
	{
		_parent.RemoveTeamVisibleState(HSTATE_DECHURT4);
	}
...
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_TRANSLATE_SEND_MSG | FILTER_MASK_HEARTBEAT | FILTER_MASK_UNIQUE
	                | FILTER_MASK_REMOVE_ON_DEATH
	};
	...
	{
		timeout_filter::Save(ar);
		ar << _ratio << _ratio_add << _ratio_max;
		return true;
	}
...
public:
	...
	filter_Accumulateattack(object_interface  object,int period, float ratio_add, float ratio_max, int mask=0)
				        :timeout_filter(object,period, FILTER_MASK| mask)
	{
		_ratio = 0.f; // LODWORD(_ratio) = 0?
	...
	void TranslateSendAttack(const XID & target, attack_msg & msg)
	{
		if (msg.skill_id) 
		{
			_ratio = _ratio + _ratio_add;
			if (_ratio > _ratio_max) _ratio = _ratio_max;
			float cur_ratio = _ratio + 1.0;
			msg.physic_damage *= cur_ratio;
			msg.magic_damage[0] *= cur_ratio;
			msg.magic_damage[1] *= cur_ratio;
			msg.magic_damage[2] *= cur_ratio;
			msg.magic_damage[3] *= cur_ratio;
			msg.magic_damage[4] *= cur_ratio;
		}
	}
...
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_HEARTBEAT | FILTER_MASK_TRANSLATE_RECV_MSG | FILTER_MASK_TRANSLATE_SEND_MSG
...
public:
	...
	filter_Accumulateattack2(object_interface  object,int period, float ratio1, float ratio2, int mask=0)
				        :timeout_filter(object,period, FILTER_MASK| mask)
	{
		_ratio1 = ratio1 + 1.0;
		_ratio2 = ratio2 + 1.0;
	...
	void TranslateSendAttack(const XID & target, attack_msg & msg)
	{
		if (msg.skill_id) 
		{
			msg.physic_damage *= _ratio1;
			msg.magic_damage[0] *= _ratio1;
			msg.magic_damage[1] *= _ratio1;
			msg.magic_damage[2] *= _ratio1;
			msg.magic_damage[3] *= _ratio1;
			msg.magic_damage[4] *= _ratio1;
		}
	}
	...
	void TranslateRecvAttack(const XID & attacker,attack_msg & msg)
	{
		if (msg.skill_id) 
		{
			msg.physic_damage *= _ratio2;
			msg.magic_damage[0] *= _ratio2;
			msg.magic_damage[1] *= _ratio2;
			msg.magic_damage[2] *= _ratio2;
			msg.magic_damage[3] *= _ratio2;
			msg.magic_damage[4] *= _ratio2;
		}
	}
...
public:
	...
	filter_Incplusmaxhp(object_interface  object,int period, float ratio1, float ratio2, int mask=0)
				        :timeout_filter(object,period, FILTER_MASK| mask)
	{
		_ratio1 = ratio1;
		_ratio2 = ratio2;
	...
	void OnAttach()
	{
		...
		_inc_maxhp = _parent.GetExtendProp().max_hp * _ratio1;
		_parent.EnhancePlusMaxHP(_inc_maxhp);
		float healValue = _ratio2 * (float)_parent.GetExtendProp().max_hp;
		_parent.Heal(healValue);
	}

	void OnRelease()
	{
		_parent.ImpairPlusMaxHP(_inc_maxhp, true);
...
public:
	...
	filter_Auracurse4asn(object_interface object, int period, float range, int skill, int level, int step = 3)
			:filter_Aurabase(object, period, FILTER_MASK, step),_range(range),_skill(skill),_level(level)
	{
...
public:
	...
	filter_Incmagic4(object_interface  object, int period, float ratio, int filter_mask=0)
				        :timeout_filter(object,period,FILTER_MASK | filter_mask),_ratio(ratio)
	{
		_filter_id = FILTER_INCMAGIC3;
	}
	...
	void OnAttach()
	{
		...
		_add_magic = _parent.GetSoulPower() * _ratio;
...
public:
	...
	filter_Decsoul(object_interface  object, int period, float ratio, int filter_mask=0)
				        :timeout_filter(object,period,FILTER_MASK | filter_mask),_ratio(ratio),_dec_soul(0)
	{
	...
	void OnAttach()
	{
		...
		_dec_soul = _parent.GetSoulPower() * _ratio;
...
protected:
	...
	{
		timeout_filter::Save(ar);
		ar << _body_ratio << _speed_ratio << _attack_ratio << _free_move;
		return true;
	}
...
public:
	...
	filter_Giantform(object_interface  object, int period, float body_ratio, int speed_ratio, int attack_ratio, int free_move)
				        :timeout_filter(object,period,FILTER_MASK),
						_body_ratio(body_ratio),_speed_ratio(speed_ratio),
						_attack_ratio(attack_ratio),_free_move(free_move)
	{
...
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_HEARTBEAT | FILTER_MASK_UNIQUE | FILTER_MASK_REMOVE_ON_DEATH | FILTER_MASK_TRANSLATE_RECV_MSG
	};
...
public:
	...
	void TranslateRecvAttack(const XID & attacker,attack_msg & msg)
	{
		if (_parent.GetPetID() != _petid) {
			_is_deleted = true;
		}
		else {
			A3DVECTOR pos;
			int hp, mp2;
			if (_petid.IsValid() && _parent.QueryObject(_petid, pos, hp, mp2))
			{
				attack_msg pet_msg = msg;
				msg.physic_damage *= (int)(1 - _ratio);
				msg.magic_damage[0] *= (int)(1 - _ratio);
				msg.magic_damage[1] *= (int)(1 - _ratio);
				msg.magic_damage[2] *= (int)(1 - _ratio);
				msg.magic_damage[3] *= (int)(1 - _ratio);
				msg.magic_damage[4] *= (int)(1 - _ratio);
				pet_msg.physic_damage *= (int)_ratio;
				pet_msg.magic_damage[0] *= (int)_ratio;
				pet_msg.magic_damage[1] *= (int)_ratio;
				pet_msg.magic_damage[2] *= (int)_ratio;
				pet_msg.magic_damage[3] *= (int)_ratio;
				pet_msg.magic_damage[4] *= (int)_ratio;
				pet_msg.attack_range = 100.f;
...
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_ADJUST_DAMAGE | FILTER_MASK_HEARTBEAT | FILTER_MASK_BEFORE_DEATH
		            | FILTER_MASK_UNIQUE | FILTER_MASK_REMOVE_ON_DEATH
	};
	...
	{
		timeout_filter::Save(ar);
		ar << _ratio << _damagemax << _range << _hurt;
		return true;
	}
...
public:
	...
	filter_Loaddmg4master(object_interface  object, int period, float ratio, int damagemax, int range, int mask)
				        :timeout_filter(object,period,FILTER_MASK | mask),_ratio(ratio),_damagemax(damagemax),_range(range),_hurt(0)
	{
	...
	virtual void BeforeDeath(const XID & attacker, char attacker_mode)
	{
		...
		if (damage > _damagemax) damage = _damagemax;
		damage /= 2;
		attack_msg msg;
		memset(&msg, 0, sizeof(msg));
		msg.attack_range = (float)_range;
		...
		_parent.SetAuraAttackState();
		_parent.EnterCombatState();
		_parent.RegionAttack1(_parent.GetPos(), (float)_range, msg, 0, XID(-1,-1));
	}
};
...
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_HEARTBEAT | FILTER_MASK_TRANSLATE_RECV_MSG | FILTER_MASK_UNIQUE 
		            | FILTER_MASK_REMOVE_ON_DEATH
	};
...
public:
	...
	filter_Decregiondmg(object_interface  object, int period, float ratio, int mask=0)
				        :timeout_filter(object,period,FILTER_MASK | mask)
	{
		_ratio = (ratio <= 1) ? 1 - ratio : 0.1;
...
public:
	...
	filter_Decregiondmg2(object_interface  object, int period, float ratio, int mask=0)
				        :timeout_filter(object,period,FILTER_MASK | mask)
	{
		_ratio = (ratio <= 1) ? 1 - ratio : 0.1;
...
class filter_Decfardmg : public timeout_filter
{
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_HEARTBEAT | FILTER_MASK_ADJUST_DAMAGE | FILTER_MASK_UNIQUE | FILTER_MASK_REMOVE_ON_DEATH
	};
...
public:
	...
	filter_Decfardmg(object_interface  object, int period, float ratio1, float ratio2, int mask=0)
				        :timeout_filter(object,period,FILTER_MASK | mask)
	{
		_ratio1 = (ratio1 <= 1) ? 1 - ratio1 : 0.1;
		_ratio2 = (ratio2 <= 1) ? 1 - ratio2 : 0.1;
		_filter_id = FILTER_DECFARDMG;
	}

	void AdjustDamage(damage_entry & dmg,const XID & attacker, const attack_msg & msg, float damage_adjust)
	{
		if (dmg.distance <= 8) return;
		float ratio = (msg.skill_id) ? _ratio2 : _ratio1;
...
public:
	...
	filter_Decphysicalmagicaldefense(object_interface  object, int period, int ratio1, int ratio2, int mask=0)
				        :timeout_filter(object,period,FILTER_MASK | mask),_ratio1(ratio1),_ratio2(ratio2)
	{
		_filter_id = FILTER_DECPHYMAGDEFENSE;
	}
	...
	void OnAttach()
	{
		_parent.InsertTeamVisibleState(HSTATE_DECPHYMAGDEFENSE, _timeout);
	...
	void OnRelease()
	{
		_parent.RemoveTeamVisibleState(HSTATE_DECPHYMAGDEFENSE);
...
public:
	...
	filter_Decphysicalmagicaldefense2(object_interface  object, int period, int ratio1, int ratio2, int mask=0)
				        :timeout_filter(object,period,FILTER_MASK | mask),_ratio1(ratio1),_ratio2(ratio2)
	{
		_filter_id = FILTER_DECPHYMAGDEFENSE2;
	}
	...
	void OnAttach()
	{
		_parent.InsertTeamVisibleState(HSTATE_DECPHYMAGDEFENSE2, _timeout);
		_parent.ImpairScaleDefense(_ratio1);
		_parent.ImpairAllScaleResistance(_ratio2);
		_parent.UpdateDefenseData();
		_parent.UpdateMagicData();
	...
	void OnRelease()
	{
		_parent.RemoveTeamVisibleState(HSTATE_DECPHYMAGDEFENSE2);
		_parent.EnhanceScaleDefense(_ratio1);
		_parent.EnhanceAllScaleResistance(_ratio2);
		_parent.UpdateDefenseData();
		_parent.UpdateMagicData();
...
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_HEARTBEAT | FILTER_MASK_ADJUST_HEAL | FILTER_MASK_UNIQUE | FILTER_MASK_REMOVE_ON_DEATH
	};
...
public:
	...
	filter_Decheal(object_interface  object, int period, float ratio, int mask=0)
				        :timeout_filter(object,period,FILTER_MASK | mask)
	{
		_ratio = (ratio <= 1) ? 1.0 - ratio : 0.0;
...
public:
	...
	void TranslateSendAttack(const XID & target, attack_msg & msg)
	{
		if(msg.skill_id && (rand()%100)< _probability)
		{
			msg.physic_damage = (int)(msg.physic_damage*_ratio);
			msg.magic_damage[0] = (int)(msg.magic_damage[0]*_ratio);
			msg.magic_damage[1] = (int)(msg.magic_damage[1]*_ratio);
...
public:
	...
	filter_Fastpray3(object_interface  object,int period, int ratio, int mask=0)
				        :timeout_filter(object,period,FILTER_MASK | mask),_ratio(ratio)
	{
...
public:
	...
	filter_Incdebuffdodge2(object_interface  object,int period, int ratio, int mask=0)
				        :timeout_filter(object,period,FILTER_MASK | mask),_ratio(ratio)
	{
...
public:
	...
	filter_Incdamagedodge2(object_interface  object,int period, int ratio, int mask=0)
				        :timeout_filter(object,period,FILTER_MASK | mask),_ratio(ratio)
	{
...
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_ADJUST_DAMAGE | FILTER_MASK_HEARTBEAT | FILTER_MASK_WEAK | FILTER_MASK_REMOVE_ON_DEATH
	};
	...
	{
		timeout_filter::Save(ar);
		ar << _repay_period << _state << _total_hurt << _per_repay;
		return true;
	}
...
public:
	...
	void OnAttach()
	{
		...
		_parent.InsertTeamVisibleState(_state ? HSTATE_DEBITHURT_MODE_B : HSTATE_DEBITHURT_MODE_A,  _timeout);
	}

	void OnRelease()
	{
		_parent.DecVisibleState(VSTATE_DEBITHURT);
		_parent.RemoveTeamVisibleState(_state ? HSTATE_DEBITHURT_MODE_B : HSTATE_DEBITHURT_MODE_A);
	}
	...
	virtual void Heartbeat(int tick)
	{
		...
		{
			_parent.RemoveTeamVisibleState(HSTATE_DEBITHURT_MODE_A);
			_parent.InsertTeamVisibleState(HSTATE_DEBITHURT_MODE_B, _timeout);
			_state = 1;
			if (_timeout > 0) _total_hurt /= _timeout;
			_per_repay = _total_hurt;
			_total_hurt = 0;
	...
	void AdjustDamage(damage_entry & dmg,const XID & attacker, const attack_msg & msg, float damage_adjust)
	{
		if (!_state) 
		{
			_total_hurt += (int)((dmg.physic_damage + 
							     dmg.magic_damage[0] +
								 dmg.magic_damage[1] +
								 dmg.magic_damage[2] +
								 dmg.magic_damage[3] +
								 dmg.magic_damage[4]) * damage_adjust + 0.5);
...
public:
	...
	filter_Healnurse(object_interface  object,int period, float ratio1, float ratio2, int mask=0)
				        :timeout_filter(object,period,FILTER_MASK | mask),_ratio1(ratio1),_ratio2(ratio2)
	{
	...
	void DoBless()
	{
		if ((float)_parent.GetExtendProp().max_hp * _ratio1 > (float) _parent.GetBasicProp().hp)
		{
			_parent.Heal((int)((float)_parent.GetExtendProp().max_hp * _ratio2));
			_parent.ClearSpecFilter(filter::FILTER_MASK_DEBUFF);
			_is_deleted = true;
		}
	}
...
class filter_Incfightproperty : public timeout_filter
{
protected:
	enum 
	{
	...
	{
		timeout_filter::Save(ar);
		ar << _atk_ratio << _def_ratio << _maxhp_ratio;
		return true;
	}
...
public:
	...
	filter_Incfightproperty(object_interface  object,int period, int atk_ratio, int def_ratio, int maxhp_ratio, int mask=0)
				        :timeout_filter(object,period,FILTER_MASK | mask),_atk_ratio(atk_ratio),_def_ratio(def_ratio),_maxhp_ratio(maxhp_ratio)
	{
		_filter_id = FILTER_INCFIGHTPROP;
	}

	void OnAttach()
	{
		_parent.IncVisibleState(VSTATE_INCFIGHTPROP);
		_parent.InsertTeamVisibleState(HSTATE_INCFIGHTPROP, _timeout);
		_parent.EnhanceScaleDamage(_atk_ratio);
		_parent.EnhanceScaleMagicDamage(_atk_ratio);
		_parent.EnhanceScaleDefense(_def_ratio);
		...
		_parent.UpdateMagicData();
		_parent.UpdateDefenseData();
		int incHP = _parent.GetExtendProp().max_hp;
		_parent.EnhanceScaleMaxHP(_maxhp_ratio, true);
		_parent.Heal(_parent.GetExtendProp().max_hp - incHP);
	}

	void OnRelease()
	{
		_parent.IncVisibleState(VSTATE_INCFIGHTPROP);
		_parent.RemoveTeamVisibleState(HSTATE_INCFIGHTPROP);
		_parent.ImpairScaleDamage(_atk_ratio);
		_parent.ImpairScaleMagicDamage(_atk_ratio);
		_parent.ImpairScaleDefense(_def_ratio);
		_parent.ImpairAllScaleResistance(_def_ratio);
		_parent.ImpairScaleMaxHP(_maxhp_ratio, true);
		_parent.UpdateAttackData();
		_parent.UpdateMagicData();
		_parent.UpdateDefenseData();
	}
...
class filter_Sealed3 : public timeout_filter
{
protected:
	enum 
	{
	...
	filter_Sealed3(){}
public:
	DECLARE_SUBSTANCE(filter_Sealed3); 
	filter_Sealed3(object_interface  object,int period)
				        :timeout_filter(object,period,FILTER_MASK)
	{
		_filter_id = FILTER_SEALED3;
	}
...
// Increment physical damage for Asessin 
class filter_Incdamageasn : public timeout_filter
{
protected:
	enum 
	{
		FILTER_MASK = FILTER_MASK_HEARTBEAT | FILTER_MASK_UNIQUE | FILTER_MASK_REMOVE_ON_DEATH | FILTER_MASK_TRANSLATE_SEND_MSG
	};
	...
	filter_Incdamageasn(){}
public:
	DECLARE_SUBSTANCE(filter_Incdamageasn); 
	filter_Incdamageasn(object_interface  object,int period, float _ratio)
				        :timeout_filter(object,period,FILTER_MASK)
	{
		_filter_id = FILTER_INCDAMAGEASN;
	}

	void OnAttach()
	{
		_parent.InsertTeamVisibleState(HSTATE_INCDAMAGEASN, _timeout);
	}

	void OnRelease()
	{
		_parent.RemoveTeamVisibleState(HSTATE_INCDAMAGEASN);
	}

	void TranslateSendAttack(const XID& target, attack_msg& msg)
    {
        if (IS_PHYSIC_ATTACK(msg.attack_attr) && !_parent.IsInvisible())
        {
            msg.physic_damage = (int)(msg.physic_damage * _ratio);
        }
    }
