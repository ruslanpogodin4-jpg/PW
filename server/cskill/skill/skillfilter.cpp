...
DEFINE_SUBSTANCE(filter_solo_Addattackdegree, filter, CLASS_SOLO_ADDATTACKDEGREE);
DEFINE_SUBSTANCE(filter_solo_Adddefencedegree, filter, CLASS_SOLO_ADDDEFENCEDEGREE);
...
DEFINE_SUBSTANCE(filter_Inchurt6, filter, CLASS_INCHURT6);
DEFINE_SUBSTANCE(filter_Incantidefensedegree, filter, CLASS_INCANTIDEFENSEDEGREE);
DEFINE_SUBSTANCE(filter_Incantiresistancedegree, filter, CLASS_INCANTIRESISTANCEDEGREE);
DEFINE_SUBSTANCE(filter_MnfactionDecresist, filter, CLASS_MNFACTION_DECRESIST);
DEFINE_SUBSTANCE(filter_Incattack4, filter, CLASS_INCATTACK4);
DEFINE_SUBSTANCE(filter_Incmagic3, filter, CLASS_INCMAGIC3);
DEFINE_SUBSTANCE(filter_Enhanceskilldamage4, filter, CLASS_ENHANCESKILLDAMAGE4);
DEFINE_SUBSTANCE(filter_Slow2, filter, CLASS_SLOW2);
DEFINE_SUBSTANCE(filter_Dechurt4, filter, CLASS_DECHURT4);
DEFINE_SUBSTANCE(filter_Accumulateattack, filter, CLASS_ACCUMULATEATTACK);
DEFINE_SUBSTANCE(filter_Accumulateattack2, filter, CLASS_ACCUMULATEATTACK2);
DEFINE_SUBSTANCE(filter_Incplusmaxhp, filter, CLASS_INCPLUSMAXHP);
DEFINE_SUBSTANCE(filter_Auracurse4asn, filter, CLASS_AURACURSE4ASN);
DEFINE_SUBSTANCE(filter_Incmagic4, filter, CLASS_INCMAGIC4);
DEFINE_SUBSTANCE(filter_Decsoul, filter, CLASS_DECSOUL);
DEFINE_SUBSTANCE(filter_Giantform, filter, CLASS_GIANTFORM);
DEFINE_SUBSTANCE(filter_Transdmg2pet, filter, CLASS_TRANSDMG2PET);
DEFINE_SUBSTANCE(filter_Loaddmg4master, filter, CLASS_LOADDMG4MASTER);
DEFINE_SUBSTANCE(filter_Decregiondmg, filter, CLASS_DECREGIONDMG);
DEFINE_SUBSTANCE(filter_Decregiondmg2, filter, CLASS_DECREGIONDMG2);
DEFINE_SUBSTANCE(filter_Decfardmg, filter, CLASS_DECFARDMG);
DEFINE_SUBSTANCE(filter_Decphysicalmagicaldefense, filter, CLASS_DECPHYMAGDEFENSE);
DEFINE_SUBSTANCE(filter_Decphysicalmagicaldefense2, filter, CLASS_DECPHYMAGDEFENSE2);
...
DEFINE_SUBSTANCE(filter_Incfightproperty, filter, CLASS_INCFIGHTPROP);
DEFINE_SUBSTANCE(filter_Sealed3, filter, CLASS_SEALED3);
DEFINE_SUBSTANCE(filter_Incdamageasn, filter, CLASS_INCDAMAGEASN);
...
{
	// Prevent translate attack when coming from elf (genie)
	if (msg.skill_id > 0) {
		SkillKeeper attack_skill = Skill::Create(msg.skill_id);
		if (attack_skill && attack_skill->IsElfSkill()) {
			return;
		}
	}
...
void filter_Activatereboundskill::AdjustDamage(damage_entry & dmg,const XID & attacker, const attack_msg & msg, float damage_adjust)
{
		...
		if (!_period || _period + _lasttime <= now) {
			...
			if (!_parent.TestCoolDown(_parent.GetSkillWrapper().GetAsid()+COOLINGID_BEGIN))
				return;
...
void filter_Activateskill2::TranslateSendAttack(const XID & target, attack_msg & msg)
	{
		...
		int sid = (_parent.GetSkillWrapper()).GetAsid2();
		int level = (_parent.GetSkillWrapper()).GetAslevel2();
		if (sid && !(msg._attack_state & AT_STATE_AURA_AND_RETORT))
		{
			...
			if(skill->GetRange().IsSelf())
			{
				...
				skill->SetLevel(level);
				skill->StateAttack();
				if (w_player.GetProbability() > 99.0) {
					_parent.SendClientEnchantResult(_parent.GetSelfID(), sid, level, false, 0, msg.section);
				}
			}	
			else 
			{
				...
				if (skill->DoBless()) {
					...
					skill->SetLevel(level);
					skill->BlessMe();
					send = w_player.GetProbability() > 99.0;
				}
				if (send && skill->DoEnchant())
				{
