...
namespace GNET
{
	...
	int PlayerWrapper::GetPetType()
	{
		return object.GetPetType();
	}
	unsigned int PlayerWrapper::GetSkillId()
	{
		return (skill) ? skill->GetId() : 0;
	}
	...
	bool PlayerWrapper::SetSkillcooldown2(bool)
	{
		if (ThrowDice())
		{
			object.ResetAllCoolDown((time < 1) ? 1 : time, GetSkillId() + COOLDOWN_SKILL_START);
		}
		return true;
	}
	...
	bool PlayerWrapper::SetSoloAddAttackDegree(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_solo_Addattackdegree(object, GetAmountInt(), GetTime()));
		}
		return true;
	}
	bool PlayerWrapper::SetSoloAddDefenseDegree(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_solo_Adddefencedegree(object, GetAmountInt(), GetTime()));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetInchurt6(bool)
	{
		if (ThrowDice() && ratio > 0)
		{
			if (object.GetImmuneMask() & (IMMUNEWEAK | IMMUNEALL))
			...
			{
				object.AddFilter(new filter_Inchurt6(object, ratio, time));
			}
		}
		return true;
	}
	bool PlayerWrapper::SetIncantidefensedegree(bool)
	{
		if (ThrowDice())
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Incantidefensedegree(object, GetValueInt(), time, buff_mask));
		}
		return true;
	}
	bool PlayerWrapper::SetIncantiresistancedegree(bool)
	{
		if (ThrowDice())
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Incantiresistancedegree(object, GetValueInt(), time, buff_mask));
		}
		return true;
	}
	bool PlayerWrapper::SetMnfactionDecresist(bool)
	{
		...
		{
			object.AddFilter(new filter_MnfactionDecresist(object, ratio));
		}
	...
	bool PlayerWrapper::SetIncattack3(bool)
	{
		...
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Incattack4(object, time, ratio * 100, GetValueInt(), buff_mask));
		}
		return true;
	}
	bool PlayerWrapper::SetIncmagic3(bool)
	{
		...
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Incmagic3(object, time, ratio * 100, GetValueInt(), buff_mask));
		}
		return true;
	}
	bool PlayerWrapper::SetEnhanceskilldamage4(bool)
	{
		if (ThrowDice())
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Enhanceskilldamage4(object, time, ratio * 100, GetValueInt(), buff_mask));
		}
		return true;
	}
	bool PlayerWrapper::SetSlow2(bool)
	{
		...
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Slow2(object, time, ratio * 100, GetValueInt(), buff_mask));
		}
		return true;
	}
	bool PlayerWrapper::SetDechurt4(bool)
	{
		if (ThrowDice())
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Dechurt4(object, time, ratio, GetValueInt(), buff_mask));
		}
		return true;
	}
	bool PlayerWrapper::SetAccumulateattack(bool)
	{
		if (ThrowDice())
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Accumulateattack(object, time, ratio, value, buff_mask));
		}
		return true;
	}
	bool PlayerWrapper::SetAccumulateattack2(bool)
	{
		if (ThrowDice())
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Accumulateattack2(object, time, ratio, value, buff_mask));
		}
		return true;
	}
	bool PlayerWrapper::SetIncplusmaxhp(bool)
	{
		...
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Incplusmaxhp(object, time, ratio, value, buff_mask));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetIncmagic4(bool)
	{
		...
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Incmagic4(object, time, ratio, buff_mask));
		}
		return true;
	}
	bool PlayerWrapper::SetDecsoul(bool)
	{
		...
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Decsoul(object, time, ratio, buff_mask));
		}
		return true;
	}
	bool PlayerWrapper::SetGiantForm(bool)
	{
		object.AddFilter(new filter_Giantform(object, time, ratio, (int)(value * 100.f), (int)(amount * 100.f), GetProbabilityInt()));
		return true;
	}
	bool PlayerWrapper::SetTransdmg2pet(bool)
	{
		if (ThrowDice())
		{
			...
			A3DVECTOR pos;
			int hp, mp2;
			if (petid.IsValid() && object.QueryObject(petid, pos, hp, mp2))
			{
				int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
				object.AddFilter(new filter_Transdmg2pet(object, time, ratio, buff_mask, petid));
				if (GetValueInt())
				{
					enchant_msg msg;
					memset(&msg, 0, sizeof(msg));
					msg.skill = GetValueInt();
					msg.skill_level = (skill) ? skill->GetLevel() : 1;
					msg.helpful = 2;
					msg.attack_range = 100.f;
					SetTalentData(msg.skill_modify);
	...
	bool PlayerWrapper::SetLoaddmg4master(bool)
	{
		if (ThrowDice())
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Loaddmg4master(object, time, ratio, GetValueInt(), GetProbabilityInt(), buff_mask));
		}
		return true;
	}

	bool PlayerWrapper::SetDecregiondmg(bool)
	{
		if (ThrowDice())
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Decregiondmg(object, time, ratio, buff_mask));
		}
		return true;
	}

	bool PlayerWrapper::SetDecregiondmg2(bool)
	{
		if (ThrowDice())
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Decregiondmg2(object, time, ratio, buff_mask));
		}
		return true;
	}

	bool PlayerWrapper::SetDecfardmg(bool)
	{
		if (ThrowDice())
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Decfardmg(object, time, ratio, value, buff_mask));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetDecphysicalmagicaldefense(bool)
	{
		...
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Decphysicalmagicaldefense(object, time, (int)(ratio * 100.f), (int)(value * 100.f), buff_mask));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetDecphysicalmagicaldefense2(bool)
	{
		...
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Decphysicalmagicaldefense2(object, time, (int)(ratio * 100.f), (int)(value * 100.f), buff_mask));
		}
		return true;
	}

	bool PlayerWrapper::SetDecheal(bool)
	{
		if (ThrowDice())
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Decheal(object, time, ratio, buff_mask));
		}
		return true;
	}

	bool PlayerWrapper::SetMagicfrenetic(bool)
	{
		if (enable)
		{
	...
	bool PlayerWrapper::SetIncsmitedamage(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_Incsmitedamage(object, time, (int)(ratio * 100.f), (int)(value * 100.f)));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetFastpray3(bool)
	{
		...
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Fastpray3(object, time, (int)(ratio * 100.f), buff_mask));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetIncdebuffdodge2(bool)
	{
		...
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Incdebuffdodge2(object, time, (int)(ratio * 100.f), buff_mask));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetIncdamagedodge2(bool)
	{
		...
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Incdamagedodge2(object, time, (int)(ratio * 100.f), buff_mask));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetHealnurse(bool)
	{
		...
		{
			int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Healnurse(object, time, ratio, value, buff_mask));
		}
		return true;
	}

	bool PlayerWrapper::SetIncfightproperty(bool)
	{
		int buff_mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
		object.AddFilter(new filter_Incfightproperty(object, time, (int)(ratio * 100.f), (int)(value * 100.f), (int)(probability * 100.f), buff_mask));
		return true;
	}

	bool PlayerWrapper::SetSealed3(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_Sealed3(object, time));
		}
		return true;
	}

	bool PlayerWrapper::SetIncdamageasn(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_Incdamageasn(object, time, ratio));
		}
		return true;
	}

	bool PlayerWrapper::SetIncdefensedegree(int inc)
	{
		if (enable)
		{
			object.EnhanceDefendDegree(inc);
			object.SendClientAttackData();
		}
		else
			object.ImpairDefendDegree(inc);
		return true;
	}

	bool PlayerWrapper::SetDecdamagefromcrits(int inc)
	{
		if (enable)
			object.EnhanceCritDamageReduce(inc);
		else
			object.ImpairCritDamageReduce(inc);
		return true;
	}
