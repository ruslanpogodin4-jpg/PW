...
int SkillStub::Condition(Skill *skill) const
{
	...
	if (itemcost > 0 && itemcostcount > 0 && skill->GetSection() <= 1 && !skill->GetPlayer()->SetCheckitem(itemcost, itemcostcount))
		return 10;
...
int SkillStub::ElfCondition(Skill *skill) const
{
	...
	if(itemcost > 0 && !skill->GetPlayer()->SetCheckitem(itemcost, 1))	
		return 10;
...
bool Skill::CanAttack(bool cost)
{
	...
	if (cost && stub->itemcost > 0 && stub->itemcostcount > 0 && GetSection() <= 1 && !player->SetUseitem(stub->itemcost))
		return false;

	if (cost && stub->arrowcost > 0 && !player->UseArrow(stub->arrowcost))
		return false;
	return true;
}
