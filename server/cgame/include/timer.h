...
public:
	...
	int SetTimer(timer & tm, int interval,int times,int start_time = -1)
	{
		...
		_tm = & tm;
		return _timer_index = tm.set_timer(interval,start_time,times,_callback,this);
	}

	int RemoveTimer()
	{
		int timer_index =_timer_index;
		timer * tm = _tm;
		if(!tm || timer_index < 0) return -1;
		int tmp = tm->remove_timer(timer_index,this);
		_timer_index = -1;
		_tm = NULL;
		return tmp;
	}
