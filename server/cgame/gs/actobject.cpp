...
int gactive_imp::MessageHandler(world *pPlane, const MSG &msg)
{
	...
	case GM_MSG_SUBSCIBE_ENEMY:
		ASSERT(msg.source.type == GM_TYPE_PLAYER);
		InsertInfoThirdSubscibe(msg.source, *(link_sid *)msg.content);
		break;

	case GM_MSG_UNSUBSCIBE_ENEMY:
		ASSERT(msg.source.type == GM_TYPE_PLAYER);
		RemoveInfoThirdSubscibe(msg.source);
		break;
...
void gactive_imp::DoHeartbeat(size_t tick)
{
	...
	//清空变化的标志
	ClearRefreshState();
	UpdateTeamVisibleStateCountdown();
...
void gactive_imp::RefreshSubscibeList()
{
	...
	if (_subscibe_list.size() || _second_subscibe_list.size() || _third_subscibe_list.size())
	{
		if (GetRefreshState() || _third_subscibe_list.size())
...
void gactive_imp::SendDelayAttackMsg(const XID &target, attack_msg &attack, size_t delay_tick)
{
	MSG msg;
	BuildMessage(msg, GM_MSG_ATTACK, target, _parent->ID, _parent->pos,
				 0, &attack, sizeof(attack));
...
void gactive_imp::SendDelayEnchantMsg(int message, const XID &target, enchant_msg &enchant, size_t delay_tick)
{
	MSG msg;
	BuildMessage(msg, message, target, _parent->ID, _parent->pos,
				 0, &enchant, sizeof(enchant));
...
void gactive_imp::UpdateTeamVisibleStateCountdown()
{
	size_t hstatecount = _visible_team_state.size();
	size_t param_index = _visible_team_state_param.size();
	for (size_t i = 0; i < hstatecount; i++)
	{
		unsigned short stateid = __TVSGetState(_visible_team_state[i]);

		if (!FilterFiltersWithParam((int)stateid))
			if ((__TVSGetParamCnt(stateid) - param_index) >= 1)
				DecreaseTeamVisibleState(stateid, 1);
	}
}
...
void gactive_imp::InsertInfoThirdSubscibe(const XID &target, const link_sid &ld)
{
	ASSERT(target.id == ld.user_id);
	if (target == _parent->ID)
		return; //订阅自己没必要吧
	for (size_t i = 0; i < _third_subscibe_list.size(); i++)
	{
		link_sid &tmp = _third_subscibe_list[i];
		if (ld.user_id == tmp.user_id)
		{
			tmp.cs_id = ld.cs_id;
			tmp.cs_sid = ld.cs_sid;
	...
	_third_subscibe_list.push_back(ld);
}
