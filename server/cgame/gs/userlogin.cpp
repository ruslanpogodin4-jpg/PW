...
//	_player->imp = CF_Create(CLS_PLAYER_IMP,CLS_GM_DISPATCHER,CLS_PLAYER_CONTROLLER,_plane,_player);
	...
	pImp->SetNewStatInfo(
		pInfo->newstats_extern.killed_monsters,
		pInfo->newstats_extern.killed_players,
		pInfo->newstats_extern.death_by_monsters,
		pInfo->newstats_extern.death_by_players);

	pImp->SetArenaStatInfo(
		pInfo->arena_extern.total_victory,
		pInfo->arena_extern.total_defeat,
		pInfo->arena_extern.total_kills,
		pInfo->arena_extern.total_deaths,
		pInfo->arena_extern.total_assistances,
		pInfo->arena_extern.arena_rank,
		pInfo->arena_extern.arena_grade_matchmaking);
	...
	// DRY
	pImp->_task_count = 0;
	FinishedTaskList* pFTL = (FinishedTaskList*)pImp->_finished_task_list.begin();
