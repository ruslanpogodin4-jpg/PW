...
int main(int argn, char **argv)
{
	...
	if (g_mobile_server && strncmp(servername, mobile_prefix, strlen(mobile_prefix)) != 0 || !g_mobile_server && strncmp(servername, mobile_prefix, strlen(mobile_prefix)) == 0)
	{
	...
	if (logic_threads < 4 || logic_threads > 16)
		logic_threads = 4;
	...
	//	InitHookTab();
	if (__PRINT_INFO_FLAG)
		ThreadUsage::Start(60, 0);
