...
int gplayer_imp::GeneralTradeMessageHandler(world *pPlane, const MSG &msg)
{
	switch (msg.message)
	{
...
int gplayer_imp::WaitingTradeMessageHandler(world *pPlane, const MSG &msg)
{
	//只处理部分
	switch (msg.message)
	{
...
int gplayer_imp::WaitingTradeCompleteHandler(world *pPlane, const MSG &msg)
{
	switch (msg.message)
	{
	case GM_MSG_HEARTBEAT:
...
int gplayer_imp::WaitingFactionTradeTradeHandler(world *pPlane, const MSG &msg)
{
	switch (msg.message)
	{
	case GM_MSG_HEARTBEAT:
...
int gplayer_imp::TradeMessageHandler(world *pPlane, const MSG &msg)
{
	//只处理部分
	switch (msg.message)
	{
...
int gplayer_imp::WaitingTradeReadHandler(world *pPlane, const MSG &msg)
{
	//只处理部分
	switch (msg.message)
	{
...
int gplayer_imp::WatingFactionTradeReadHandler(world *pPlane, const MSG &msg)
{
	//只处理部分
	switch (msg.message)
	{
...
bool gplayer_imp::CanTrade(const XID &target)
{
	//现在一律禁止交易
	//	return false;
	if (_cheat_punish)
...
void gplayer_imp::StartTrade(int trade_id, const XID &target)
{
	//战斗状态现在允许交易
	...
	public:
		...
		void CallWriteBack(bool success)
		{
			...
			if (pPlayer->ID.id != _userid || !pPlayer->IsActived() || pPlayer->cs_index != _cs_index || pPlayer->cs_sid != _cs_sid)
			{
				return; // not match
			}
			ASSERT(pPlayer->imp);
...
void gplayer_imp::TradeComplete(int trade_id, int reason, bool need_read)
{
...
bool gplayer_imp::StartFactionTrade(int trade_id, int get_mask, int put_mask, bool no_response)
{
	if (_player_state != PLAYER_STATE_NORMAL || _cur_session)
		return false;
	ASSERT(_trade_obj == NULL);
...
void gplayer_imp::WaitingTradeWriteBack(int trade_id, bool bSuccess)
{
	if (_player_state != PLAYER_WAITING_TRADE)
		return;
	if (_trade_obj->GetTradeID() != trade_id)
...
void gplayer_imp::WaitingTradeReadBack(int trade_id, const GDB::itemlist *pInv, int money)
{
	if (_player_state != PLAYER_WAIT_TRADE_READ)
		return;
	if (_trade_obj->GetTradeID() != trade_id)
		return;
	if (pInv)
...
void gplayer_imp::PutItemAfterTrade(item_list &backpack)
{
	XID self = _parent->ID;
	for (size_t i = 0; i < backpack.Size(); i++)
	{
		if (backpack[i].type == -1)
			break;
		item it;
...
void gplayer_imp::DropAllAfterTrade(item_list &backpack, size_t money)
{
	XID self = _parent->ID;
	...
	if (money)
		DropMoneyItem(_plane, pos, money, self, 0, 0);
}

void gplayer_imp::FromTradeToNormal(int type)
{
	if (type < 0)
	{
		//将物品扔在地上
...
void gplayer_imp::FromFactionTradeToNormal(int type)
{
	if (type)
	{
		//将物品扔在地上
...
void gplayer_imp::FactionTradeTimeout()
{
...
void gplayer_imp::WaitingFactionTradeReadBack(const GDB::itemlist *pInv, int money, int timestamp, int mask)
{
	if (_player_state != PLAYER_WAIT_FACTION_TRADE_READ)
		return;
	if (pInv)
	{
...
void gplayer_imp::FactionTradeComplete(int trade_id, const GNET::syncdata_t &data)
{
...
void gplayer_imp::SyncTradeComplete(int trade_id, size_t money, const GDB::itemlist &item_change, bool writetrashbox, bool cash_change)
{
	if (_player_state != PLAYER_WAITING_FACTION_TRADE)
	{
		//未在合适的状态中
