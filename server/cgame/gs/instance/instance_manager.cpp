...
bool instance_world_manager::InitNetClient(const char *gmconf)
{
...
void instance_world_manager::timer_tick(int index, void *object, int remain)
{
...
int instance_world_manager::Init(const char *gmconf_file, const char *servername)
{
	...
	if (instance_count > MAX_INSTANCE_PER_SVR)
		instance_count = MAX_INSTANCE_PER_SVR;
	...
	if (_pool_threshold_low <= 0 || _pool_threshold_high <= 0 || _pool_threshold_low > _pool_threshold_high)
	{
		__PRINTINFO("世界缓冲池阈值错误\n");
		return -1011;
	}

	if (pool_threadhold_init > _pool_threshold_high)
		pool_threadhold_init = _pool_threshold_high;
	if (pool_threadhold_init <= 0)
		pool_threadhold_init = (_pool_threshold_low + _pool_threshold_high) / 2;
	...
	if (_player_max_count > INS_MAX_PLAYER_COUNT)
		_player_max_count = INS_MAX_PLAYER_COUNT;
	if (_npc_max_count > INS_MAX_NPC_COUNT)
		_npc_max_count = INS_MAX_NPC_COUNT;
	if (_matter_max_count > INS_MAX_MATTER_COUNT)
		_matter_max_count = INS_MAX_MATTER_COUNT;
	...
	if (grid_sight_range < 20.f || grid_sight_range > 100.f)
		grid_sight_range = 100.f;
...
bool instance_world_manager::InitNetIO(const char *servername)
{
...
void instance_world_manager::SendRemoteMessage(int id, const MSG &msg)
{
	//这里需要加上消息过滤
	return _ioman.SendMessage(id, msg);
}

int instance_world_manager::BroadcastSvrMessage(const rect &rt, const MSG &message, float extend_size)
{
	//副本不需要进行服务器广播
	return 0;
}

int instance_world_manager::GetServerNear(const A3DVECTOR &pos) const
{
	//副本不存在Server near
	return -1;
}

int instance_world_manager::GetServerGlobal(const A3DVECTOR &pos) const
{
	return _ioman.GetGlobalServer(pos, _world_tag);
}

void instance_world_manager::RestartProcess()
{
	//考虑让所有人都断线
...
void instance_world_manager::ShutDown()
{
	...
	for (size_t i = 0; i < ins_count; i++)
	{
		...
		if (!pPlane)
			continue;
		if (pPlane->w_ctrl)
			pPlane->w_ctrl->OnServerShutDown();
	}
...
int instance_world_manager::SendRemotePlayerMsg(int uid, const MSG &msg)
{
...
int instance_world_manager::GetOnlineUserNumber()
{
...
void instance_world_manager::GetPlayerCid(player_cid &cid)
{
	cid = _cid;
}

void instance_world_manager::FreeWorld(world *pPlane, int index)
{
	mutex_spinlock(&_key_lock);
	if (_planes_state[index] == 0 || _cur_planes[index] != pPlane || pPlane->w_player_count || pPlane->w_obsolete == 0)
	{
...
void instance_world_manager::RegroupCoolDownWorld()
{
...
void instance_world_manager::FillWorldPool()
{
	mutex_spinlock(&_pool_lock);
...
void instance_world_manager::Heartbeat()
{
...
bool instance_world_manager::CompareInsKey(const instance_key &key, const instance_hash_key &hkey)
{
...
{
	if (_world_limit.can_reenter && (plane->w_life_time < 0 || plane->w_life_time > INSTANCE_REENTER_INTERVAL) && (!plane->w_obsolete || _idle_time - plane->w_obsolete >= INSTANCE_REENTER_INTERVAL))
		return g_timer.get_systime() + INSTANCE_REENTER_INTERVAL;
	else
		return 0;
}

int instance_world_manager::CheckPlayerSwitchRequest(const XID &who, const instance_key *ikey, const A3DVECTOR &pos, int ins_timer)
{
	int rst = 0;
	//首先检查key是否存在
...
void instance_world_manager::HandleSwitchRequest(int link_id, int user_id, int localsid, int source, const instance_key &ikey)
{
	instance_hash_key key;
	TransformInstanceKey(ikey.target, key);
	int world_index;
...
void instance_world_manager::PlayerLeaveThisWorld(int plane_index, int userid)
{
	RemovePlayerWorldIdx(userid, plane_index);
}

void instance_world_manager::PostMessage(world *plane, const MSG &msg)
{
	_msg_queue.AddMsg(plane, msg);
}

void instance_world_manager::PostMessage(world *plane, const MSG &msg, int latancy)
{
	_msg_queue.AddMsg(plane, msg, latancy);
}

void instance_world_manager::PostMessage(world *plane, const XID *first, const XID *last, const MSG &msg)
{
	_msg_queue.AddMultiMsg(plane, first, last, msg);
}

void instance_world_manager::PostPlayerMessage(world *plane, int *player_list, size_t count, const MSG &msg)
{
	_msg_queue.AddPlayerMultiMsg(plane, count, player_list, msg);
}

void instance_world_manager::PostMultiMessage(world *plane, abase::vector<gobject *, abase::fast_alloc<>> &list, const MSG &msg)
{
	_msg_queue.AddMultiMsg(plane, list, msg);
}

void instance_world_manager::GetLogoutPos(gplayer_imp *pImp, int &world_tag, A3DVECTOR &pos)
{
...
void instance_world_manager::UserLogin(int cs_index, int cs_sid, int uid, const void *auth_data, size_t auth_size, bool isshielduser, char flag)
{
	instance_user_login(cs_index, cs_sid, uid, auth_data, auth_size, isshielduser, flag);
}

void instance_world_manager::SetFilterWhenLogin(gplayer_imp *pImp, instance_key *ikey)
{
...
void instance_world_manager::SetIncomingPlayerPos(gplayer *pPlayer, const A3DVECTOR &pos, int special_mask)
{
	if ((0 == (special_mask & IKSM_REENTER)) &&
		pPlayer->plane->SetIncomingPlayerPos(pPlayer, pos))
		return;
	...
	if (pPlayer->pos.y < height)
		pPlayer->pos.y = height;
}

bool instance_world_manager::GetTownPosition(gplayer_imp *pImp, const A3DVECTOR &opos, A3DVECTOR &pos, int &world_tag)
{
	return pImp->_plane->GetTownPosition(pImp, opos, pos, world_tag) ||
		   world_manager::GetTownPosition(pImp, opos, pos, world_tag);
}
bool instance_world_manager::ClearSpawn(int sid)
{
	size_t ins_count = _max_active_index;
	for (size_t i = 0; i < ins_count; i++)
	{
...
bool faction_world_manager::FactionLogin(const instance_hash_key &hkey, const GNET::faction_fortress_data *data, const GNET::faction_fortress_data2 *data2)
{
	//首先取得或者创建一个世界
	mutex_spinlock(&_key_lock);
...
bool faction_world_manager::NotifyFactionData(GNET::faction_fortress_data2 *data2)
{
...
bool faction_world_manager::SendReplyToWaitList(const instance_hash_key &hkey, bool is_verify)
{
	bool bRst = false;
	//进入临界区
...
void faction_world_manager::ClearWaitList(const instance_hash_key &hkey, int err_code)
{
	//进入临界区
	spin_autolock keeper(_wait_queue_lock);
...
bool faction_world_manager::AddWaitList(const XID &who, const instance_hash_key &hkey, const instance_key &ikey, const A3DVECTOR &pos)
{
...
int faction_world_manager::CheckPlayerSwitchRequest(const XID &who, const instance_key *ikey, const A3DVECTOR &pos, int ins_timer)
{
	//检查是否去正确的副本
...
void faction_world_manager::GetLogoutPos(gplayer_imp *pImp, int &world_tag, A3DVECTOR &pos)
{
...
void faction_world_manager::UserLogin(int cs_index, int cs_sid, int uid, const void *auth_data, size_t auth_size, bool isshielduser, char flag)
{
	//帮派基地无法直接登陆
	GMSV::SendLoginRe(cs_index, uid, cs_sid, 3, flag); // login failed
}

void faction_world_manager::SetFilterWhenLogin(gplayer_imp *pImp, instance_key *ikey)
{
	pImp->_filters.AddFilter(new aeff_filter(pImp, FILTER_CHECK_INSTANCE_KEY));
}

void faction_world_manager::SetIncomingPlayerPos(gplayer *pPlayer, const A3DVECTOR &origin_pos, int special_mask)
{
...
bool faction_world_manager::GetTownPosition(gplayer_imp *pImp, const A3DVECTOR &opos, A3DVECTOR &pos, int &tag)
{
...
void faction_world_manager::OnDeliveryConnected()
{
...
int faction_world_manager::OnMobDeath(world *pPlane, int faction, int tid)
{
	npc_template *pTemplate = npc_stubs_manager::Get(tid);
	if (!pTemplate || !pTemplate->faction_building_id)
		return 0;
