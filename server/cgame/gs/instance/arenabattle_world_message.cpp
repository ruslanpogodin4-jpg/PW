 ...
 *	处理世界消息
 */
int arenabattle_world_message_handler::RecvExternMessage(int msg_tag, const MSG &msg)
{
	//对于副本，只接受给万家的消息
	if (msg.target.type != GM_TYPE_PLAYER && msg.target.type != GM_TYPE_SERVER)
		return 0;
...
void arenabattle_world_message_handler::PlayerPreEnterServer(gplayer *pPlayer, gplayer_imp *pImp, instance_key &ikey)
{
...
int arenabattle_world_message_handler::HandleMessage(world *pPlane, const MSG &msg)
{
	switch (msg.message)
	{
	case GM_MSG_SWITCH_USER_DATA:
