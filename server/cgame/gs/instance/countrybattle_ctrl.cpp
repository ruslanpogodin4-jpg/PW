...

	{
		...
		for (unsigned int i = 0; i < players.size(); ++i)
		{
			...
			if (ranks.size() < RANK_MAX_SIZE)
			{
				...
				if (val <= ranks[min_val_idx].rank_val)
					min_val_idx = (ranks.size() - 1);
			}
...
void countrybattle_ctrl::Tick(world *pPlane)
{
	...
	{
	case BTYPE_FLAG:
		FlagModeHeartbeat(pPlane);
		break;
	case BTYPE_TOWER:
		TowerModeHeartbeat(pPlane);
		break;
	case BTYPE_STRONGHOLD:
		StrongholdModeHeartbeat(pPlane);
		break;
	}
...
void countrybattle_ctrl::TowerModeHeartbeat(world *pPlane)
{
	if (!_tower_generated)
	{
		countrybattle_world_manager *pManager = (countrybattle_world_manager *)world_manager::GetInstance();
		{
			...
			{
				if (group_set.find(list[i].group) == group_set.end())
					group_set.insert(list[i].group);
			}
...
void countrybattle_ctrl::StrongholdModeHeartbeat(world *pPlane)
{
	...
	if (attacker_score)
	{
		if (attacker_occupy == list.size())
			attacker_score *= 3;
		interlocked_add(&_offense_score.score, attacker_score);
	}
	if (defender_score)
	{
		if (defender_occupy == list.size())
			defender_score *= 3;
...
{
	if (_data.battle_type != BTYPE_FLAG)
		return false;
...
{
	if (_data.battle_type != BTYPE_STRONGHOLD)
		return;
	spin_autolock keeper(_lock);
	countrybattle_world_manager *pManager = (countrybattle_world_manager *)world_manager::GetInstance();
	const countrybattle_world_manager::STRONGHOLD_LIST &list = pManager->GetStrongholdList();
...
bool countrybattle_ctrl::GetStrongholdNearby(bool offense, const A3DVECTOR &opos, A3DVECTOR &pos, int &tag)
{
	...
	for (size_t i = 0; i < _stronghold_state.size(); i++)
	{
		if (offense)
		{
			if (_stronghold_state[i].first != SH_STATE_ATTACKER)
				continue;
		}
		else
		{
			if (_stronghold_state[i].first != SH_STATE_DEFENDER)
				continue;
		}
