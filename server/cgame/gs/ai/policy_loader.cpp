...
namespace
{
	...
	expr *ConvertExpr(void *pTree)
	{
		...
		{
			C_PLAYER_COUNT_IN_REGION *regiondata = (C_PLAYER_COUNT_IN_REGION *)temp->mConditon.pParam;
			return new expr_box_player_count(regiondata->zvMin.x, regiondata->zvMin.z, regiondata->zvMax.x, regiondata->zvMax.z);
		}
		default:
			ASSERT(false);
			break;
...
#define OPARAM(x) ((x *)pOperation->pParam)
		...
		{
			O_SAVE_PLAYER_COUNT_IN_RADIUS_TO_PARAM *pSR = (O_SAVE_PLAYER_COUNT_IN_RADIUS_TO_PARAM *)(pOperation->pParam);
			pOP = new op_get_sphere_player_count((unsigned int)pSR->fRadiusValue, pSR->uRadiusType, pSR->iTargetID, pSR->uTargetType);
		}
		break;
		case CTriggerData::o_save_player_count_in_region_to_param:
		{
			O_SAVE_PLAYER_COUNT_IN_REGION_TO_PARAM *pSR = (O_SAVE_PLAYER_COUNT_IN_REGION_TO_PARAM *)(pOperation->pParam);
			pOP = new op_get_box_player_count(pSR->zvMin.x, pSR->zvMin.z, pSR->zvMax.x, pSR->zvMax.z, pSR->iTargetID, pSR->uTargetType);
		}
		break;
