...
CTriggerData::_s_tree_item *CTriggerData::CopyConditonTree(CTriggerData::_s_tree_item *pRoot)
{
	...
	if (pRoot->pLeft)
		pNew->pLeft = CTriggerData::CopyConditonTree(pRoot->pLeft);
	if (pRoot->pRight)
		pNew->pRight = CTriggerData::CopyConditonTree(pRoot->pRight);
	return pNew;
}
...
{
	if ((size_t)idx > listTriggerPtr.size())
		return;
