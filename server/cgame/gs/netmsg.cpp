...
#include "instance/arenabattle_manager.h"

inline static bool check_player(gplayer *pPlayer, int cs_index, int sid, int uid)
{
	return pPlayer->IsActived() && pPlayer->cs_index == cs_index && pPlayer->cs_sid == sid && pPlayer->ID.id == uid;
}
...
void trade_end(int trade_id, int role1, int role2, bool need_read1, bool need_read2)
{
	...
	if (role1 == role2 || role1 <= 0 || role2 <= 0)
		return;
...
void trade_start(int trade_id, int role1, int role2, int localid1, int localid2)
{
	...
	try
	{
		if (role1 == role2 || role1 <= 0 || role2 <= 0)
			throw -1001;
		if (world_manager::GetWorldParam().forbid_trade)
			throw -1002;
		...
		if (pPlayer1->ID.id != role1 || pPlayer1->cs_sid != localid1)
			throw -3;
		if (pPlayer2->ID.id != role2 || pPlayer2->cs_sid != localid2)
			throw -4;
		if (!pPlayer1->IsActived() || pPlayer1->IsZombie() || pPlayer1->login_state != gplayer::LOGIN_OK)
			throw -5;
		if (!pPlayer2->IsActived() || pPlayer2->IsZombie() || pPlayer2->login_state != gplayer::LOGIN_OK)
			throw -6;

		if (pPlayer1->pos.squared_distance(pPlayer2->pos) > 10.f * 10.f)
			throw -7;
		if (!pPlayer1->imp || !pPlayer2->imp)
			throw -10;

		if (!pPlayer1->imp->CanTrade(pPlayer2->ID))
			throw -8;
		if (!pPlayer2->imp->CanTrade(pPlayer1->ID))
			throw -9;
...
void handle_user_msg(int cs_index, int sid, int uid, const void *msg, size_t size, const void *aux_data, size_t size2, char channel)
{
	...
	spin_autolock keeper(&pPlayer->spinlock);
	if (!pPlayer->IsActived() || pPlayer->cs_index != cs_index || pPlayer->ID.id != uid || pPlayer->cs_sid != sid || pPlayer->b_disconnect || !pPlayer->imp)
	{
		//这种特殊情况直接忽略
		return;
	}
	...
	int dsize = 0;
	if (size2)
		dsize = pImp->TransformChatData(aux_data, size2, buffer, sizeof(buffer));
...
void handle_user_cmd(int cs_index, int sid, int uid, const void *buf, size_t size)
{
	...
	pPlayer->Lock();
	if (!pPlayer->IsActived() || pPlayer->cs_index != cs_index || pPlayer->ID.id != uid || pPlayer->cs_sid != sid || pPlayer->imp == NULL)
	{
		pPlayer->Unlock();
		return;
	}
...
void switch_server_timeout(int link_id, int user_id, int localsid)
{
...
void faction_trade_lock(int trade_id, int roleid)
{
	try
	{
		...
		if (pPlayer->ID.id != roleid)
			throw -2;
		if (!pPlayer->IsActived() || pPlayer->IsZombie() || pPlayer->login_state != gplayer::LOGIN_OK)
			throw -3;
		if (!pPlayer->imp)
			throw -4;
		// FactionOPRequest不会修改玩家的数据库，putmask设置为ALL
		if (!pPlayer->imp->StartFactionTrade(trade_id, 0, DBMASK_PUT_ALL))
			throw -5;
	}
	catch (int)
	{
...
void notify_player_reward(int role, int reward, int reward2, int param, int rewardmask)
{
	...
	if (pPlayer->ID.id != role || !pPlayer->IsActived() || !pPlayer->imp || (pPlayer->login_state != gplayer::LOGIN_OK && pPlayer->login_state != gplayer::WAITING_ENTER))
	{
		return;
	}
	gplayer_imp *pImp = (gplayer_imp *)pPlayer->imp;
...
namespace GNET
{
	...
	void ReceivePlayerFactionInfo(int roleid, unsigned int faction_id, char faction_role, unsigned char faction_pvp_mask, int64_t unifid)
	{
		...
		if (pPlayer->ID.id != roleid || !pPlayer->IsActived() || !pPlayer->imp || (pPlayer->login_state != gplayer::LOGIN_OK && pPlayer->login_state != gplayer::WAITING_ENTER))
		{
			return;
		}
		gplayer_imp *pImp = (gplayer_imp *)pPlayer->imp;
...
{
	arenabattle_param param;
	param.battle_id = battle_id;
	param.battle_type = battle_type;
	param.battle_mode = battle_type;
	param.attacker = attacker;
	param.defender = defender;
	param.player_count = max_player_cnt;
	param.start_timestamp = start_time;
	param.end_timestamp = end_time;
	if (world_manager::GetInstance()->CreateArenaBattle(param))
	{
		GLog::formatlog("Success to Create battle param for the arena battle_id: %d", battle_id);
		GMSV::ResponseArenaBattleStart(0, battle_id);
	}
	else
	{
		GLog::formatlog("Failed to Create battle param for the arena battle_id: %d", battle_id);
		GMSV::ResponseArenaBattleStart(-1, battle_id);
	}
}
...
void player_invite_arenabattle(int role_id, int team_id, int battle_id, int world_tag)
{
	...
	if (!pPlayer)
	{
		GLog::formatlog("Failed to find roleid %d for the arena battle_id: %d", role_id, battle_id);
		return;
	}
	spin_autolock keeper(pPlayer->spinlock);
	...
	{
		GLog::formatlog("Failed to Evaluate roleid %d for the arena battle_id: %d", role_id, battle_id);
		return;
	}
	gplayer_imp *pImp = (gplayer_imp *)pPlayer->imp;
	pImp->ArenaJoinStep1(team_id, battle_id, world_tag);
	GLog::formatlog("Send confirmation signal for roleid %d for the arena battle_id: %d", role_id, battle_id);
}
...
void player_cancel_matchmaking_arena(int role_id, int reason)
{
	...
	gplayer_imp *pImp = (gplayer_imp *)pPlayer->imp;
	pImp->_runner->error_message(S2C::ERR_ARENA_MATCHMAKING_FAILED);
	pImp->_runner->player_arena_queue_cancel(reason);
}
...
{
	if (target_tag != -1 && target_tag != world_manager::GetWorldTag())
		return false;
...
void player_change_gender_ret(int roleid, int ret)
{
	...
	spin_autolock keeper(pPlayer->spinlock);
	if ((pPlayer->ID.id != roleid) || !(pPlayer->IsActived()) || (pPlayer->imp == NULL) || (pPlayer->login_state != gplayer::LOGIN_OK))
		return;

	// ErrorCode: 1 - ERR_PR_PROFILE
	if (ret == 1)
		ret = S2C::ERR_CHANGE_GENDER_PROFILE;
...
static void single_divorce(int role)
{
	...
	{
		pImp->SetSpouse(0);
		if (pPlayer->login_state == gplayer::LOGIN_OK && pImp->_plane)
		{
...
bool get_faction_fortress_create_cost(int *cost, size_t &size)
{
	...
	if (!cfg || dt != DT_FACTION_FORTRESS_CONFIG)
		return false;
	if (size < sizeof(cfg->require_item) / sizeof(cfg->require_item[0]) * 2)
		return false;
	size = 0;
	for (size_t i = 0; i < sizeof(cfg->require_item) / sizeof(cfg->require_item[0]); i++)
	{
		if (cfg->require_item[i].id <= 0 || cfg->require_item[i].count <= 0)
			break;
...
bool get_faction_fortress_initial_value(int *technology, size_t &tsize, int *material, size_t &msize, int *building, size_t &bsize)
{
	...
	if (!cfg || dt != DT_FACTION_FORTRESS_CONFIG)
		return false;
	if (tsize < faction_world_ctrl::TECHNOLOGY_COUNT || msize < faction_world_ctrl::MATERIAL_COUNT || bsize < sizeof(cfg->init_building) / sizeof(int) * 2)
		return false;
...
bool query_player_info(int role, char *name, size_t &name_len, int &level, int &sec_level, int &reputation, int &create_time, int &factionid, int itemid1, int &itemcount1, int itemid2, int &itemcount2, int itemid3, int &itemcount3, int &reincarn_time, int &realm_level)
{
	...
	if (itemid1)
		itemcount1 = pImp->GetItemCount(itemid1);
	if (itemid2)
		itemcount2 = pImp->GetItemCount(itemid2);
	if (itemid3)
		itemcount3 = pImp->GetItemCount(itemid3);
