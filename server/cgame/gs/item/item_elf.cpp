...
{
	ar >> ess;
	if (ess.level < 1 || ess.level > player_template::GetMaxLevel() || ess.total_attribute < ess.level - 1 || ess.total_attribute > ess.level - 1 + int(ess.level / 10) * 100 || ess.strength < 0 || ess.agility < 0 || ess.vitality < 0 || ess.energy < 0 || (ess.strength > ATTRIBUTE_UPPER_LIMIT ? ess.strength + ess.strength - ATTRIBUTE_UPPER_LIMIT : ess.strength) + (ess.agility > ATTRIBUTE_UPPER_LIMIT ? ess.agility + ess.agility - ATTRIBUTE_UPPER_LIMIT : ess.agility) + (ess.vitality > ATTRIBUTE_UPPER_LIMIT ? ess.vitality + ess.vitality - ATTRIBUTE_UPPER_LIMIT : ess.vitality) + (ess.energy > ATTRIBUTE_UPPER_LIMIT ? ess.energy + ess.energy - ATTRIBUTE_UPPER_LIMIT : ess.energy) > ess.total_attribute || ess.total_genius != (ess.level <= 100 ? ess.level / 5 + 1 : 21 + (ess.level - 100)) || ess.genius[0] < 0 || ess.genius[1] < 0 || ess.genius[2] < 0 || ess.genius[3] < 0 || ess.genius[4] < 0 || ess.genius[0] > GENIUS_UPPER_LIMIT || ess.genius[1] > GENIUS_UPPER_LIMIT || ess.genius[2] > GENIUS_UPPER_LIMIT || ess.genius[3] > GENIUS_UPPER_LIMIT || ess.genius[4] > GENIUS_UPPER_LIMIT || ess.genius[0] + ess.genius[1] + ess.genius[2] + ess.genius[3] + ess.genius[4] > ess.total_genius || ess.refine_level < 0 || ess.refine_level > MAX_ELF_REFINE_LEVEL || ess.stamina < 0) // stamina上限status_value不检查了
		throw -201;
	return true;
}
...
bool elf_item::UpdateEquipData() //获取装备类型，需求等级,更新elf_extend_prop
{
	...
	if (prop.cur_genius[0] > GENIUS_UPPER_LIMIT)
		prop.cur_genius[0] = GENIUS_UPPER_LIMIT;
	if (prop.cur_genius[1] > GENIUS_UPPER_LIMIT)
		prop.cur_genius[1] = GENIUS_UPPER_LIMIT;
	if (prop.cur_genius[2] > GENIUS_UPPER_LIMIT)
		prop.cur_genius[2] = GENIUS_UPPER_LIMIT;
	if (prop.cur_genius[3] > GENIUS_UPPER_LIMIT)
		prop.cur_genius[3] = GENIUS_UPPER_LIMIT;
	if (prop.cur_genius[4] > GENIUS_UPPER_LIMIT)
		prop.cur_genius[4] = GENIUS_UPPER_LIMIT;
...
{
	if (str < 0 || agi < 0 || vit < 0 || eng < 0 || (str == 0 && agi == 0 && vit == 0 && eng == 0))
		return false;
...
{
	if (str < 0 || agi < 0 || vit < 0 || eng < 0 || str > ess.strength || agi > ess.agility || vit > ess.vitality || eng > ess.energy || (str == 0 && agi == 0 && vit == 0 && eng == 0))
		return false;
...
{
	if (g0 < 0 || g1 < 0 || g2 < 0 || g3 < 0 || g4 < 0 || g0 > GENIUS_UPPER_LIMIT - prop.cur_genius[0] || g1 > GENIUS_UPPER_LIMIT - prop.cur_genius[1] || g2 > GENIUS_UPPER_LIMIT - prop.cur_genius[2] || g3 > GENIUS_UPPER_LIMIT - prop.cur_genius[3] || g4 > GENIUS_UPPER_LIMIT - prop.cur_genius[4] || (g0 == 0 && g1 == 0 && g2 == 0 && g3 == 0 && g4 == 0))
		return false;
	if (g0 + g1 + g2 + g3 + g4 +
			ess.genius[0] + ess.genius[1] + ess.genius[2] + ess.genius[3] + ess.genius[4] >
		ess.total_genius) //数据不会越界
		return false;
	if (ischeck)
		return true;
...
unsigned int elf_item::InsertExp(unsigned int exp, short exp_level, gactive_imp *imp, bool &is_levelup, bool ischeck) // exp_level为经验丸（100）或人物等级
{
	...
	while (exp > 0)
	{
		...
		if (obtain_factor < 0.1)
			obtain_factor = 0.1;													 //注经验时获得比例最少10%
...
bool elf_item::LevelUp(gactive_imp *imp)
{
	...
	pImp->_runner->equipment_info_changed(1ULL << item::EQUIP_INDEX_ELF, 0, &id1, sizeof(id1), pImp->GetColorName()); //此函数使用了CalcEquipmentInfo的结果
...
bool elf_item::ChangeElfSecureStatus(int status, bool ischeck) //状态转化
{
	...
	case STATUS_SECURE:
		if (status == STATUS_TRANSFORM)
		{
			if (ischeck)
				return true;
			ess.status_value = g_timer.get_systime() + 1;
...
int elf_item::OnCharge(int element_level, size_t count, int &cur_time)
{
	...
	if (count <= 0 || element_level <= 0 || element_level > 10)
		return 0;
	...
	if (offset > use_count * element_stamina[element_level])
		use_count++;
	if (use_count > count)
	{
		use_count = count;
