...
DEFINE_SUBSTANCE(invisible_death_filter, filter, CLS_FILTER_DEATH_INVISIBLE)

void invisible_filter::Heartbeat(int tick)
{
	if (_protected_timeout > 0)
		_protected_timeout -= tick;
	if (_timeout > 0)
	{
		_timeout -= tick;
		if (_timeout <= 0)
...
void invisible_filter::DoEnchant(const XID &attacker, enchant_msg &msg)
{
	...
	if (msg.skill == 1907)
	{
		if (_parent.GetBasicProp().level > msg.ainfo.level)
			return;
	}
	_is_deleted = true;
}
...
void invisible_death_filter::OnAttach()
{
	_parent.InsertTeamVisibleState(368);
}

void invisible_death_filter::OnRelease()
{
	_parent.RemoveTeamVisibleState(368);
}