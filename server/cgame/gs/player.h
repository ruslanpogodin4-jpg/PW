...
struct gplayer : public gactive_object
{
	...
	WRAPPER &Export(WRAPPER &wrapper)
	{
		...
		wrapper << ID << pos << msg_mask << crc << id_mafia << rank_mafia << market_id << vehicle << gm_invisible << custom_crc << base_info.race << base_info.faction << base_info.level << base_info.hp << base_info.max_hp << body_size << cs_sid << cs_index << login_state << adv_data1 << adv_data2 << pariah_state << sec_level << bind_type << mount_color << bind_target << mount_id << spouse_id << team_id << disabled_equip_mask << force_id << country_id << title_id << reincarnation_times << realmlevel << mafia_pvp_mask << mnfaction_id << cash_vip_level << cash_vip_score;
