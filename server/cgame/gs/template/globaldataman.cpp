...
static bool load_malldata(const char *file, abase::vector<MALL_ITEM_SERV> &__global_mall_item_service, int &__mall_timestamp)
{
	...
	if (!fpServer)
		return false;
	printf("Loading mall data from '%s'\n", file);
	bool bRst = true;
	try
	{
		int timestamp;
		...
		printf("Mall data timestamp: %d\n", timestamp);
		int nCount;
		if (fread(&nCount, 1, sizeof(int), fpServer) != sizeof(int) || nCount < 0 || nCount > 65535)
			throw -2;
		printf("Mall data item count: %d\n", nCount);

		// now output a trans target list for server usage
		for (int i = 0; i < nCount; i++)
		{
			printf("Loading mall item %d\n", i);
			GSHOP_ITEM_CLIENT data;
...
#else
			goods.buy_times_limit = -1;
			goods.buy_times_limit_mode = 0;
#endif
	...
	{
		printf("Failed to load mall data from '%s'\n", file);
		bRst = false;
	}
	printf("Loaded mall data from '%s' successfully\n", file);
	fclose(fpServer);
	return bRst;
}
...
{
	if (!load_transdata(file))
	{
		printf("Failed to load trans data from '%s'\n", file);
		return false;
	}
	printf("Loaded trans data from '%s' successfully\n", file);
	if (!load_malldata(file2, global_mall_item_service, mall_timestamp))
	{
		printf("Failed to load mall data from '%s'\n", file2);
		return false;
	}
