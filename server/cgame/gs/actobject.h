...
template <typename WRAPPER>
WRAPPER &operator>>(WRAPPER &ar, object_layer_ctrl &ctrl)
{
	ar.pop_back(&ctrl, sizeof(ctrl));
	return ar;
}
...
WRAPPER &operator<<(WRAPPER &ar, object_layer_ctrl &ctrl)
{
	ar.push_back(&ctrl, sizeof(ctrl));
	return ar;
}

struct newstats_info
{
	int _killed_monsters;
	int _killed_players;
	int _death_by_monsters;
	int _death_by_players;

public:
	newstats_info() : _killed_monsters(0), _killed_players(0), _death_by_monsters(0), _death_by_players(0)
	{
	}

	bool Save(archive &ar)
	{
		ar << _killed_monsters << _killed_players << _death_by_monsters << _death_by_players;
		return true;
	}
	...
	bool Swap(newstats_info &rhs)
	{
		_killed_monsters = rhs._killed_monsters;
		_killed_players = rhs._killed_players;
		_death_by_monsters = rhs._death_by_monsters;
		_death_by_players = rhs._death_by_players;
		return true;
	}

	void GetNewStatInfo(int &killed_monsters, int &killed_players, int &death_by_monsters, int &death_by_players)
	{
		killed_monsters = _killed_monsters;
		killed_players = _killed_players;
		death_by_monsters = _death_by_monsters;
		death_by_players = _death_by_players;
	}

	void SetNewStatInfo(int killed_monsters, int killed_players, int death_by_monsters, int death_by_players)
	{
		_killed_monsters = killed_monsters;
		_killed_players = killed_players;
		_death_by_monsters = death_by_monsters;
		_death_by_players = death_by_players;
	}
};

struct arenastats_info
{
	int _total_victory;
	int _total_defeat;
	int _total_kills;
	int _total_deaths;
	int _total_assistances;
	int _arena_rank;
	int _arena_grade_matchmaking;

public:
	arenastats_info() : _total_victory(0), _total_defeat(0), _total_kills(0), _total_deaths(0), _total_assistances(0), _arena_rank(0), _arena_grade_matchmaking(0)
	{
	}

	bool Save(archive &ar)
	{
		ar << _total_victory << _total_defeat << _total_kills << _total_deaths << _total_assistances << _arena_rank << _arena_grade_matchmaking;
		return true;
	}
	...
	bool Swap(arenastats_info &rhs)
	{
		_total_victory = rhs._total_victory;
		_total_defeat = rhs._total_defeat;
		_total_kills = rhs._total_kills;
		_total_deaths = rhs._total_deaths;
		_total_assistances = rhs._total_assistances;
		_arena_rank = rhs._arena_rank;
		_arena_grade_matchmaking = rhs._arena_grade_matchmaking;
		return true;
	}
	...
	{
		total_victory = _total_victory;
		total_defeat = _total_defeat;
		total_kills = _total_kills;
		total_deaths = _total_deaths;
		total_assistances = _total_assistances;
		arena_rank = _arena_rank;
		arena_grade_matchmaking = _arena_grade_matchmaking;
	}
	...
	{
		_total_victory = total_victory;
		_total_defeat = total_defeat;
		_total_kills = total_kills;
		_total_deaths = total_deaths;
		_total_assistances = total_assistances;
		_arena_rank = arena_rank;
		_arena_grade_matchmaking = arena_grade_matchmaking;
	}
};
...
protected:
	...
	abase::vector<link_sid, abase::fast_alloc<>> _third_subscibe_list;		//第二订阅列表
...
public:
	...
	newstats_info _newstats_info;
	arenastats_info _arenastats_info;
	...
	inline void CheckVisibleDataForTeam()
	{
		if (_team_visible_state_flag)
			SendTeamDataToMembers();
	}
...
public:
	...
	act_session *GetNextSession()
	{
		if (HasNextSession())
			return _session_list[0];
		else
			return NULL;
	}
	...
	{
		if (!_moving_action_env.ActionValid(action_id))
			return true;
...
public:
	...
	virtual bool ReturnWaypoint(int point, bool globalteleport = false) { return false; }
	...
	virtual int OI_GetPetType() { return -1; }
	...
	virtual void SetInfectSkill(int skill, int level)
	{
		_infected_skill_id = skill;
		_infected_skill_lvl = level;
	}
	virtual int GetInfectLevel(int skill)
	{
		if (_infected_skill_id == skill)
			return _infected_skill_lvl;
		return -1;
	}
...
public:
	...
	inline int GetMagicRuneDamage()
	{
		if (_cur_rune.rune_level_min)
		{
			...
			{
				if (_cur_rune.rune_level_max >= _cur_item.weapon_level && _cur_rune.rune_level_min <= _cur_item.weapon_level)
				{
...
protected:
	...
	void RemoveInfoSubscibe(const XID &target)
	{
		...
		for (; last != first;)
		{
			...
			if (target.id == last->user_id)
			{
				...
				if (_subscibe_list.empty() && _second_subscibe_list.empty() && _third_subscibe_list.empty())
					_subscibe_timer = 0;
				return;
			}
		}
	...
	void InsertInfoThirdSubscibe(const XID &target, const link_sid &ld);
	void RemoveInfoThirdSubscibe(const XID &target)
	{
		link_sid *last = _third_subscibe_list.end();
		link_sid *first = _third_subscibe_list.begin();
		for (; last != first;)
		{
			--last;
			if (target.id == last->user_id)
			{
				_third_subscibe_list.erase_noorder(last);
				if (_subscibe_list.empty() && _third_subscibe_list.empty() && _third_subscibe_list.empty())
					_subscibe_timer = 0;
				return;
			}
		}
	...
	inline void RemoveTeamVisibleState(unsigned short state)
	{
		...
		for (size_t i = count; i > 0; i--)
		{
			...
			if (__TVSGetState(_visible_team_state[index]) == state)
			{
				...
				if (param_count)
					_visible_team_state_param.erase(_visible_team_state_param.begin() + param_index, _visible_team_state_param.begin() + param_index + param_count);
	...
	inline void DecreaseTeamVisibleState(unsigned short state, size_t param_count)
	{
		ASSERT(param_count >= 1 && param_count <= 3);
		size_t param_index = _visible_team_state_param.size();
		size_t count = _visible_team_state.size();
		for (size_t i = count; i > 0; i--)
		{
			...
			if (__TVSGetState(_visible_team_state[index]) == state)
			{
				ASSERT(param_count == pcount);
				for (size_t m = 0; m < param_count; m++)
					_visible_team_state_param[param_index + m] = _visible_team_state_param[param_index + m] - 1;
				return;
			}
		}
	}
	...
	void UpdateTeamVisibleStateCountdown();
...
public:
	...
	{
		int vigortotal = _vigour_base + _vigour_en;
		return vigortotal > 1500 ? 1500 : vigortotal;
	}
...
protected:
	...
	inline void IncHP(int hp_gen)
	{
		...
		if (tmp > _cur_prop.max_hp)
			tmp = _cur_prop.max_hp;
		if (tmp != _basic.hp)
		{
			_basic.hp = tmp;
			SetRefreshState();
