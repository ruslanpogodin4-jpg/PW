...
void world::SetWorldCtrl(world_data_ctrl *ctrl)
{
	ASSERT(w_ctrl == NULL);
	w_ctrl = ctrl;
}

bool world::Init(int world_index)
{
...
bool world::InitNPCGenerator(CNPCGenMan &npcgen)
{
	...
	if (!w_npc_gen.BeginSpawn())
		return false;
	return true;
}
bool world::InitNPCGenerator(CNPCGenMan &ctrldata, npcgen_data_list &npcgen_list)
{
	if (!npcgen_list.size())
		return false;
...
bool world::InitNPCGeneratorByClone(CNPCGenMan &ctrldata, npcgen_data_list &npcgen_list)
{
	if (!npcgen_list.size())
		return false;
...
bool world::TriggerSpawn(int condition, bool notify_world_ctrl)
{
	if (condition == 0)
		return false;
	if (world_manager::GetGlobalController().CheckServerForbid(SERVER_FORBID_CTRL, condition))
		return false;
	if (notify_world_ctrl && w_ctrl)
		w_ctrl->OnTriggerSpawn(condition);
	return w_npc_gen.TriggerSpawn(condition);
}

bool world::ClearSpawn(int condition, bool notify_world_ctrl)
{
	if (condition == 0)
		return false;
	if (notify_world_ctrl && w_ctrl)
		w_ctrl->OnClearSpawn(condition);
	w_npc_gen.ClearSpawn(condition);
	return true;
}

bool world::CreateGrid(int row, int column, float step, float sx, float sy)
{
...
int world::BuildSliceMask(float near, float far)
{
	if (far < near)
		return -1;
	w_off_list.clear();
	...
	if (fabs(near - n1 * w_grid.slice_step) > 1e-3)
		n1++;
	if (fabs(far - f1 * w_grid.slice_step) > 1e-3)
		f1++;
	for (int i = 1; i <= f1; i++)
	{
		for (int j = -i; j < i; j++)
		...
		if (n1 == i)
			w_near_vision = w_off_list.size();
		if (tf1 == i)
			w_true_vision = w_off_list.size();
	}
...
void world::RemovePlayer(gplayer *pPlayer)
{
...
void world::RemoveNPC(gnpc *pNPC)
{
...
void world::RemoveMatter(gmatter *pMatter)
{
...
int world::InsertPlayer(gplayer *pPlayer)
{
...
bool world::IsPlayerExist(int player_id)
{
	return w_world_man->GetPlayerServerIdx(player_id) >= 0;
}

int world::InsertMatter(gmatter *pMatter)
{
...
#endif
	if (obj->imp)
		rst = obj->imp->DispatchMessage(plane, msg);
	if (!rst)
	{
...
void world::try_dispatch_extern_msg(const MSG &msg)
{
	if (msg.ttl <= 0)
	{
	...
	case GM_TYPE_MATTER:
	{
		...
		//发送到目标机器上
		if (w_idx != (size_t)w_index && w_idx < MAX_GS_NUM)
			SendRemoteMessage(w_idx, msg);
...
int world::DispatchMessage(const MSG &msg)
{
#ifndef _NDEBUG
...
#ifdef _DEBUG
		if (msg.message != 31)
			__PRINTF("handle message %d to %p\n", msg.message, obj);
			//滤掉session repeat的函数
#endif
		obj->Lock();
...
int world::DispatchMessage(gobject *obj, const MSG &msg)
{
	obj->Lock();
	call_message_handler(obj, this, msg);
	return 0;
...
namespace
{
	...
	struct object_box_collector_2
	{
		...
		inline void operator()(gobject *pObj, const A3DVECTOR &pos)
		{
			if (_rt.IsIn(pObj->pos.x, pObj->pos.z))
			{
				_list.push_back(pObj);
			}
		}
...
int world::BroadcastLocalMessage(const MSG &msg, float fRadius, int mask)
{
...
int world::BroadcastMessage(const MSG &msg, float fRadius, int mask)
{
	//首先判断是否和外面的区域相交
	...
	if (!GetGrid().inner_region.IsIn(rt))
	{
		...
		if (msg.content_length)
			memcpy(content + sizeof(MSG), msg.content, msg.content_length);
		pMsg->pos.x = fRadius;
		pMsg->param = mask;
		pMsg->ttl = 1;
...
int world::BroadcastSphereMessage(const MSG &msg, const A3DVECTOR &target, float fRadius)
{
	//首先判断是否和外面的区域相交
...
int world::BroadcastCylinderMessage(const MSG &msg, const A3DVECTOR &target, float fRadius)
{
	//首先判断是否和外面的区域相交
...
int world::BroadcastTaperMessage(const MSG &msg, const A3DVECTOR &target, float fRadius, float cos_halfangle)
{
	//首先判断是否和外面的区域相交
	rect rt(msg.pos.x - fRadius, msg.pos.z - fRadius, msg.pos.x + fRadius, msg.pos.z + fRadius);
	if (!GetGrid().inner_region.IsIn(rt))
...
int world::BroadcastLocalBoxMessage(const MSG &msg, const rect &rt)
{
	abase::vector<gobject *, abase::fast_alloc<>> list;
	switch (msg.target.type)
	{
...
int world::BroadcastLocalSphereMessage(const MSG &msg, const A3DVECTOR &target, float fRadius)
{
	abase::vector<gobject *, abase::fast_alloc<>> list;
	switch (msg.target.type)
	{
...
int world::BroadcastLocalCylinderMessage(const MSG &msg, const A3DVECTOR &target, float fRadius)
{
	abase::vector<gobject *, abase::fast_alloc<>> list;
	switch (msg.target.type)
	{
...
int world::BroadcastLocalTaperMessage(const MSG &msg, const A3DVECTOR &target, float fRadius, float cos_halfangle)
{
	abase::vector<gobject *, abase::fast_alloc<>> list;
	switch (msg.target.type)
	{
...
bool world::QueryObject(const XID &id, object_info &info)
{
	//对一个对象进行查询
	//查询策略是：
	// 1.首先在本地寻找,
...
void world::DuplicateWorld(world *dest) const
{
	dest->Init(w_index);
...
void world::BattleFactionSay(int faction, const void *msg, size_t size, char emote_id, const void *aux_data, size_t dsize, int self_id, int self_level)
{
	if (w_ctrl)
	{
...
void world::BattleSay(const void *msg, size_t size)
{
	if (w_ctrl)
	{
...
void world::InstanceSay(const void *msg, size_t size, bool middle, const void *data, size_t dsize)
{
	if (!world_manager::GetWorldLimit().common_data_notify)
		return; //只有这个功能打开，player_list才是有效的
...
void world::SetCommonValue(int key, int value, bool notify_world_ctrl)
{
	...
	if (key > 100000)
	{
		...
		if (notify_world_ctrl && w_ctrl)
			w_ctrl->OnSetCommonValue(key, value);
	}
}

int world::GetCommonValue(int key)
{
	return w_common_data.GetValue(key);
}

int world::ModifyCommonValue(int key, int offset)
{
	...
	if (key > 100000)
	{
		...
		if (w_ctrl)
			w_ctrl->OnSetCommonValue(key, rst);
	}
	return rst;
}

void world::AddPlayerNode(gplayer *pPlayer)
{
	spin_autolock keeper(w_player_node_lock);
...
void world::DelPlayerNode(gplayer *pPlayer)
{
	spin_autolock keeper(w_player_node_lock);
	int cs_index = pPlayer->cs_index;
	std::pair<int, int> val(pPlayer->ID.id, pPlayer->cs_sid);
...
void world::CommonDataNotify(int key, int value)
{
...
void world::AddSceneServiceNpc(int tid, int id)
{
...
void world::DelSceneServiceNpc(int tid, int id)
{
	spin_autolock keeper(w_scene_service_npcs_lock);
	abase::static_multimap<int, int>::iterator it = w_scene_service_npcs.find(tid);
...
void world::GetSceneServiceNpc(abase::vector<int> &list)
{
	spin_autolock keeper(w_scene_service_npcs_lock);
...
int world::GetSpherePlayerListSize(const A3DVECTOR &target, float fRadius)
{
	abase::vector<gobject *, abase::fast_alloc<>> list;
	object_sphere_collector<Foreach1<gplayer>> worker(this, list, fRadius);
	ForEachSlice(target, fRadius, worker);
...
int world::RebuildMapRes()
{
	int mapres_type = w_world_man->GetMapRes().GetType();
	if (mapres_type == MAPRES_TYPE_ORIGIN)
		return 0;
	if (w_map_generator || w_terrain || w_movemap || w_traceman)
		return -16;
	...
	try
	{
		...
		if (!w_map_generator->Generate(local_rt, mapres.GetMapResInfo()))
			throw -2;
...
void world::SyncPlayerWorldGen(gplayer *pPlayer)
{
	if (w_map_generator)
		w_map_generator->SyncPlayerWorldGen((gplayer_imp *)pPlayer->imp);
}
