...
void PlayerTaskInterface::DeliverGold(unsigned long ulGoldNum)
{
	int gold = ulGoldNum;
...
void PlayerTaskInterface::DeliverExperience(unsigned long ulExp)
{
...
void PlayerTaskInterface::DeliverSP(unsigned long ulSP)
{
	int exp = 0;
...
void PlayerTaskInterface::DeliverReputation(long lReputation)
{
...
int PlayerTaskInterface::GetTaskItemCount(unsigned long ulTaskItem)
{
...
int PlayerTaskInterface::GetCommonItemCount(unsigned long ulCommonItem)
{
...
bool PlayerTaskInterface::IsInFaction(unsigned long)
{
	return ((gplayer *)_imp->_parent)->id_mafia;
}

int PlayerTaskInterface::GetFactionRole()
{
...
void PlayerTaskInterface::TakeAwayGold(unsigned long ulNum)
{
	_imp->SpendMoney(ulNum);
	_imp->_runner->spend_money(ulNum);
}

void PlayerTaskInterface::TakeAwayTaskItem(unsigned long ulTemplId, unsigned long ulNum)
{
	item_list &inv = _imp->GetTaskInventory();
...
void PlayerTaskInterface::TakeAwayCommonItem(unsigned long ulTemplId, unsigned long ulNum)
{
	item_list &inv = _imp->GetInventory();
	int rst = 0;
	size_t num = ulNum;
...
long PlayerTaskInterface::GetReputation()
{
...
void PlayerTaskInterface::SetCurPeriod(unsigned long per)
{
...
bool PlayerTaskInterface::CanDeliverCommonItem(unsigned long ulItemTypes)
{
	return _imp->GetInventory().GetEmptySlotCount() >= ulItemTypes;
}

void PlayerTaskInterface::DeliverCommonItem(unsigned long ulItem, unsigned long count, long lPeriod)
{
	if (count == 0)
		return;
	...
	if (pData)
	{
		...
		// ����false����û��ȫ������
		if (_imp->ObtainItem(gplayer_imp::IL_INVENTORY, pData, true))
			FreeItem(pData);
	}
	else
	{
...
bool PlayerTaskInterface::CanDeliverTaskItem(unsigned long ulItemTypes)
{
	return _imp->GetTaskInventory().GetEmptySlotCount() >= ulItemTypes;
}

void PlayerTaskInterface::DeliverTaskItem(unsigned long ulItem, unsigned long count)
{
	if (count == 0)
		return;
	element_data::item_tag_t tag = {element_data::IMT_CREATE, 0};
	...
	if (pData)
	{
		if (count > pData->pile_limit)
			...
			pData->count = count;
		// ����false����û��ȫ������
		if (_imp->ObtainItem(gplayer_imp::IL_TASK_INVENTORY, pData, true))
			FreeItem(pData);
	}
	else
	{
...
void PlayerTaskInterface::NotifyClient(const void *pBuf, size_t sz)
{
	_imp->_runner->send_task_var_data(pBuf, sz);
}

float PlayerTaskInterface::UnitRand()
{
	return abase::Rand(0.f, 1.f);
}

int PlayerTaskInterface::RandNormal(int low, int high)
{
	return abase::RandNormal(low, high);
}

int PlayerTaskInterface::RandSelect(const float *option, int size)
{
	return abase::RandSelect(option, size);
}

int PlayerTaskInterface::GetTeamMemberNum()
{
	return _imp->GetTeamMemberNum();
}

void PlayerTaskInterface::NotifyPlayer(unsigned long ulPlayerId, const void *pBuf, size_t sz)
{
	XID id(GM_TYPE_PLAYER, ulPlayerId);
	_imp->SendTo<0>(GM_MSG_PLAYER_TASK_TRANSFER, id, 0, pBuf, sz);
}

void PlayerTaskInterface::GetTeamMemberInfo(int nIndex, task_team_member_info *pInfo)
{
...
void PlayerTaskTeamInterface::SetMarriage(int nPlayer)
{
	marriage_op = 1;
	if (couple[0] == 0)
	{
...
void PlayerTaskInterface::SendMessage(unsigned long task_id, int channel, int param)
{
...
{
	if (_imp->OI_IsMafiaMember())
		_imp->IncFactionContrib(iConsumeContrib, iExpContrib);
}
...
{
	if (_imp->OI_IsMafiaMember())
		_imp->DecFactionContrib(ulNum, 0);
}
...
{
	if (_imp->OI_IsMafiaMember())
		_imp->DecFactionContrib(0, ulNum);
}
...
{
	if (iValue > 0)
		return _imp->IncForceContribution(iValue);
	else
		return _imp->DecForceContribution(-iValue);
}
bool PlayerTaskInterface::ChangeForceReputation(int iValue)
{
	if (iValue > 0)
		return _imp->IncForceReputation(iValue);
	else
		return _imp->DecForceReputation(-iValue);
}
...
{
	if (sp > _imp->_basic.skill_point)
		sp = _imp->_basic.skill_point;
	if (sp <= 0)
		return false;
