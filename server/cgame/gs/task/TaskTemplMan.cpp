...
ATaskTemplMan::ATaskTemplMan() : m_pEleDataMan(0),
...
{
	if (lhs->m_ulPremItems && !rhs->m_ulPremItems)
		return true;
	else if (!lhs->m_ulPremItems && rhs->m_ulPremItems)
		return false;
	else if (lhs->m_ulPremItems && rhs->m_ulPremItems)
...
bool ATaskTemplMan::TasksSeekOutDiff::operator<(const TasksSeekOutDiff &rhs) const
{
	if (!task && rhs.task)
		return false;
	else if (task && !rhs.task)
		return true;
	else if (!task && !rhs.task)
		return true;
	else
		return compare_tasks_canseekout(task, rhs.task);
}
...
#ifdef _ELEMENTCLIENT
	if (CECCommandLine::GetBriefConfig(_AL("noautodelivertask")))
		return;
#endif
#endif
...
void ATaskTemplMan::UpdateTimeLimitCheckList()
{
	...
	for (; it != m_TaskTemplMap.end(); ++it)
		if (it->second->m_ulMaxReceiver)
			m_TmLmtChkLst.push_back(it->second);
}

void ATaskTemplMan::OnTaskGetDynTasksTimeMark(TaskInterface *pTask)
{
	if (m_ulDynTasksTimeMark == 0)
		return;
...
{
	// ��ʱ���Ƿ����?0�㣬����0��ˢ�´������㣬ˢ��ʱ�����?
	if (idStorage == 0 || idStorage > TASK_STORAGE_COUNT)
		return false;
...
struct WeightTaskData
{
	...
	WeightTaskData(int idTask, int taskWeight) : id(idTask),
												 weight(taskWeight)
	{
	}
...
{
	if (!pEssence || !pTask || !pStorageLst || idStorage <= 0 || idStorage > TASK_STORAGE_COUNT)
		return false;
	...
	if (bDebug)
		startIndex = pTask->RandNormal(0, 365 * 3);
	...
	if (totalWeight < pEssence->max_weight)
	{
		...
		{
			// ����һ���������֮���˳�ѭ��?
			if (traversalCount >= s_RandTableSize)
				break;
			int randIndex = startIndex + offsetIndex;
			if (randIndex >= s_RandTableSize)
			{
		...
		if (totalWeight < pEssence->max_weight)
		{
			...
			{
				if (setIndex.find(i) == setIndex.end() && totalWeight + vecTask[i].weight <= pEssence->max_weight)
				{
					setIndex.insert(i);
...
void TaskShowErrMessage(int nIndex)
{
	...
	if (szMsg)
		TaskInterface::ShowMessage(szMsg);
}
			...
			if (ulWorldId == 1)
			{
				...
				else
				{
					...
					if (distance > dist2 && distance <= dist3)
						level = ATaskTempl::DISTANCE_NEAR;
					else if (distance > dist3 && distance <= dist4)
						level = ATaskTempl::DISTANCE_MEDIUM;
					else if (distance > dist4 && distance <= dist5)
						level = ATaskTempl::DISTANCE_FAR;
					else
						level = ATaskTempl::DISTANCE_FAR_FAR_AWAY;
				}
				if (level > ret)
					ret = level;
			}
...
unsigned long ATaskTemplMan::GetStoragePremItemID(unsigned int idStorage)
{
	...
	unsigned long ret = 0;
	if (pEssence)
		ret = pEssence->storage_open_item;
	return ret;
}
int ATaskTemplMan::GetStorageRefreshPerDay(unsigned int idStorage)
{
	...
	int ret = 0;
	if (pEssence)
		ret = pEssence->storage_refresh_per_day;
	return ret;
}
int ATaskTemplMan::GetMaxDeliverPerDay(unsigned int idStorage)
{
	...
	int ret = 0;
	if (pEssence)
		ret = pEssence->storage_deliver_per_day;
	return ret;
}
bool ATaskTemplMan::IsReceiveMaxStorageTasksPerDay(unsigned int idStorage, unsigned char receiveCount)
{
	...
	bool ret = false;
	if (pEssence)
		ret = pEssence->storage_deliver_per_day <= receiveCount;
	return ret;
}
bool ATaskTemplMan::HasStorageTaskDeliverLimit(unsigned int idStorage)
{
	...
	bool ret = false;
	if (pEssence)
		ret = pEssence->storage_deliver_per_day != 0; // Ĭ��0����������ÿ�췢�Ŵ���
	return ret;
}
...
bool ATaskTemplMan::LoadTasksFromPack(const char *szPackPath, bool bLoadDescript)
{
	...
	char *encryptpath = "/tmp/tasks_temp";
	...
	if (tph.magic != TASK_PACK_MAGIC || tph.version != _task_templ_cur_version)
		return false;
...
void ATaskTemplMan::AddOneTaskTempl(ATaskTempl *pTask)
{
	...
	if (pTask->m_bDeathTrig)
		m_DeathTrigMap[pTask->m_ID] = pTask;
	else if (pTask->m_bAutoDeliver)
		m_AutoDelvMap[pTask->m_ID] = pTask;

	if (pTask->m_bPQTask)
		m_PQTemplMap[pTask->m_ID] = pTask;

	if (pTask->m_bSkillTask)
		m_SkillTaskLst.push_back(pTask);
