...
namespace ai_trigger
{
	...
	{
		gactive_imp *pImp = self->GetAIObject()->GetImpl();
		return pImp->_plane->GetSpherePlayerListSize(pImp->_parent->pos, _fRadius);
	}
	...
	bool op_get_sphere_player_count::DoSomething(policy *self)
	{
		...
		gactive_imp *pImp = self->GetAIObject()->GetImpl(); // pegar a implementacao da inteligencia artificial

		logic_val lv_radius(_rtype, _radius, pImp);
		logic_val lv_dest(_dtype, _dest, pImp);

		count = pImp->_plane->GetSpherePlayerListSize(pImp->_parent->pos, (float)_radius);
		lv_dest.setval(count);
		return true;
	}
	...
	bool op_get_box_player_count::DoSomething(policy *self)
	{
		...
		logic_val lv_dest(_dtype, _dest, pImp); // [esp+20h] [ebp-28h]

		count = pImp->_plane->GetBoxPlayerListSize(pImp->_parent->pos, _rect);
		lv_dest.setval(count);
		return true;
	}
}
