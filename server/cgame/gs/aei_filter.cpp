...
DEFINE_SUBSTANCE(aei_arena_filter, filter, CLS_FILTER_CHECK_ARENABATTLE_KEY)
void aei_filter::OnAttach()
{
	_key = _parent.GetImpl()->_plane->w_ins_key;
}

void aei_filter::OnRelease()
{
	// do nothing
}

void aei_filter::Heartbeat(int tick)
{
...
void aebf_filter::OnModify(int ctrlname, void *ctrlval, size_t ctrllen)
{
...
void aebf_filter::Heartbeat(int tick)
{
...
void aeff_filter::Heartbeat(int tick)
{
	gplayer_imp *pImp = (gplayer_imp *)_parent.GetImpl();
	...
	if (_state == NORMAL)
	{
		...
		if (_timeout <= 0)
		{
			...
			if (pCtrl->iskick || cur_mafia != pCtrl->factionid && (!pCtrl->inbattle || cur_mafia != pCtrl->offense_faction) && !(world_manager::GetWorldLimit().gmfree && _parent.CheckGMPrivilege()))
			{
				_state = WAIT_ESCAPE;
				_timeout = abase::Rand(55, 90);
...
void aeff_filter::OnModify(int ctrlname, void *ctrlval, size_t ctrllen)
{
...
void aegw_filter::Heartbeat(int tick)
{
	gplayer_imp *pImp = (gplayer_imp *)_parent.GetImpl();
...
void aect_filter::Heartbeat(int tick)
{
	gplayer_imp *pImp = (gplayer_imp *)_parent.GetImpl();
	...
	if (_state == NORMAL)
	{
		...
		{
			//检查踢出标志全局变量
			if (!pImp->GetCountryId() && !(world_manager::GetWorldLimit().gmfree && _parent.CheckGMPrivilege()))
			{
				//该踢出了
				_state = WAIT_ESCAPE;
				_timeout = 3;
...
void aecb_filter::Heartbeat(int tick)
{
	gplayer_imp *pImp = (gplayer_imp *)_parent.GetImpl();
	world *pPlane = pImp->_plane;
	...
	{
		if (_origin_country <= 0 || cur_country != _origin_country && !(world_manager::GetWorldLimit().gmfree && _parent.CheckGMPrivilege()) || pImp->_parent->IsZombie() && !pImp->CanResurrect(0))
		{
			_origin_country = -1;
			if (_timeout > 0)
				_timeout--;
...
void aecb_filter::OnModify(int ctrlname, void *ctrlval, size_t ctrllen)
{
...
void aetb_filter::Heartbeat(int tick)
{
	gplayer_imp *pImp = (gplayer_imp *)_parent.GetImpl();
...
void aetb_filter::OnModify(int ctrlname, void *ctrlval, size_t ctrllen)
{
...
void aecv_filter::Heartbeat(int tick)
{
	gplayer_imp *pImp = (gplayer_imp *)_parent.GetImpl();
...
void aemf_filter::Heartbeat(int tick)
{
	gplayer_imp *pImp = (gplayer_imp *)_parent.GetImpl();
	world *pPlane = pImp->_plane;
...
void aei_arena_filter::Heartbeat(int tick)
{
	gplayer_imp *pImp = (gplayer_imp *)_parent.GetImpl();
	gplayer *pPlayer = pImp->GetParent();
	world *pPlane = pImp->_plane;
	if (_state == NORMAL)
	{
		...
		if (pCtrl->_offense_data.death_count != _attacker_score || pCtrl->_offense_data.goal_count != _attacker_goal || pCtrl->_defence_data.death_count != _defender_score || pCtrl->_defence_data.goal_count != _defender_goal)
		{
			_attacker_score = pCtrl->_offense_data.death_count;
			_attacker_goal = pCtrl->_offense_data.goal_count;
			_defender_score = pCtrl->_defence_data.death_count;
			_defender_goal = pCtrl->_defence_data.goal_count;
			int count = 0;
			int *rolelist;

			if (pPlayer->IsBattleOffense())
			{
				count = pCtrl->_defender_player_list.size();
				rolelist = new int[count];
				int idx = 0;
				std::map<int, gplayer *>::iterator it = pCtrl->_defender_player_list.begin();
				while (it != pCtrl->_defender_player_list.end())
				{
					rolelist[idx] = it->second->ID.id;
					idx++;
					it++;
				}
			}
			...
			{
				count = pCtrl->_attacker_player_list.size();
				rolelist = new int[count];
				int idx = 0;
				std::map<int, gplayer *>::iterator it = pCtrl->_attacker_player_list.begin();
				while (it != pCtrl->_attacker_player_list.end())
				{
					rolelist[idx] = it->second->ID.id;
					idx++;
					it++;
			...
			pImp->_runner->player_arena_score_update(pCtrl->_defence_data.goal_count, pCtrl->_offense_data.goal_count, pCtrl->_defence_data.death_count, pCtrl->_offense_data.death_count, count, rolelist);
			delete[] rolelist;
		}
		if (pCtrl->_battle_result)
		{
			_state = WAIT_ESCAPE;
			_timeout = 105;
			spin_autolock l(pCtrl->_lock);
			int listsize = pCtrl->_attacker_player_list.size() + pCtrl->_defender_player_list.size();

			role_result *playerlist = new role_result[listsize];

			int idx = 0;
			std::map<int, gplayer *>::iterator it2 = pCtrl->_attacker_player_list.begin();
			while (it2 != pCtrl->_attacker_player_list.end())
			{
				gplayer_imp *playerimp = (gplayer_imp *)it2->second->imp;

				playerlist[idx].roleid = it2->second->ID.id;
				playerlist[idx].occupation = playerimp->GetPlayerClass();
				playerlist[idx].rank = playerimp->_arenastats_info._arena_rank;
				playerlist[idx].victories = playerimp->_arenastats_info._total_victory;
				size_t len;
				const void *name;
				name = playerimp->GetPlayerName(len);
				if (len > MAX_USERNAME_LENGTH)
					len = MAX_USERNAME_LENGTH;
				playerlist[idx].name_len = len;
				memcpy(playerlist[idx].rolename, name, len);
				idx++;
				it2++;
			}

			std::map<int, gplayer *>::iterator it3 = pCtrl->_defender_player_list.begin();

			while (it3 != pCtrl->_defender_player_list.end())
			{
				gplayer_imp *playerimp = (gplayer_imp *)it3->second->imp;

				playerlist[idx].roleid = it3->second->ID.id;
				playerlist[idx].occupation = playerimp->GetPlayerClass();
				playerlist[idx].rank = playerimp->_arenastats_info._arena_rank;
				playerlist[idx].victories = playerimp->_arenastats_info._total_victory;
				size_t len;
			...
			pImp->_runner->player_arena_battle_result(
				88,
				pCtrl->_offense_data.dmg_count,
				pCtrl->_offense_data.goal_count - pCtrl->_defence_data.death_count,
				77,
				pCtrl->_defence_data.dmg_count,
				pCtrl->_defence_data.goal_count - pCtrl->_offense_data.death_count,
				pCtrl->_battle_result,
				pCtrl->_winner_faction,
				pCtrl->_data.battle_type,
				pCtrl->_attacker_player_list.size(),
				pCtrl->_defender_player_list.size(),
				playerlist,
				sizeof(role_result) * listsize);
			pImp->_runner->kickout_instance(KIR_ARENA_END, _timeout);
			delete[] playerlist;
		}
	}
	else if (_state == WAIT_ESCAPE)
...
void aei_arena_filter::OnModify(int ctrlname, void *ctrlval, size_t ctrllen)
{
	if (ctrlname == FMID_CLEAR_AEI_ARENA)
	{
		_state = WAIT_ESCAPE;
		_team_id = -1;
	}

	if (ctrlname == FMID_LEAVE_AEI_ARENA && _state == WAIT_ESCAPE)
		_timeout = 0;
}