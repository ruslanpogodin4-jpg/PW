...
class invisible_death_filter : public filter
{
protected:
	enum
	{
		MASK = FILTER_MASK_REMOVE_ON_DEATH | FILTER_MASK_UNIQUE
	};
	...
	invisible_death_filter() {}

public:
	DECLARE_SUBSTANCE(invisible_death_filter);
	invisible_death_filter(object_interface parent)
		: filter(parent, MASK)
	{
		_filter_id = FILTER_INDEX_INVISIBLE_DEATH;
	}
