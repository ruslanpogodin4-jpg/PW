...
void playersolochallenge::PlayerEnterSoloChallengeInstance(gplayer_imp *pImp)
{
	...
	//pImp->_filters.Clear();//Big bug, do not use this function
	...
	GLog::formatlog("SOLO_CHALLENGE_LOG ENTER user %d enters Solo Tower Challenge",pImp->_parent->ID.id);
}

void playersolochallenge::PlayerLeaveSoloChallengeInstance(gplayer_imp *pImp)
{
	GLog::formatlog("SOLO_CHALLENGE_LOG LEAVE user %d exists Solo Tower Challenge.",pImp->_parent->ID.id);
}
...
void playersolochallenge::CalcPlayMode(SOLO_TOWER_CHALLENGE_LEVEL_CONFIG *conf, int stage_level, gplayer_imp *pImp)
{
	...
	for(int i = 0; i < 32; ++i)
	{
		if(conf -> steps[(stage_level-1)/9].playing_method_controller[i] == 0)
		  break;
		++playmode_count;
	}// Get the number of play modes from config table
	
	if(stage_level % 9 == 0)// This is a boss stage
	{
	...
	if(IsTopLevel(stage_level)) //Its playing a new stage, select one that has not been repetead
	{
		if(stage_level % 9 == 1)
		{
			_play_modes = 0;
		...
		int index = abase::Rand(0,left_play_mode.size() - 1);// Random play mode
...
void playersolochallenge::StageComplete(gplayer_imp *pImp, bool isCompleteSuccess)//单人副本发奖
{
	...
	if(IsTopLevel(cur_stage_level))//关卡>最大关卡，奖励道具
	{
		...
		if(_max_stage_level == SOLO_TOWER_CHALLENGE_MAX_STAGE)
		{
			...
			GLog::formatlog("SOLO_CHALLENGE_LOG MAXSTAGE User %d has passed highest level. Total time is %d, time %ld.", pImp->_parent->ID.id, _total_first_climbing_time, g_timer.get_systime());
		}
	}
	...
	GLog::log(GLOG_INFO,"SOLO_CHALLENGE_LOG THROUGH_STAGE User %d has passed level %d,Time %ld,Max points %d ,Current points %d ",pImp->_parent->ID.id,cur_stage_level,g_timer.get_systime(),_total_score,_cur_score);
...
void playersolochallenge::StageStart(gplayer_imp *pImp, bool isStartSuccess)
{
	...
	GLog::log(GLOG_INFO,"SOLO_CHALLENGE_LOG STAGE_START User %d starts level %d,time %ld, gamemode %d, gameplay record %d.",pImp->_parent->ID.id, cur_stage_level, g_timer.get_systime(), _cur_stage_play_mode, _play_modes);
...
{
 	if(_left_draw_award_times)//Reward items only for the last opened stage
 	{
 		DATA_TYPE dt;
 		SOLO_TOWER_CHALLENGE_AWARD_LIST_CONFIG* conf = (SOLO_TOWER_CHALLENGE_AWARD_LIST_CONFIG*)world_manager::GetDataMan().get_data_ptr(SOLO_TOWER_CHALLENGE_AWARD_LIST_CONFIG_ID, ID_SPACE_CONFIG, dt);
 		if ((conf == NULL) || (dt != DT_SOLO_TOWER_CHALLENGE_AWARD_LIST_CONFIG))
 		...
 		int award_page_config_id = conf->level[_max_stage_level - 1].award_page_config_id;// Reward configuration table id
		...
		int index = abase::RandSelect(&(conf_award->list[0].probability),sizeof(conf_award->list[0]),20);// Select a random price
		...
		GLog::formatlog("SOLO_CHALLENGE_LOG AWARD user %d Obtained in single copy an amount of %d of item %d ,%d draws remaining, Top level %d .",pImp->_parent->ID.id, effective_count,item_id_rand,_left_draw_award_times, _max_stage_level);
	}
	else
	{
		return S2C::ERR_SOLO_CHALLENGE_AWARD_FAILURE;
...
int playersolochallenge::ScoreCost(gplayer_imp *pImp, int filter_index, int args[])
{
	...
	args[0] = _cur_score;
	args[1] = filter_index;
	GLog::log(GLOG_INFO,"SOLO_CHALLENGE_LOG SCORE User %d use %d points to change for skill %d remaining points %d, time%ld",pImp->_parent->ID.id,conf_score_cost->score_buff_list[filter_index].score_cost,filter_index, _cur_score, g_timer.get_systime());
	return 0;
}
