...
namespace ai_trigger
{
	...
	class expr_sphere_player_count : public expr //取npc特征变量数据
	{
		float _fRadius;

	public:
		expr_sphere_player_count(const float fRadius) : _fRadius(fRadius) {}
		virtual expr *Clone() const
		{
	...
	class expr_box_player_count : public expr //取npc特征变量数据
	{
		rect _rect;
	...
	public:
		op_get_sphere_player_count(int a, int b, int c, int d) : _radius(a), _rtype(b), _dest(c), _dtype(d) {}
		virtual operation *Clone() const
		{
	...
	public:
		op_get_box_player_count(float l, float t, float r, float b, int dest, int dtype) : _rect(l, t, r, b), _dest(dest), _dtype(dtype) {}
		virtual operation *Clone() const
		{
