...
object_interface::SetBodyScale(float scale) {
	((gplayer_imp *)_imp)->SetBodyScale(scale);
}
int
...
void object_interface::SendAttack(const XID * target, attack_msg & attack)
{
	_imp->SendAttackMsg(*target, attack);
}
//技能
...
//	_imp->_runner->get_extprop_attack();
	_imp->_runner->self_get_property(_imp->_basic.status_point,_imp->_cur_prop,
									...
									_imp->_penetration, _imp->_resilience, _imp->GetVigour(), _imp->_anti_defense_degree, _imp->_anti_resistance_degree, _imp->_damage_reduce, _imp->_magic_damage_reduce[0], _imp->_skill.GetPraySpeed(), _imp->_task_count,_imp->_newstats_info._killed_monsters,_imp->_newstats_info._killed_players,_imp->_newstats_info._death_by_monsters,_imp->_newstats_info._death_by_players);
}
...
void object_interface::ImpairSoulPower(int val)
{
	_imp->ImpairSoulPower(val);
}
void object_interface::EnhanceSoulPower(int val)
{
	_imp->EnhanceSoulPower(val);
}
void object_interface::EnhanceAllScaleResistance(int res)
{
	for(int cls = 0; cls < MAGIC_CLASS; cls++)
		_imp->_en_percent.resistance[cls] += res;
}
void object_interface::ImpairAllScaleResistance(int res)
{
	for(int cls = 0; cls < MAGIC_CLASS; cls++)
		_imp->_en_percent.resistance[cls] -= res;
}
