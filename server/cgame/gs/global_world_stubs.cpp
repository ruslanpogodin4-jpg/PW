...
int InitWorld(const char *gmconf_file, const char *servername)
{
	...
	while ((token = tok.token()))
	{
		...
		if (strcmp(token, servername) == 0)
		{
			...
			else if (atoi(conf->find(str, "arenabattle_server").c_str()) == 1)
			{
				is_instance = INIT_ARENABATTLE;
			}
			else
			{
	...
	case INIT_MNFACTIONBATTLE:
		...
		return iwm->Init(gmconf_file, servername);
	case INIT_ARENABATTLE:
		__PRINTINFO("Arena Battle Server\n");
		iwm = new arenabattle_world_manager();
		return iwm->Init(gmconf_file, servername);
	default:
