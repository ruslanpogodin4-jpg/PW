...
class aei_arena_filter : public filter
{
	int _team_id;
	int _battle_result;
	int _battle_end_timer;
...
public:
	DECLARE_SUBSTANCE(aei_arena_filter);
	aei_arena_filter(gactive_imp *imp, int filter_id, int team_id)
		: filter(object_interface(imp), MASK), _state(NORMAL), _timeout(0), _team_id(team_id)
	{
		_filter_id = filter_id;
		_battle_result = 0;
		_battle_end_timer = 0;
...
protected:
	...
	aei_arena_filter() : _timeout(0), _team_id(0) {}
	virtual void OnModify(int ctrlname, void *ctrlval, size_t ctrllen);
};
