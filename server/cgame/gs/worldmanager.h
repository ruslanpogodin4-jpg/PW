...
public:
	...
	void InsertTransmitEntry(int src_wp, const TRANSMIT_ENTRY &entry)
	{
		...
		{
			for (TRANSMIT_MAP::iterator ie = w_transmit_map.upper_bound(src_wp); it != ie; ++it)
				if (it->second.target_waypoint == entry.target_waypoint)
					return;
		}
...
public:
	...
	virtual bool CreateArenaBattle(const arenabattle_param &) { return false; } //收到要求创建战场的协议
...
public:
	virtual bool IsArenaBattleWorld() { return false; }
