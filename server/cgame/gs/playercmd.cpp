...
int gplayer_controller::MessageHandler(world *pPlane, const MSG &msg)
{
	return 0;
}
void gplayer_controller::ResurrectByItem(float exp_reduce, int param)
{
...
void gplayer_controller::ResurrectInTown(float exp_reduce, int param)
{
	A3DVECTOR pos;
	gplayer_imp *pImp = ((gplayer_imp *)_imp);
	if (!pImp->CanResurrect(param))
...
void gplayer_controller::ResurrectByCash(float exp_reduce, int param)
{
	gplayer_imp *pImp = (gplayer_imp *)_imp;
	if (!pImp->CanResurrect(param))
		return;
...
int gplayer_controller::UnLockInventoryHandler(int cmd_type, const void *buf, size_t size)
{
	switch (cmd_type)
	{
...
int gplayer_controller::MarketCommandHandler(int cmd_type, const void *buf, size_t size)
{
	if (!CheckBanish())
		return 0;
	switch (cmd_type)
...
int gplayer_controller::CosmeticCommandHandler(int cmd_type, const void *buf, size_t size)
{
	if (!CheckBanish())
		return 0;
...
int gplayer_controller::ZombieCommandHandler(int cmd_type, const void *buf, size_t size)
{
	if (!CheckBanish())
		return 0;
	switch (cmd_type)
	...
	case C2S::RESURRECT_IN_TOWN:
		_load_stats++;
		{
			...
			if (pImp->_basic.level <= LOW_PROTECT_LEVEL)
				exp_reduce = 0.f;
	...
	case C2S::RESURRECT_BY_CASH:
		_load_stats++;
		{
			...
			if (world_manager::GetWorldLimit().nocash_resurrect)
				break;
...
int gplayer_controller::StayInCommandHandler(int cmd_type, const void *buf, size_t size)
{
	if (!CheckBanish())
		return 0;
	switch (cmd_type)
...
int gplayer_controller::BoundCommandHandler(int cmd_type, const void *buf, size_t size)
{
	if (!CheckBanish())
		return 0;
	switch (cmd_type)
...
int gplayer_controller::TravelCommandHandler(int cmd_type, const void *buf, size_t size)
{
	if (!CheckBanish())
		return 0;
	switch (cmd_type)
...
int gplayer_controller::SealedCommandHandler(int cmd_type, const void *buf, size_t size)
{
	if (!CheckBanish())
		return 0;
	gplayer_imp *pPlayer = (gplayer_imp *)_imp;
...
int gplayer_controller::CommandHandler(int cmd_type, const void *buf, size_t size)
{
	if (!CheckBanish())
		return 0;
	switch (cmd_type)
...
#pragma pack(pop)
		if (size == sizeof(ColorNamePacket))
		{
			ColorNamePacket &cmd = *(ColorNamePacket *)buf;
			gplayer_imp *pImp = (gplayer_imp *)_imp;
			pImp->SetColorName(cmd.color);
		}
	...
	case C2S::SELECT_TARGET:
	{
		...
		return 0;
	case C2S::NORMAL_ATTACK:
		if (CheckDeny(CMD_ATTACK))
			return 0;
		if (CheckDeny(CMD_NORMAL_ATTACK))
			return 0;
		{
	...
	case C2S::PICKUP:
			...
			return 0;
		{
			...
			if (_pickup_counter > MAX_PICKUP_PER_SECOND)
				_pickup_counter = MAX_PICKUP_PER_SECOND;
			else if (_pickup_counter < -1024)
				_pickup_counter = -5;
			_pickup_timestamp = ts;

			_pickup_counter--;
			if (_pickup_counter <= 0)
				break; //超过了最大捡取速度
			...
			if (!pImp->_plane->QueryObject(obj, info) || (info.race & 0x0000FFFF) != (int)type)
			{
	...
	case C2S::SERVICE_HELLO:
	{
		...
		/*bool vipperm = pImp->CheckVipService(CVS_REPAIR);*/

		if (cmd.id == REMOTE_NPCID)
		{
			/*if (!vipperm)
			{
			...
			pImp->_plane->GetSceneServiceNpc(list);
			for (size_t i = 0; i < list.size(); i++)
			{
				if (list[i] == REMOTE_NPCID)
				{
					vipservice = list[i + 1];
				}
			}
			MAKE_ID(target, vipservice);
		}
		else
		{
			MAKE_ID(target, cmd.id);
		...
		if (vipservice != 0)
		{
			((gplayer_imp *)_imp)->SayHelloToNPC(target);
		}
		else if (cmd.id != vipservice)
		{
	...
	case C2S::GET_OWN_WEALTH:
	{
		...
		pImp->_runner->get_player_money(pImp->GetMoney(), pImp->_money_capacity, pImp->GetColorName());
	}
	break;
	...
	case C2S::CONCURRECT_EMOTE_REQUEST:
	{
		...
		if (!pImp->_plane->QueryObject(id, info) || info.pos.squared_distance(pImp->_parent->pos) >= 2.0f * 2.0f)
		{
	...
	case C2S::QUERY_CASH_INFO:
	{
		...
		if (world_manager::GetWorldParam().korea_shop)
			GNET::SendBillingBalance(pImp->_parent->ID.id);
		else if (world_manager::GetWorldParam().southamerican_shop)
			GNET::SendBillingBalanceSA(pImp->_parent->ID.id);
	}
	break;
	// lgc
	case C2S::ELF_ADD_ATTRIBUTE:
	...
	case C2S::MULTI_EXCHANGE_ITEM:
	{
		...
		if (size < sizeof(C2S::CMD::multi_exchange_item) || !mei.count || size != sizeof(C2S::CMD::multi_exchange_item) + mei.count * sizeof(C2S::CMD::multi_exchange_item::_operation))
		{
			error_cmd(S2C::ERR_FATAL_ERR);
			break;
		}
		...
		if (!pImp->CheckCoolDown(COOLDOWN_INDEX_MULTI_EXCHANGE_ITEM))
			break;
	...
	case C2S::SYSAUCTION_OP:
	{
		...
		if (size < sizeof(C2S::CMD::sysauction_op))
			break;
		if (world_manager::GetWorldParam().korea_shop || world_manager::GetWorldParam().southamerican_shop)
			break;
	...
	case C2S::CALC_NETWORK_DELAY:
	{
		...
		if (size != sizeof(C2S::CMD::calc_network_delay))
			break;
		if (cnd.timestamp)
		{
	...
	case C2S::GET_FACTION_FORTRESS_INFO:
	{
		...
		if (size != sizeof(C2S::CMD::get_faction_fortress_info))
			break;
	...
	case C2S::CONGREGATE_REPLY:
	{
		...
		if (size != sizeof(C2S::CMD::congregate_reply))
			break;
	...
	case C2S::GET_FORCE_GLOBAL_DATA:
	{
		...
		if (size != sizeof(C2S::CMD::get_force_global_data))
			break;
		size_t count;
		const void *data;
		size_t data_size;
	...
	{
		if (size != sizeof(C2S::CMD::produce4_choose))
			break;
	...
	case C2S::RECHARGE_ONLINE_AWARD:
	{
		...
		if (size < sizeof(C2S::CMD::recharge_online_award) || !roa.count || roa.count > 32 || size != sizeof(C2S::CMD::recharge_online_award) + roa.count * sizeof(C2S::CMD::recharge_online_award::entry))
			break;
	...
	case C2S::TOGGLE_ONLINE_AWARD:
	{
		...
		if (size != sizeof(C2S::CMD::toggle_online_award))
			break;

		gplayer_imp *pImp = (gplayer_imp *)_imp;
		if (!pImp->CheckCoolDown(COOLDOWN_INDEX_TOGGLE_ONLINE_AWARD))
			break;
	...
	{
		if (size != sizeof(C2S::CMD::query_profit_time))
			break;
		gplayer_imp *pImp = (gplayer_imp *)_imp;
	...
	case C2S::COUNTRYBATTLE_GET_PERSONAL_SCORE:
	{
		if (size != sizeof(C2S::CMD::countrybattle_get_personal_score))
			break;
		gplayer_imp *pImp = (gplayer_imp *)_imp;
	...
	case C2S::COUNTRYBATTLE_GET_STRONGHOLD_STATE:
	{
		if (size != sizeof(C2S::CMD::countrybattle_get_stronghold_state))
			break;
		gplayer_imp *pImp = (gplayer_imp *)_imp;
	...
	case C2S::QUERY_TOUCH_POINT:
	{
		if (size != sizeof(C2S::CMD::query_touch_point))
			break;
	...
	case C2S::COST_TOUCH_POINT:
	{
		if (size != sizeof(C2S::CMD::cost_touch_point))
			break;
	...
	case C2S::QUERY_TITLE:
	{
		if (size != sizeof(C2S::CMD::query_title))
			break;
	...
	case C2S::CHANGE_CURR_TITLE:
	{
		if (size != sizeof(C2S::CMD::change_title))
			break;
	...
	case C2S::DAILY_SIGNIN:
	{
		if (size != sizeof(C2S::CMD::daily_signin))
			break;
	...
	case C2S::LATE_SIGNIN:
	{
		if (size != sizeof(C2S::CMD::late_signin))
			break;
	...
	case C2S::APPLY_SIGNIN_AWARD:
	{
		if (size != sizeof(C2S::CMD::apply_signinaward))
			break;
	...
	case C2S::REFRESH_SIGNIN:
	{
		if (size != sizeof(C2S::CMD::refresh_signin))
			break;
	...
	case C2S::SWITCH_IN_PARALLEL_WORLD:
	{
		if (size != sizeof(C2S::CMD::switch_in_parallel_world))
			break;
	...
	case C2S::QUERY_PARALLEL_WORLD:
	{
		if (size != sizeof(C2S::CMD::query_parallel_world))
			break;
	...
	case C2S::QUERY_UNIQUE_DATA:
	{
		if (size < sizeof(C2S::CMD::query_unique_data))
			break;
		C2S::CMD::query_unique_data &ct = *(C2S::CMD::query_unique_data *)buf;
		if (size != (sizeof(C2S::CMD::query_unique_data) + ct.count * sizeof(int)))
			break;
	...
	case C2S::GET_REINCARNATION_TOME:
	{
		if (size != sizeof(C2S::CMD::get_reincarnation_tome))
			break;
	...
	case C2S::REWRITE_REINCARNATION_TOME:
	{
		if (size != sizeof(C2S::CMD::rewrite_reincarnation_tome))
			break;
	...
	case C2S::ACTIVATE_REINCARNATION_TOME:
	{
		if (size != sizeof(C2S::CMD::activate_reincarnation_tome))
			break;
	...
	case C2S::AUTO_TEAM_SET_GOAL:
	{
		if (size != sizeof(C2S::CMD::auto_team_set_goal))
			break;
	...
	case C2S::AUTO_TEAM_JUMP_TO_GOAL:
	{
		if (size != sizeof(C2S::CMD::auto_team_jump_to_goal))
			break;
	...
	{
		if (size != sizeof(C2S::CMD::trickbattle_upgrade_chariot))
			break;
	...
	case C2S::SWALLOW_GENERALCARD:
	{
		if (size != sizeof(C2S::CMD::swallow_generalcard))
			break;
	...
	case C2S::EQUIP_TRASHBOX_ITEM:
	{
		...
		//暂时只允许从卡牌仓库装备物品
		if (cmd.where != gplayer_imp::IL_TRASH_BOX4)
			break;
	...
	{
		if (size < sizeof(C2S::CMD::send_mass_mail))
			break;
		{
	...
	case C2S::RANDOM_MALL_SHOPPING:
	{
		if (world_manager::GetWorldParam().korea_shop || world_manager::GetWorldParam().southamerican_shop)
			break;
		if (world_manager::GetWorldParam().random_shop_limit)
			break;
		if (size != sizeof(C2S::CMD::random_mall_shopping))
			break;
	...
	case C2S::QUERY_CAN_INHERIT_ADDONS:
	{
		if (size != sizeof(C2S::CMD::query_can_inherit_addons))
			break;
	...
	case C2S::ACTIVATE_REGION_WAYPOINTS:
	{
		...
		if ((size < sizeof(C2S::CMD::activate_region_waypoints)) || (size != sizeof(C2S::CMD::activate_region_waypoints) + cmd.num * sizeof(int)))
			break;
	...
	case C2S::INSTANCE_REENTER_REQUEST:
	{
		...
		if (size != sizeof(C2S::CMD::instance_reenter_request))
			break;
	...
	case C2S::ASTROLABE_OPERATE_REQUEST:
	{
		...
		{
			if (size == sizeof(C2S::CMD::astrolabe_operate_request) + sizeof(C2S::INFO::astrolabe_opt_swallow))
			{
		...
		{
			if (size == sizeof(C2S::CMD::astrolabe_operate_request) + sizeof(C2S::INFO::astrolabe_opt_addon_roll))
			{
		...
		{
			if (size == sizeof(C2S::CMD::astrolabe_operate_request) + sizeof(C2S::INFO::astrolabe_opt_aptit_inc))
			{
		...
		{
			if (size == sizeof(C2S::CMD::astrolabe_operate_request) + sizeof(C2S::INFO::astrolabe_opt_slot_roll))
			{
	...
	case C2S::PROPERTY_SCORE_REQUEST:
	{
		...
		if (size != sizeof(C2S::CMD::property_score_request))
			break;
	...
	case C2S::PICKUP_ALL:
			...
			return 0;
		{
			...
			for (size_t index = 0; index < (size_t)pms.count; ++index)
			{
	...
	case C2S::FIX_POSITION_TRANSMIT_OPERATE_REQUEST:
	{
		...
		{
			if (size == sizeof(C2S::CMD::fix_position_transmit_operate_request) + sizeof(C2S::INFO::fix_position_transmit_opt_add_position))
			{
		...
		{
			if (size == sizeof(C2S::CMD::fix_position_transmit_operate_request) + sizeof(C2S::INFO::fix_position_transmit_opt_delete_position))
			{
		...
		{
			if (size == sizeof(C2S::CMD::fix_position_transmit_operate_request) + sizeof(C2S::INFO::fix_position_transmit_opt_transmit))
			{
		...
		{
			if (size == sizeof(C2S::CMD::fix_position_transmit_operate_request) + sizeof(C2S::INFO::fix_position_transmit_opt_rename))
			{
	...
	case C2S::RELOAD_INSTANCE:
	{
		gplayer_imp *pImp = (gplayer_imp *)_imp;
		pImp->ReloadInstance();
	}
	break;
	case C2S::ARENA_REQUEST_BATTLE_QUEUE:
	{
		C2S::CMD::arena_request_battle_queue &arbq = *(C2S::CMD::arena_request_battle_queue *)buf;
		if (size != sizeof(C2S::CMD::arena_request_battle_queue))
		{
			error_cmd(S2C::ERR_FATAL_ERR);
			break;
		}
		gplayer_imp *pImp = (gplayer_imp *)_imp;
		pImp->PlayerArenaQueueEnter(arbq.battle_mode);
	}
	break;

	case C2S::ARENA_ACCEPT_BATTLE:
	{
		C2S::CMD::arena_accept_battle &arat = *(C2S::CMD::arena_accept_battle *)buf;
		if (size != sizeof(C2S::CMD::arena_accept_battle))
		{
			error_cmd(S2C::ERR_FATAL_ERR);
			GLog::log(GLOG_INFO, "Size of Arena Accept packet invalid");
		...
		if (arat.accepted)
		{
			GLog::log(GLOG_INFO, "Player Entered Step2 of Arena");

			pImp->ArenaJoinStep2();
		}
		else
		{
			pImp->_arena_battle_id = 0;
			pImp->_arena_world_tag = 0;
			pImp->_arena_enter_elapsed_time = 0;
			GLog::log(GLOG_INFO, "Player Refuse Step2 of Arena");
		}
	}
	break;
	case C2S::ARENA_EXIT_BATTLE:
	{
		gplayer_imp *pImp = (gplayer_imp *)_imp;
		pImp->_filters.ModifyFilter(FILTER_CHECK_INSTANCE_KEY, FMID_LEAVE_AEI_ARENA, NULL, 0);
	}
	break;
...
int gplayer_controller::GMCommandHandler(int cmd_type, const void *buf, size_t size)
{
#define DEFCMD(type)                              \
	C2S::CMD::type &cmd = *(C2S::CMD::type *)buf; \
	if (size != sizeof(cmd))                      \
	{                                             \
		error_cmd(S2C::ERR_FATAL_ERR);            \
		break;                                    \
	}
#define TESTGMPRIVILEGE(X)                     \
	if (!_gm_auth->X())                        \
	{                                          \
	...
	case C2S::GMCMD_REMOVE_SPEC_ITEM:
	{
		...
		if (cmd.roleid <= 0 || cmd.type <= 0 || cmd.where >= gplayer_imp::IL_MAX || cmd.count == 0)
			break;
		XID target(GM_TYPE_PLAYER, cmd.roleid);
	...
	case C2S::GMCMD_CHANGE_DS:
	{
		...
		if (cmd.flag != GMSV::CHDS_FLAG_DS_TO_CENTRALDS && cmd.flag != GMSV::CHDS_FLAG_CENTRALDS_TO_DS)
			break;
...
int gplayer_controller::DebugCommandHandler(int cmd_type, const void *buf, size_t size)
{
...
#pragma pack()
	...
	case 10816:
	{
		...
		if (size < 2 + 4 || size > 2 + 10 * 4 || (size - 2) % 4 != 0)
		{
			break;
		}
...
#pragma pack()
	...
	case 10834:
	{
		...
		gplayer_imp *pImp = (gplayer_imp *)_imp;
		if (repu > 0)
			pImp->IncForceReputation(repu);
		else if (repu < 0)
			pImp->DecForceReputation(-repu);
		if (contrib > 0)
			pImp->IncForceContribution(contrib);
		else if (contrib < 0)
			pImp->DecForceContribution(-contrib);
	}
	break;
...
#pragma pack()
		...
		if (ccc->which == 0)
		{
			if (ccc->index >= ROLE_REPUTATION_UCHAR_SIZE)
				break;
			value = ++pImp->_role_reputation_uchar[ccc->index];
		}
		else if (ccc->which == 1)
		{
			if (ccc->index >= ROLE_REPUTATION_USHORT_SIZE)
				break;
			value = ++pImp->_role_reputation_ushort[ccc->index];
		}
		else if (ccc->which == 2)
		{
			if (ccc->index >= ROLE_REPUTATION_UINT_SIZE)
				break;
...
#pragma pack()
		...
		((gplayer_imp *)_imp)->ChangeEquipAddon((unsigned char)req->equip_idx, (unsigned char)req->equip_socket_idx, req->old_stone_type, req->new_stone_type, req->recipe_type, req->materials_id, (unsigned char *)req->idx, 1);
	}
	break;
...
#pragma pack()
		...
		((gplayer_imp *)_imp)->ReplaceEquipAddon((unsigned char)req->equip_idx, (unsigned char)req->equip_socket_idx, req->old_stone_type, req->new_stone_type, (unsigned char)req->new_stone_idx);
	}
	break;
	case 10814:
	{
...
#pragma pack()
	...
	case 4873:
	{
		...
		else
		{
			...
			if (!cfg || dt != DT_GT_CONFIG)
				break;
			if (cfg->inc_attack_degree < 0 || cfg->inc_defend_degree < 0)
				break;
...
int gplayer_controller::cmd_user_move(const void *buf, size_t size)
{
	...
	if (pImp->AddSession(pMove))
		pImp->StartSession();
	return 0;
}

int gplayer_controller::cmd_user_stop_move(const void *buf, size_t size)
{
