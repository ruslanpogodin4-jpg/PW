...
bool TeamRelationJob::OnRun()
{
	...
	for (size_t i = 0; i < _list.size(); i++)
	{
		...
		if (plist.end() != std::find(plist.begin(), plist.end(), pPlayer))
			return false; // 会重复lock导致死锁
	...
	try
	{
		...
		for (size_t i = 0; i < _list.size(); i++)
		{
			...
			if (pPlayer->pos.squared_distance(pos) > 30.f * 30.f) {
				SetErrorMsg(S2C::ERR_TEAM_TOO_FAR);
				throw -5;
			}
			if (!pPlayer->imp)
				throw -6;
...
bool WeddingBookJob::OnTeamCheck(gplayer **list, size_t count)
{
	...
	if (!married1 || !married2)
		return false;
	if (list[0]->spouse_id != list[1]->ID.id)
		return false;
	if (list[1]->spouse_id != list[0]->ID.id)
		return false;
	return true;
}
...
bool WeddingBookJob::OnLeaderCheck(gplayer_imp *pImp)
{
	...
	if (!pImp->CheckItemExist(WEDDING_BOOK_TICKET_ID, 1))
		return false;
	...
	if (bookcard_index >= 0)
	{
		...
		if ((size_t)bookcard_index >= inv.Size())
			return false;
...
bool WeddingCancelBookJob::OnLeaderCheck(gplayer_imp *pImp)
{
	...
	while ((rst = inv.Find(rst, WEDDING_INVITECARD_ID1)) >= 0)
	{
		...
		{
			if (start_time == wedding_start_time && end_time == wedding_end_time && scene == wedding_scene && (groom == self || bride == self) && invitee == self)
				break;
		}
		rst++;
	}
...
bool ArenaJoinQueueJob::OnTeamCheck(gplayer **list, size_t count)
{
	bool valid = true;
	int classes[USER_CLASS_COUNT] = {0};

	for(int i = 0; i < count ; i++){
		gplayer_imp *pImp = (gplayer_imp *)list[i]->imp;
		classes[pImp->GetPlayerClass()]++;
	}

	for (int i = 0; i < USER_CLASS_COUNT; i++){
		if(classes[i] > 1) {
			valid = false;
			SetErrorMsg(S2C::ERR_ARENA_TEAM_INVALID_CLASS);
			break;
		}
	}
...
bool ArenaJoinQueueJob::OnLeaderCheck(gplayer_imp *pImp)
{
	return OnMemberCheck(pImp);
}

bool ArenaJoinQueueJob::OnMemberCheck(gplayer_imp *pImp)
{
	if (!pImp->CheckCoolDown(COOLDOWN_INDEX_ARENA_QUEUE_APPLY))
	{
		SetErrorMsg(S2C::ERR_OBJECT_IS_COOLING);
		return false;
	}
	if (!pImp->IsItemExist(ticket))
	{
		SetErrorMsg(S2C::ERR_ARENA_TEAM_TICKET_MISSING);
		return false;
	}

	if (world_manager::GetWorldTag() != 1){
		SetErrorMsg(S2C::ERR_ARENA_TEAM_MAP_ENTRANCE);
		return false;
	}
...
bool ArenaJoinQueueJob::OnExecute(gplayer **list, size_t count)
{
	GMSV::IntraArenaApplyEntry entry_list[count];

	for (size_t i = 0; i < count; i++)
	{
		gplayer_imp *pImp = (gplayer_imp *)(list[i]->imp);
		pImp->SetCoolDown(COOLDOWN_INDEX_ARENA_QUEUE_APPLY, ARENA_QUEUE_APPLY_COOLDOWN_TIME);
		pImp->_runner->player_arena_queue_notification(g_timer.get_systime());
		entry_list[i].roleid = pImp->_parent->ID.id;
		entry_list[i].total_victory = pImp->_arenastats_info._total_victory;
		entry_list[i].total_defeat = pImp->_arenastats_info._total_defeat;
		entry_list[i].kill_number = pImp->_arenastats_info._total_kills;
		entry_list[i].death_number = pImp->_arenastats_info._total_deaths;
		GLog::formatlog("Requested enter matchmaking for the arena roleid: %d\n", pImp->_parent->ID.id);
	}
	GMSV::GMArenaBattleQueueApply(entry_list, count, battle_mode, teamid);
