...
namespace S2C
{
	namespace CMD
	{
		...
		inline unsigned int MakeObjectState(T *pObject)
		{
			...
			if (pObject->IsZombie())
				state = gactive_object::STATE_ZOMBIE;
			if (pObject->extend_state || pObject->extend_state2 || pObject->extend_state3 || pObject->extend_state4 || pObject->extend_state5 || pObject->extend_state6)
				state |= gactive_object::STATE_EXTEND_PROPERTY;
			if (pObject->effect_count)
				state |= gactive_object::STATE_EFFECT;
			return state;
		}
		...
		inline WRAPPER &MakePlayerExtendState(WRAPPER &ar, gplayer *pPlayer, unsigned int state, unsigned int state2)
		{
			...
			if (state2 & gactive_object::STATE_BODY_SIZE)
			{
				ar << pPlayer->body_scale;
			}
			return ar;
		}
		...
		inline WRAPPER &MakeNPCExtendState(WRAPPER &ar, gnpc *pObject, unsigned int state)
		{
			...
			if (state & gactive_object::STATE_NPC_NAME)
			{
				...
				if (name_size >= sizeof(pObject->npc_name))
					name_size = sizeof(pObject->npc_name);
				ar << name_size;
		...
		template <>
		struct Make<CMD::arena_queue_notification>
		{
			template <typename WRAPPER>
			inline static WRAPPER &From(WRAPPER &wrapper, int timestamp_start)
			{
				Make<single_data_header>::From(wrapper, ARENA_QUEUE_NOTIFICATION);
				return wrapper << timestamp_start;
			}
		};

		template <>
		struct Make<CMD::arena_queue_cancel>
		{
			template <typename WRAPPER>
			inline static WRAPPER &From(WRAPPER &wrapper, int failure_reason)
			{
				Make<single_data_header>::From(wrapper, ARENA_QUEUE_CANCEL);
				return wrapper << failure_reason;
			}
		};

		template <>
		struct Make<CMD::arena_queue_confirm_battle>
		{
			template <typename WRAPPER>
			inline static WRAPPER &From(WRAPPER &wrapper, int team_id, int battle_id)
			{
				Make<single_data_header>::From(wrapper, ARENA_QUEUE_CONFIRM_BATTLE);
				return wrapper << team_id << battle_id;
			}
		};

		template <>
		struct Make<CMD::enter_arena_battleground>
		{
			template <typename WRAPPER>
			inline static WRAPPER &From(WRAPPER &wrapper, int role_in_war, int battle_id, int end_time)
			{
				Make<single_data_header>::From(wrapper, ENTER_ARENA_BATTLEGROUND);
				return wrapper << role_in_war << battle_id << end_time;
			}
		};

		template <>
		struct Make<CMD::arena_score_update>
		{
			template <typename WRAPPER>
			inline static WRAPPER &From(WRAPPER &wrapper, int TeamInvaderSize, int TeamDefenderSize, int TeamInvaderScore, int TeamDefenderScore, int count)
			{
				Make<single_data_header>::From(wrapper, ARENA_SCORE_UPDATE);
				return wrapper << TeamInvaderSize << TeamDefenderSize << TeamInvaderScore << TeamDefenderScore << count;
			}

			template <typename WRAPPER>
			inline static WRAPPER &AddRole(WRAPPER &wrapper, int roleid)
			{
		...
		template <>
		struct Make<CMD::arena_end_result>
		{
			template <typename WRAPPER>
			inline static WRAPPER &From(WRAPPER &wrapper, int InvaderPoints,
										int InvaderDmg,
										...
										int BattleMode,
										int CountInvader,
										int CountDefender, const void *data, size_t data_size)
			{
				Make<single_data_header>::From(wrapper, ARENA_END_RESULT);
				wrapper << InvaderPoints << InvaderDmg << InvaderSurvivors << DefenderPoints << DefenderDmg << DefenderSurvivors << BattleResult << Winner << BattleMode << CountInvader << CountDefender;
				wrapper.push_back(data, data_size);
				return wrapper;
			}

			//	template <typename WRAPPER>
			//	inline static WRAPPER &AddEnemies(WRAPPER &wrapper, int roleid, int occupation, int victories, int rank, const char *rolename)
			//	{
			//		wrapper << roleid << occupation << victories << rank;
			//		wrapper.push_back(rolename, 80);
			//		return wrapper;
			//	}
		};
		...
		struct Make<CMD::self_get_property>
		{
			...
			inline static WRAPPER &From(WRAPPER &wrapper, size_t status_point, const struct extend_prop &prop, int attack_degree, int defend_degree, int crit_rate, int crit_damage_bonus, int invisible_degree, int anti_invisible_degree, int penetration, int resilience, int vigour, int anti_def_degree, int anti_resist_degree, int damage_reduce, int magic_damage_reduce, int pray_speed, int task_count, int killed_monsters, int killed_players, int death_by_monsters, int death_by_players)
			{
				...
				return wrapper << status_point << attack_degree << defend_degree << crit_rate << crit_damage_bonus << invisible_degree << anti_invisible_degree << penetration << resilience << vigour << anti_def_degree << anti_resist_degree << damage_reduce << magic_damage_reduce << pray_speed << task_count << killed_monsters << killed_players << death_by_monsters << death_by_players << prop;
			}
		};
		...
		struct Make<CMD::send_equipment_info>
		{
			...
			inline static WRAPPER &From(WRAPPER &wrapper, gplayer *pPlayer, uint64_t mask, OCTETS &data, unsigned int color_name)
			{
				Make<single_data_header>::From(wrapper, EQUIPMENT_DATA);
				wrapper << color_name << (unsigned short)pPlayer->crc << pPlayer->ID.id << mask;
		...
		{
			template <typename WRAPPER>
			inline static WRAPPER &From(WRAPPER &wrapper, gplayer *pPlayer, uint64_t mask_add, uint64_t mask_del, const void *buf, size_t size, unsigned int color_name)
			{
				Make<single_data_header>::From(wrapper, EQUIPMENT_INFO_CHANGED);
				wrapper << color_name << (unsigned short)pPlayer->crc << pPlayer->ID.id << mask_add << mask_del;
				if (size)
				{
					ASSERT(size % 4 == 0);
		...
		{
			template <typename WRAPPER>
			inline static WRAPPER &From(WRAPPER &wrapper, size_t amount, size_t capacity, unsigned int color_name)
			{
				Make<single_data_header>::From(wrapper, GET_OWN_MONEY);
				return wrapper << amount << capacity << color_name;
			}
		};
		...
		struct Make<CMD::object_state_notify>
		{
			...
			inline static WRAPPER &From(WRAPPER &wrapper, const XID &id, unsigned short *state_list, size_t state_count, int *param_list, size_t param_count)
			{
				...
				wrapper << id.id << count;
				if (count)
					wrapper.push_back(state_list, ((int)count) * sizeof(unsigned short));
				count = param_count;
				wrapper << count;
				if (count)
					wrapper.push_back(param_list, ((int)count) * sizeof(int));
				return wrapper;
			}
		};
		...
		struct Make<CMD::player_waypoint_list>
		{
			...
			inline static WRAPPER &From(WRAPPER &wrapper, const unsigned short *wlist, size_t count)
			{
				...
				wrapper << count;
				if (count)
					wrapper.push_back(wlist, sizeof(unsigned short) * count);
				return wrapper;
			};
		};
		...
		struct Make<CMD::query_title>
		{
										...
										const void *data, size_t data_size, const void *edata, size_t edata_size)
			{
				...
				if (data_size)
					wrapper.push_back(data, data_size);
				if (edata_size)
					wrapper.push_back(edata, edata_size);
		...
		struct Make<CMD::scene_service_npc_list>
		{
			...
			inline static WRAPPER &From(WRAPPER &wrapper, unsigned int count, int *data)
			{
				...
				wrapper << count;
				if (count)
					wrapper.push_back(data, count * sizeof(int) * 2);
				return wrapper;
			}
		};
		...
		struct Make<CMD::equip_can_inherit_addons>
		{
			...
			inline static WRAPPER &From(WRAPPER &wrapper, int equip_id, unsigned char inv_idx, unsigned char addon_num, const int *addon_id_list)
			{
				...
				if (addon_num > 0)
					wrapper.push_back(addon_id_list, addon_num * sizeof(int));
				return wrapper;
			}
		};
		...
		{
			template <typename WRAPPER>
			inline static WRAPPER &From(WRAPPER &wrapper, int id, float scale)
			{
