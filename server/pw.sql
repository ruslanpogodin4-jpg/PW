...
--
-- Host: localhost:3306
-- Tempo de geração: 14-Fev-2023 às 14:47
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateUserInfo` (IN `name1` VARCHAR(32), IN `prompt1` VARCHAR(32), IN `answer1` VARCHAR(32), IN `truename1` VARCHAR(64), IN `idnumber1` VARCHAR(32), IN `email1` VARCHAR(32), IN `mobilenumber1` VARCHAR(32), IN `province1` VARCHAR(32), IN `city1` VARCHAR(32), IN `phonenumber1` VARCHAR(32), IN `address1` VARCHAR(32), IN `postalcode1` VARCHAR(32), IN `gender1` INTEGER, IN `birthday1` VARCHAR(32), IN `qq1` VARCHAR(32))
BEGIN
  START TRANSACTION;
...
CREATE TABLE `users` (
  ...
  `passwd` varchar(64) CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci DEFAULT NULL,
...
--
-- Índices para tabela `forbid`
--
ALTER TABLE `forbid`
  ADD PRIMARY KEY (`userid`,`type`);

--
-- Índices para tabela `iplimit`
--
ALTER TABLE `iplimit`
  ADD PRIMARY KEY (`uid`);
...
--
-- Índices para tabela `usecashlog`
--
...
--
-- Índices para tabela `usecashnow`
--
